...
from ecotrace.modules.analytics.application.query_service import (
    aggregate_items,
    ...
    rows = (
        db.execute(
            select(SustainabilityBaseline).where(
                SustainabilityBaseline.organization_id == organization_id,
                SustainabilityBaseline.baseline_type == baseline_type,
                SustainabilityBaseline.is_primary.is_(True),
            )
        )
        .scalars()
        .all()
...
def list_intensity_defs(
    db: Session, user: User, organization_id: uuid.UUID, *, page: int, page_size: int
) -> Page[IntensityDefResponse]:
    require_org_read(db, user, organization_id)
    total = db.execute(
        select(func.count())
        .select_from(IntensityMetricDefinition)
        .where(IntensityMetricDefinition.organization_id == organization_id)
    ).scalar_one()
    rows = (
        db.execute(
            select(IntensityMetricDefinition)
            .where(IntensityMetricDefinition.organization_id == organization_id)
            .order_by(IntensityMetricDefinition.code)
    ...
    return paginate(
        [_intensity_response(r) for r in rows], page=page, page_size=page_size, total_items=total
    )


def create_intensity_def(
    db: Session,
    user: User,
    organization_id: uuid.UUID,
    payload: IntensityDefCreate,
    *,
    request_id: str | None = None,
    ip_address: str | None = None,
    user_agent: str | None = None,
) -> IntensityDefResponse:
    ...
    existing = db.execute(
        select(IntensityMetricDefinition).where(
            ...
            IntensityMetricDefinition.code == payload.code.strip(),
        )
    ).scalar_one_or_none()
    if existing:
        raise ConflictError("Intensity metric code already exists.")
    row = IntensityMetricDefinition(
        organization_id=organization_id,
        code=payload.code.strip(),
        name=payload.name.strip(),
        description=payload.description,
        numerator_type=payload.numerator_type,
        denominator_activity_type_id=payload.denominator_activity_type_id,
        denominator_unit_code=payload.denominator_unit_code,
    ...
    write_audit_log(
        db,
        action="intensity_metric.created",
        actor_user_id=user.id,
        organization_id=organization_id,
        entity_type="intensity_metric_definition",
...
def update_intensity_def(
    db: Session,
    user: User,
    organization_id: uuid.UUID,
    definition_id: uuid.UUID,
    payload: IntensityDefUpdate,
) -> IntensityDefResponse:
    require_planning_write(db, user, organization_id)
...
def list_kpi_defs(
    ...
    total = db.execute(
        select(func.count())
        .select_from(EnvironmentalKpiDefinition)
        .where(EnvironmentalKpiDefinition.organization_id == organization_id)
    ).scalar_one()
    rows = (
        db.execute(
            select(EnvironmentalKpiDefinition)
            .where(EnvironmentalKpiDefinition.organization_id == organization_id)
            .order_by(EnvironmentalKpiDefinition.code)
            .offset((page - 1) * page_size)
            .limit(page_size)
        )
        .scalars()
    ...
    return paginate(
        [_kpi_response(r) for r in rows], page=page, page_size=page_size, total_items=total
    )
    ...
    *,
    ...
    existing = db.execute(
        select(EnvironmentalKpiDefinition).where(
            ...
            EnvironmentalKpiDefinition.code == payload.code.strip(),
        )
    ).scalar_one_or_none()
    if existing:
        raise ConflictError("KPI definition code already exists.")
    row = EnvironmentalKpiDefinition(
        organization_id=organization_id,
        code=payload.code.strip(),
        name=payload.name.strip(),
        description=payload.description,
    ...
    write_audit_log(
        db,
        action="environmental_kpi.created",
        actor_user_id=user.id,
        organization_id=organization_id,
        entity_type="environmental_kpi_definition",
        entity_id=str(row.id),
        request_id=request_id,
        ip_address=ip_address,
        user_agent=user_agent,
...
def list_baselines(
    ...
    *,
...
) -> Page[BaselineResponse]:
    require_org_read(db, user, organization_id)
    stmt = select(SustainabilityBaseline).where(
        SustainabilityBaseline.organization_id == organization_id
    )
    count_stmt = (
        select(func.count())
        .select_from(SustainabilityBaseline)
        .where(SustainabilityBaseline.organization_id == organization_id)
    )
    if status:
    ...
    rows = (
        db.execute(
            stmt.order_by(SustainabilityBaseline.code)
            .offset((page - 1) * page_size)
            .limit(page_size)
        )
        .scalars()
    ...
    return paginate(
        [_baseline_response(r) for r in rows], page=page, page_size=page_size, total_items=total
    )
...
def create_baseline(
    ...
    *,
    ...
    existing = db.execute(
        select(SustainabilityBaseline).where(
            SustainabilityBaseline.organization_id == organization_id,
            SustainabilityBaseline.code == payload.code.strip(),
        )
    ).scalar_one_or_none()
    if existing:
    ...
    row = SustainabilityBaseline(
        ...
        baseline_unit=payload.baseline_unit,
    ...
    write_audit_log(
        ...
        actor_user_id=user.id,
        organization_id=organization_id,
        entity_type="sustainability_baseline",
        entity_id=str(row.id),
        request_id=request_id,
        ip_address=ip_address,
        user_agent=user_agent,
...
def get_baseline(
    db: Session, user: User, organization_id: uuid.UUID, baseline_id: uuid.UUID
) -> BaselineResponse:
    require_org_read(db, user, organization_id)
...
def list_targets(
    ...
    *,
    ...
    require_org_read(db, user, organization_id)
    stmt = select(SustainabilityTarget).where(
        SustainabilityTarget.organization_id == organization_id
    )
    count_stmt = (
        select(func.count())
        .select_from(SustainabilityTarget)
        .where(SustainabilityTarget.organization_id == organization_id)
    )
    if status:
    ...
    rows = (
        db.execute(
            stmt.order_by(SustainabilityTarget.code).offset((page - 1) * page_size).limit(page_size)
        )
        .scalars()
        .all()
    )
    return paginate(
        [_target_response(r) for r in rows], page=page, page_size=page_size, total_items=total
    )
    ...
    *,
    ...
    existing = db.execute(
        select(SustainabilityTarget).where(
            SustainabilityTarget.organization_id == organization_id,
            SustainabilityTarget.code == payload.code.strip(),
        )
    ).scalar_one_or_none()
    if existing:
    ...
    db.add(
        SustainabilityTargetRevision(
            target_id=row.id,
            revision=1,
            snapshot_json=_target_snapshot(row),
    ...
    write_audit_log(
        ...
        actor_user_id=user.id,
        organization_id=organization_id,
        entity_type="sustainability_target",
        entity_id=str(row.id),
        request_id=request_id,
        ip_address=ip_address,
        user_agent=user_agent,
...
def get_target(
    db: Session, user: User, organization_id: uuid.UUID, target_id: uuid.UUID
) -> TargetResponse:
    require_org_read(db, user, organization_id)
...
def target_progress(
    ...
    *,
    ...
    return {
        "target": _target_response(row).model_dump(mode="json", by_alias=True),
        "progress": progress,
        "baselineYearHint": baseline_year,
    }
...
def list_initiatives(
    ...
    *,
    ...
    page_size: int,
    status: str | None = None,
) -> Page[InitiativeResponse]:
    require_org_read(db, user, organization_id)
    stmt = select(ReductionInitiative).where(ReductionInitiative.organization_id == organization_id)
    count_stmt = (
        select(func.count())
        .select_from(ReductionInitiative)
        .where(ReductionInitiative.organization_id == organization_id)
    )
    if status:
    ...
    rows = (
        db.execute(
            stmt.order_by(ReductionInitiative.code).offset((page - 1) * page_size).limit(page_size)
        )
        .scalars()
        .all()
    )
    return paginate(
        [_initiative_response(r) for r in rows], page=page, page_size=page_size, total_items=total
    )
    ...
    *,
    ...
    existing = db.execute(
        select(ReductionInitiative).where(
            ReductionInitiative.organization_id == organization_id,
            ReductionInitiative.code == payload.code.strip(),
        )
    ).scalar_one_or_none()
    if existing:
        raise ConflictError("Initiative code already exists.")
    row = ReductionInitiative(
        organization_id=organization_id,
        ...
        expected_reduction_kg_co2e=payload.expected_reduction_kg_co2e,
    ...
    write_audit_log(
        ...
        actor_user_id=user.id,
        organization_id=organization_id,
        entity_type="reduction_initiative",
        entity_id=str(row.id),
        request_id=request_id,
        ip_address=ip_address,
        user_agent=user_agent,
...
def get_initiative(
    db: Session, user: User, organization_id: uuid.UUID, initiative_id: uuid.UUID
) -> InitiativeResponse:
    require_org_read(db, user, organization_id)
    ...
    *,
    ...
    if payload.status not in allowed:
        raise ValidationAppError(
            f"Cannot transition initiative from {row.status} to {payload.status}."
        )
    previous = row.status
    ...
    write_audit_log(
        db,
        action="initiative.transitioned",
        actor_user_id=user.id,
        organization_id=organization_id,
        entity_type="reduction_initiative",
        entity_id=str(row.id),
