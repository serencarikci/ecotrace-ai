...
class IntensityMetricDefinition(Base, UUIDPrimaryKeyMixin, TimestampMixin):
    ...
    __table_args__ = (
        UniqueConstraint("organization_id", "code", name="uq_intensity_metric_org_code"),
        Index("ix_intensity_metric_definitions_organization_id", "organization_id"),
        Index("ix_intensity_metric_definitions_code", "code"),
        Index("ix_intensity_metric_definitions_is_active", "is_active"),
    )
    ...
    denominator_activity_type_id: Mapped[uuid.UUID | None] = mapped_column(
...
class EnvironmentalKpiDefinition(Base, UUIDPrimaryKeyMixin, TimestampMixin):
    ...
    __table_args__ = (
        UniqueConstraint("organization_id", "code", name="uq_env_kpi_org_code"),
        Index("ix_environmental_kpi_definitions_organization_id", "organization_id"),
        Index("ix_environmental_kpi_definitions_code", "code"),
        Index("ix_environmental_kpi_definitions_kpi_type", "kpi_type"),
        Index("ix_environmental_kpi_definitions_is_active", "is_active"),
    )
    organization_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True), ForeignKey("organizations.id", ondelete="CASCADE"), nullable=False
    )
...
class SustainabilityBaseline(Base, UUIDPrimaryKeyMixin, TimestampMixin):
    ...
    __table_args__ = (
        UniqueConstraint("organization_id", "code", name="uq_baseline_org_code"),
        Index("ix_sustainability_baselines_organization_id", "organization_id"),
        Index("ix_sustainability_baselines_baseline_type", "baseline_type"),
        Index("ix_sustainability_baselines_reporting_period_id", "reporting_period_id"),
        Index("ix_sustainability_baselines_inventory_id", "inventory_id"),
    ...
    reporting_period_id: Mapped[uuid.UUID | None] = mapped_column(
        UUID(as_uuid=True), ForeignKey("reporting_periods.id", ondelete="SET NULL"), nullable=True
    )
    inventory_id: Mapped[uuid.UUID | None] = mapped_column(
        UUID(as_uuid=True), ForeignKey("carbon_inventories.id", ondelete="SET NULL"), nullable=True
    )
...
class SustainabilityTarget(Base, UUIDPrimaryKeyMixin, TimestampMixin):
    ...
    __table_args__ = (
        UniqueConstraint("organization_id", "code", name="uq_target_org_code"),
        Index("ix_sustainability_targets_organization_id", "organization_id"),
        Index("ix_sustainability_targets_target_type", "target_type"),
        Index("ix_sustainability_targets_status", "status"),
        Index("ix_sustainability_targets_target_year", "target_year"),
        Index("ix_sustainability_targets_owner_user_id", "owner_user_id"),
        Index("ix_sustainability_targets_facility_id", "facility_id"),
    )
    organization_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True), ForeignKey("organizations.id", ondelete="CASCADE"), nullable=False
    )
    ...
    baseline_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True),
        ForeignKey("sustainability_baselines.id", ondelete="RESTRICT"),
        nullable=False,
    )
...
class SustainabilityTargetRevision(Base, UUIDPrimaryKeyMixin, TimestampMixin):
    ...
    target_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True),
        ForeignKey("sustainability_targets.id", ondelete="CASCADE"),
        nullable=False,
    )
...
class ReductionInitiative(Base, UUIDPrimaryKeyMixin, TimestampMixin):
    ...
    __table_args__ = (
        UniqueConstraint("organization_id", "code", name="uq_initiative_org_code"),
        Index("ix_reduction_initiatives_organization_id", "organization_id"),
        Index("ix_reduction_initiatives_target_id", "target_id"),
        Index("ix_reduction_initiatives_facility_id", "facility_id"),
        Index("ix_reduction_initiatives_status", "status"),
        Index("ix_reduction_initiatives_planned_start_date", "planned_start_date"),
    )
    organization_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True), ForeignKey("organizations.id", ondelete="CASCADE"), nullable=False
    )
    target_id: Mapped[uuid.UUID | None] = mapped_column(
        UUID(as_uuid=True),
        ForeignKey("sustainability_targets.id", ondelete="SET NULL"),
        nullable=True,
    )
    code: Mapped[str] = mapped_column(String(64), nullable=False)
    name: Mapped[str] = mapped_column(String(255), nullable=False)
    ...
    expected_reduction_kg_co2e: Mapped[Decimal] = mapped_column(
