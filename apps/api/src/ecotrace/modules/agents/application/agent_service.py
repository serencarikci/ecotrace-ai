...
from ecotrace.core.intelligence_constants import (
    AGENT_CODES,
...
from ecotrace.modules.production_operations.infrastructure.models import (
    AgentActionRequest,
    AgentDefinition,
    AgentExecution,
    AgentTool,
...
def ensure_catalog(db: Session) -> None:
    ...
    defs = [
        (
            ...
            "Summarize inventories and emission breakdowns.",
            [
                "get_carbon_inventory_summary",
                "compare_carbon_inventories",
                "get_facility_emissions",
        ...
        (
            "data_quality",
            "Data Quality Agent",
            "Inspect data quality issues and propose reviews.",
            ["get_data_quality_issues", "get_activity_records", "create_draft_corrective_action"],
        ),
        (
            "target_monitoring",
            "Target Monitoring Agent",
            "Explain target progress and trajectory risks.",
            ["get_target_progress", "create_draft_target_review"],
        ),
        (
            "report_generation",
            "Report Generation Agent",
            "Draft sustainability report narratives.",
            ["get_scheduled_report_status", "create_draft_report"],
        ),
        (
            "supplier_review",
            "Supplier Review Agent",
            "Review supplier sustainability evidence.",
            ["get_supplier_sustainability_data", "create_draft_recommendation"],
        ),
        (
            "regulatory_document",
            "Regulatory Document Agent",
            "Summarize regulatory documents with citations.",
    ...
    for code, name, desc, tools in defs:
        db.add(
            AgentDefinition(
                ...
                required_permissions_json=["ai_read"],
    ...
    for tool in sorted(READ_ONLY_TOOLS | CONTROLLED_WRITE_TOOLS):
        db.add(
            AgentTool(
                ...
                required_permission="ai_read" if tool in READ_ONLY_TOOLS else "ai_write",
                write_classification="read" if tool in READ_ONLY_TOOLS else "write",
                approval_required=tool in CONTROLLED_WRITE_TOOLS,
                is_enabled=True,
            )
        )
    db.flush()
...
def get_agent(db: Session, agent_code: str) -> dict[str, Any]:
    ensure_catalog(db)
    row = db.execute(
        select(AgentDefinition).where(AgentDefinition.code == agent_code)
    ).scalar_one_or_none()
    if row is None:
    ...
    *,
    ...
    agent = db.execute(
        select(AgentDefinition).where(
            AgentDefinition.code == agent_code, AgentDefinition.is_enabled.is_(True)
        )
    ).scalar_one_or_none()
    if agent is None:
    ...
    execution = AgentExecution(
        ...
        model_provider="local_grounded",
        model_name="ecotrace-agent",
        trace_id=trace_id,
    )
    db.add(execution)
    db.flush()
    write_audit_log(
        ...
        metadata={"agentCode": agent_code, "traceId": trace_id},
    )
        ...
        if tool_code in READ_ONLY_TOOLS:
            ...
            db.add(
                AgentToolCall(
                    ...
                    tool_code=tool_code,
                    input_json=redact_secrets({"prompt": prompt[:500]}),
            ...
            req = AgentActionRequest(
                ...
                title=f"Proposed {tool_code}",
                description=f"Agent {agent_code} proposes controlled write: {tool_code}",
                input_payload_json=redact_secrets({"prompt": prompt[:500]}),
                proposed_changes_json={"action": tool_code, "draft": True, "summary": prompt[:240]},
                risk_level="medium" if tool_code != "create_draft_automation_rule" else "high",
                status="pending",
            ...
            db.add(
                AgentToolCall(
                    ...
                    agent_execution_id=execution.id,
                    tool_code=tool_code,
                    input_json={"queued_for_approval": True},
                    output_json={"actionRequestId": str(req.id)},
    ...
    summary = {
        ...
        "tools": tool_results,
        "pendingApprovals": action_requests,
        "citations": [
            {"label": f"T{i + 1}", "documentName": t["tool"], "databaseSource": "tool"}
            for i, t in enumerate(tool_results)
        ],
        "disclaimer": "Grounded agent summary using allowlisted tools only.",
    }
...
def get_execution(
    db: Session, user: User, organization_id: uuid.UUID, execution_id: uuid.UUID
) -> dict[str, Any]:
    require_ai_read(db, user, organization_id)
    row = db.execute(
    ...
    calls = db.execute(
        select(AgentToolCall).where(AgentToolCall.agent_execution_id == row.id)
    ).scalars()
    payload = _ser_execution(row)
    payload["toolCalls"] = [
        {"toolCode": c.tool_code, "status": c.status, "output": c.output_json} for c in calls
    ]
    return payload
...
def list_action_requests(
    ...
    rows = db.execute(
        ...
        .where(AgentActionRequest.organization_id == organization_id)
        .order_by(AgentActionRequest.created_at.desc())
        .limit(100)
    ).scalars()
...
def get_action_request(
    db: Session, user: User, organization_id: uuid.UUID, request_id: uuid.UUID
) -> dict[str, Any]:
    require_ai_read(db, user, organization_id)
...
def approve_action(
    ...
    *,
    ...
    write_audit_log(
        ...
        actor_user_id=user.id,
        organization_id=organization_id,
        entity_type="agent_action_request",
...
def execute_action(
    ...
    artifact: dict[str, Any] = {
        "type": "draft",
        "payload": row.proposed_changes_json,
    ...
    if row.action_type == "acknowledge_alert":
        ...
        if alert_id:
            alert = db.execute(
                select(Alert).where(
                    Alert.id == uuid.UUID(str(alert_id)), Alert.organization_id == organization_id
                )
            ).scalar_one_or_none()
...
def _run_readonly_tool(
    db: Session, user: User, organization_id: uuid.UUID, tool_code: str, prompt: str
) -> dict[str, Any]:
    mapped = {
        "get_carbon_inventory_summary": "summarize_inventory",
        "compare_carbon_inventories": "compare_inventories",
        "get_facility_emissions": "highest_emitting_facility",
        "get_target_progress": "explain_target_progress",
        "get_product_footprint": "summarize_product_footprint",
        "get_digital_product_passport": "summarize_passport",
        "get_scenario_results": "compare_scenarios",
        "search_organization_documents": "find_related_documents",
    ...
    if tool_code == "get_anomaly_results":
        ...
        count = db.execute(
            ...
            .where(AnomalyEvent.organization_id == organization_id, AnomalyEvent.status == "open")
        ).scalar_one()
        return {"summary": f"Open anomalies: {count}", "openCount": count}
    if tool_code == "get_supplier_sustainability_data":
        from ecotrace.modules.production_operations.infrastructure.models import (
            SupplierMonitoringProfile,
        )

        count = db.execute(
            select(func.count())
            .select_from(SupplierMonitoringProfile)
            .where(SupplierMonitoringProfile.organization_id == organization_id)
        ).scalar_one()
...
def _get_action(
    ...
    row = db.execute(
        select(AgentActionRequest).where(
            AgentActionRequest.id == request_id,
            AgentActionRequest.organization_id == organization_id,
        )
    ).scalar_one_or_none()
    if row is None:
...
def _ser_agent(row: AgentDefinition) -> dict[str, Any]:
    return {
        ...
        "allowedTools": row.allowed_tools_json or [],
        "requiredPermissions": row.required_permissions_json or [],
        "maxExecutionSeconds": row.max_execution_seconds,
        "maxToolCalls": row.max_tool_calls,
        "maxTokenUsage": row.max_token_usage,
...
def _ser_execution(row: AgentExecution) -> dict[str, Any]:
    return {
        ...
        "estimatedCost": float(row.estimated_cost) if row.estimated_cost is not None else 0.0,
    }
...
def _ser_action(row: AgentActionRequest) -> dict[str, Any]:
    return {
        "id": str(row.id),
        "organizationId": str(row.organization_id),
        "agentExecutionId": str(row.agent_execution_id) if row.agent_execution_id else None,
        "actionType": row.action_type,
        "title": row.title,
        "description": row.description,
        "proposedChanges": row.proposed_changes_json,
