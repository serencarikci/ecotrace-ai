...
def _tool(
    code: str, description: str, *, write: bool = False, input_schema: dict[str, Any] | None = None
) -> dict[str, Any]:
    return {
        "name": code,
        ...
        "authorization": "organization_scoped",
...
def mcp_adapter_notes() -> dict[str, Any]:
    return {
        ...
        "adapterGuidance": "An external MCP adapter should authenticate as an EcoTrace service account, pass organization scope on every call, enforce the allowlist below, apply timeouts, and never expose SQL/shell/arbitrary HTTP tools.",
        "tools": build_mcp_catalog(),
    }
