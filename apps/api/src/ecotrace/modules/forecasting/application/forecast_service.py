...
from ecotrace.modules.forecasting.application.methods import (
    accuracy_bundle,
    d,
    run_method,
    select_method,
    target_trajectory_label,
)
...
from ecotrace.shared.application.org_access import (
    require_automation_manage,
    require_automation_read,
    require_automation_write,
)
...
def list_definitions(db: Session, user: User, organization_id: uuid.UUID) -> list[dict[str, Any]]:
    ...
    rows = db.execute(
        select(ForecastDefinition).where(ForecastDefinition.organization_id == organization_id)
    ).scalars()
    return [_ser_def(r) for r in rows]


def create_definition(
    db: Session, user: User, organization_id: uuid.UUID, payload: dict[str, Any]
) -> dict[str, Any]:
    ...
    row = ForecastDefinition(
        organization_id=organization_id,
        code=str(payload["code"]),
        name=str(payload["name"]),
        metric_type=str(payload.get("metricType") or "total_emissions"),
        entity_type=str(payload.get("entityType") or "organization"),
        entity_id=uuid.UUID(payload["entityId"]) if payload.get("entityId") else None,
        method=method,
        historical_period_count=int(payload.get("historicalPeriodCount") or 12),
        forecast_horizon=int(payload.get("forecastHorizon") or 6),
        granularity=str(payload.get("granularity") or "monthly"),
        configuration_json=payload.get("configuration") or {},
        is_active=True,
    )
    db.add(row)
    db.flush()
    write_audit_log(
        db,
        action="forecast.definition.created",
        actor_user_id=user.id,
        organization_id=organization_id,
        entity_type="forecast_definition",
...
def update_definition(
    ...
    for key, attr in {
        ...
        "historicalPeriodCount": "historical_period_count",
        "isActive": "is_active",
...
def run_forecast(
    ...
    *,
    ...
    if backtest and len(series) >= 6:
        ...
        metrics = {
            **accuracy,
            ...
            "selectionStrategy": "Hold out recent periods and minimize MAE among supported deterministic models.",
        }
    ...
    run = ForecastRun(
        ...
        status="completed",
        training_start_date=today - timedelta(days=30 * len(series)),
        training_end_date=today,
        forecast_start_date=today + timedelta(days=30),
        forecast_end_date=today + timedelta(days=30 * horizon),
        model_version=FORECAST_ENGINE_VERSION,
        data_point_count=len(series),
        accuracy_metrics_json=metrics,
        source_data_quality="provisional",
    ...
    for i, value in enumerate(preds):
        ...
        db.add(
            ForecastPoint(
                ...
                lower_bound=d(value - band),
                upper_bound=d(value + band),
            )
        )
    write_audit_log(
        db,
        action="forecast.run.completed",
        actor_user_id=user.id,
        organization_id=organization_id,
...
def list_runs(
    ...
    rows = db.execute(
        select(ForecastRun)
        .where(
            ForecastRun.organization_id == organization_id,
            ForecastRun.forecast_definition_id == definition_id,
        )
...
def get_run(
    db: Session, user: User, organization_id: uuid.UUID, run_id: uuid.UUID
) -> dict[str, Any]:
    require_automation_read(db, user, organization_id)
    row = db.execute(
        select(ForecastRun).where(
            ForecastRun.id == run_id, ForecastRun.organization_id == organization_id
        )
    ).scalar_one_or_none()
    if row is None:
...
def get_points(
    ...
    rows = db.execute(
        ...
        .order_by(ForecastPoint.period_start.asc())
    ).scalars()
    return [
        {
            "id": str(p.id),
            ...
            "predictedValue": float(p.predicted_value),
            "lowerBound": float(p.lower_bound) if p.lower_bound is not None else None,
            "upperBound": float(p.upper_bound) if p.upper_bound is not None else None,
            "actualValue": float(p.actual_value) if p.actual_value is not None else None,
        }
        for p in rows
    ]
...
def target_trajectory(db: Session, user: User, organization_id: uuid.UUID) -> dict[str, Any]:
    ...
    target = db.execute(
        select(SustainabilityTarget)
        .where(
            SustainabilityTarget.organization_id == organization_id,
            SustainabilityTarget.status.in_(["active", "approved", "draft"]),
        )
        .order_by(SustainabilityTarget.updated_at.desc())
        .limit(1)
    ).scalar_one_or_none()
    ...
    if target is None or len(series) < 3:
        return {
            ...
            "disclaimer": "Model-based projection only; not a guarantee.",
        }
    preds = run_method("linear_trend", series, 12)
    current = series[0]
    target_value = float(
        getattr(target, "target_value", None)
        or getattr(target, "absolute_target_value", None)
        or current * 0.7
    )
    label = target_trajectory_label(
        current=current, target=target_value, forecast_at_target=preds[-1], periods_remaining=12
    )
    ...
    return {
        ...
        "forecastAtTargetDate": preds[-1],
        "expectedGap": gap,
        "requiredMonthlyImprovement": monthly,
        "label": label,
        "disclaimer": "Model-based projection only; not a statistical probability or guarantee.",
    }
...
def _load_series(db: Session, organization_id: uuid.UUID, metric_type: str) -> list[float]:
    ...
    runs = list(
        db.execute(
            ...
            .join(CarbonInventory, CarbonCalculationRun.inventory_id == CarbonInventory.id)
            .where(CarbonInventory.organization_id == organization_id)
            .order_by(CarbonCalculationRun.created_at.asc())
            .limit(36)
        ).scalars()
    )
    chrono: list[float] = []
...
def _get_def(
    ...
    row = db.execute(
        select(ForecastDefinition).where(
            ForecastDefinition.id == definition_id,
            ForecastDefinition.organization_id == organization_id,
        )
    ).scalar_one_or_none()
    if row is None:
...
def _ser_def(row: ForecastDefinition) -> dict[str, Any]:
    return {
        ...
        "method": row.method,
        "historicalPeriodCount": row.historical_period_count,
        "forecastHorizon": row.forecast_horizon,
        "granularity": row.granularity,
        "isActive": row.is_active,
        "disclaimer": "Forecasts are estimates only.",
    }
...
def _ser_run(row: ForecastRun) -> dict[str, Any]:
    return {
        "id": str(row.id),
        "forecastDefinitionId": str(row.forecast_definition_id),
        "status": row.status,
        "modelVersion": row.model_version,
        "dataPointCount": row.data_point_count,
        "accuracyMetrics": row.accuracy_metrics_json,
        "sourceDataQuality": row.source_data_quality,
        "generatedAt": row.generated_at.isoformat() if row.generated_at else None,
        "disclaimer": row.disclaimer,
        "forecastStartDate": row.forecast_start_date.isoformat()
        if row.forecast_start_date
        else None,
        "forecastEndDate": row.forecast_end_date.isoformat() if row.forecast_end_date else None,
    }
