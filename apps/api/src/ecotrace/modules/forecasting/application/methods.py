...
def holt_linear(
    values: list[float], horizon: int, alpha: float = 0.3, beta: float = 0.1
) -> list[float]:
    if len(values) < 2:
...
def select_method(values: list[float], horizon: int = 3) -> str:
    ...
    actual = values[-hold:]
    candidates = {
        "linear_trend": linear_trend(train, hold),
        "moving_average": moving_average(train, hold),
        "weighted_moving_average": weighted_moving_average(train, hold),
        "seasonal_naive": seasonal_naive(train, hold, season=min(12, len(train))),
        "simple_exponential_smoothing": simple_exponential_smoothing(train, hold),
        "holt_linear": holt_linear(train, hold),
    }
...
def target_trajectory_label(
    *, current: float, target: float, forecast_at_target: float | None, periods_remaining: int
) -> str:
