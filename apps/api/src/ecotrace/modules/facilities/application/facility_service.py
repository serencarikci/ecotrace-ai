    ...
    *,
    ...
    if facility_type not in FACILITY_TYPES:
        raise ValidationAppError(
            ...
            details=[{"field": "facilityType", "message": "Unknown facility type code."}],
        )
    code = country_code.strip().upper()
    if len(code) != 2 or not code.isalpha():
        raise ValidationAppError(
            "Invalid country code.",
            details=[{"field": "countryCode", "message": "Must be ISO 3166-1 alpha-2."}],
        )
    if not TZ_RE.match(timezone.strip()):
        raise ValidationAppError(
            "Invalid timezone.",
            details=[{"field": "timezone", "message": "Use an IANA-like timezone id."}],
        )
...
def list_facilities(
    ...
    *,
    ...
    rows = list(
        db.execute(
            stmt.order_by(Facility.name.asc()).offset((page - 1) * page_size).limit(page_size)
        )
        .scalars()
        .all()
    )
...
def create_facility(
    ...
    *,
    ...
    _validate_facility_fields(
        ...
        start=payload.operational_start_date,
        end=payload.operational_end_date,
    )
    ...
    exists = db.execute(
        select(Facility.id).where(
            Facility.organization_id == organization_id, Facility.code == code
        )
    ).scalar_one_or_none()
    if exists:
    ...
    facility = Facility(
        ...
        operational_start_date=payload.operational_start_date,
        operational_end_date=payload.operational_end_date,
...
def get_facility_detail(
    db: Session, user: User, organization_id: uuid.UUID, facility_id: uuid.UUID
) -> FacilityResponse:
    ensure_org_access(db, user, organization_id)
