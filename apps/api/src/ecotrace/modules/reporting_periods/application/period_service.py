...
def _ensure_no_same_type_overlap(
    db: Session,
    organization_id: uuid.UUID,
    period_type: str,
    start: date,
    ...
    stmt = select(ReportingPeriod).where(
        ...
        and_(ReportingPeriod.start_date <= end, ReportingPeriod.end_date >= start),
    )
    if exclude_id:
...
def list_periods(
    ...
    *,
    ...
    rows = list(
        db.execute(
            stmt.order_by(ReportingPeriod.start_date.desc())
    ...
    for row in rows:
        count = db.execute(
            ...
            .where(ActivityRecord.reporting_period_id == row.id)
        ).scalar_one()
    ...
    *,
    ...
    if db.execute(
        select(ReportingPeriod.id).where(
            ReportingPeriod.organization_id == organization_id, ReportingPeriod.code == code
        )
    ).scalar_one_or_none():
        raise ConflictError("Reporting period code already exists.")
    _ensure_no_same_type_overlap(
        db, organization_id, payload.period_type, payload.start_date, payload.end_date
    )
...
def update_period(
    ...
    *,
    ...
    if end < start:
        raise ValidationAppError("endDate must be on or after startDate.")
    _ensure_no_same_type_overlap(
        db, organization_id, period.period_type, start, end, exclude_id=period.id
    )
...
def archive_period(
    ...
    *,
    ...
    write_audit_log(
        db,
        action="reporting_period.archived",
        actor_user_id=user.id,
        organization_id=organization_id,
        entity_type="reporting_period",
        entity_id=str(period.id),
