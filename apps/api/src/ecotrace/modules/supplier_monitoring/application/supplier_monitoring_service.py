...
def list_profiles(db: Session, user: User, organization_id: uuid.UUID) -> list[dict[str, Any]]:
    ...
    rows = db.execute(
        select(SupplierMonitoringProfile).where(
            SupplierMonitoringProfile.organization_id == organization_id
        )
    ).scalars()
...
def get_profile(
    db: Session, user: User, organization_id: uuid.UUID, supplier_id: uuid.UUID
) -> dict[str, Any]:
    require_automation_read(db, user, organization_id)
...
def update_profile(
    ...
    for key, attr in {
        ...
        "requiredDocumentTypes": "required_document_types_json",
...
def assess(
    ...
    assessment = SupplierAssessment(
        ...
        risk_level=risk,
        findings_json={"overdueReview": overdue, "note": "Internal non-certified assessment."},
        recommendations_json={"actions": ["Request latest sustainability questionnaire"]},
    ...
    write_audit_log(
        db,
        action="supplier.assessment.completed",
    ...
    if overdue:
        ...
        alert_service.create_alert(
            ...
            title="Supplier review overdue",
            message="Supplier monitoring review is overdue.",
            severity="medium",
        )
    db.flush()
...
def list_assessments(
    ...
    rows = db.execute(
        ...
        .where(
            SupplierAssessment.organization_id == organization_id,
            SupplierAssessment.supplier_id == supplier_id,
        )
        .order_by(SupplierAssessment.assessment_date.desc())
    ).scalars()
...
def _get_or_create(
    ...
    row = db.execute(
        select(SupplierMonitoringProfile).where(
            SupplierMonitoringProfile.organization_id == organization_id,
            SupplierMonitoringProfile.supplier_id == supplier_id,
        )
    ).scalar_one_or_none()
    if row:
        return row
    row = SupplierMonitoringProfile(
        organization_id=organization_id,
        supplier_id=supplier_id,
        monitoring_status="active",
        risk_level="medium",
        required_document_types_json=["certificate", "policy"],
...
def _ser_profile(row: SupplierMonitoringProfile) -> dict[str, Any]:
    return {
        ...
        "requiredDocumentTypes": row.required_document_types_json or [],
        ...
        "disclaimer": "Internal non-certified monitoring scores.",
        "overdue": bool(row.next_review_at and row.next_review_at < datetime.now(UTC)),
    }
...
def _ser_assessment(row: SupplierAssessment) -> dict[str, Any]:
    return {
        ...
        "emissionsScore": float(row.emissions_score) if row.emissions_score is not None else None,
        "dataQualityScore": float(row.data_quality_score)
        if row.data_quality_score is not None
        else None,
        "documentCompletenessScore": float(row.document_completeness_score)
        if row.document_completeness_score is not None
        else None,
        "sustainabilityScore": float(row.sustainability_score)
