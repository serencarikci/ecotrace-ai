...
from ecotrace.modules.activity_data.application.attachment_service import (
    MIME_BY_EXT,
...
def get_passport(
    db: Session, organization_id: uuid.UUID, passport_id: uuid.UUID
) -> DigitalProductPassport:
...
def _sections(db: Session, passport_id: uuid.UUID) -> list[DigitalProductPassportSection]:
    return list(
        db.execute(
            select(DigitalProductPassportSection)
            .where(DigitalProductPassportSection.passport_id == passport_id)
            .order_by(DigitalProductPassportSection.display_order.asc())
        )
        .scalars()
        .all()
    )
...
def list_passports(
    ...
    *,
    ...
    stmt = select(DigitalProductPassport).where(
        DigitalProductPassport.organization_id == organization_id
    )
    if search:
        like = f"%{search.strip()}%"
        stmt = stmt.where(
            or_(
                DigitalProductPassport.title.ilike(like),
                DigitalProductPassport.passport_code.ilike(like),
                DigitalProductPassport.public_slug.ilike(like),
            )
        )
    if status:
    ...
    rows = list(
        db.execute(
            stmt.order_by(DigitalProductPassport.version.desc())
    ...
    return paginate(
        [_to_response(db, r) for r in rows], page=page, page_size=page_size, total_items=int(total)
    )
...
def create_passport(
    ...
    *,
    ...
    if db.execute(
        select(DigitalProductPassport.id).where(DigitalProductPassport.passport_code == code)
    ).scalar_one_or_none():
        raise ConflictError("Passport code must be unique.")
    if db.execute(
        select(DigitalProductPassport.id).where(DigitalProductPassport.public_slug == slug)
    ).scalar_one_or_none():
    ...
    row = DigitalProductPassport(
        ...
        product_carbon_footprint_id=payload.product_carbon_footprint_id,
        passport_code=code,
        ...
        public_slug=slug,
        qr_code_reference=_public_url(slug),
    ...
    for section in payload.sections:
        db.add(
            DigitalProductPassportSection(
                ...
                structured_data_json=section.structured_data_json,
...
def clone_passport_version(
    ...
    *,
    ...
    max_version = db.execute(
        select(func.coalesce(func.max(DigitalProductPassport.version), 0)).where(
            DigitalProductPassport.organization_id == organization_id,
            DigitalProductPassport.product_id == source.product_id,
            DigitalProductPassport.public_slug == source.public_slug,
        )
    ).scalar_one()
    new_code = f"{source.passport_code}-v{max_version + 1}"
    while db.execute(
        select(DigitalProductPassport.id).where(DigitalProductPassport.passport_code == new_code)
    ).scalar_one_or_none():
    ...
    row = DigitalProductPassport(
        ...
        product_carbon_footprint_id=source.product_carbon_footprint_id,
        passport_code=new_code,
        title=source.title,
        description=source.description,
        version=max_version + 1,
        status="draft",
        language_code=source.language_code,
        public_slug=draft_slug,
        qr_code_reference=_public_url(source.public_slug),
    ...
    write_audit_log(
        ...
        ip_address=ip_address,
        user_agent=user_agent,
        metadata={"sourceId": str(source.id), "stableSlug": source.public_slug},
    )
    db.commit()
    db.refresh(row)
    return _to_response(db, row)
...
def publish_passport(
    ...
    *,
    ...
    holders = list(
        db.execute(
            select(DigitalProductPassport).where(
                DigitalProductPassport.public_slug == stable_slug,
                DigitalProductPassport.id != row.id,
            )
        )
        .scalars()
        .all()
    ...
    write_audit_log(
        ...
        ip_address=ip_address,
        user_agent=user_agent,
        metadata={"publicSlug": stable_slug},
    )
    db.commit()
    db.refresh(row)
    return _to_response(db, row)
...
def get_passport_detail(
    db: Session, user: User, organization_id: uuid.UUID, passport_id: uuid.UUID
) -> PassportResponse:
    ensure_org_access(db, user, organization_id)
...
def upload_document(
    ...
    *,
    ...
    if (
        expected
        and resolved
        and (resolved not in expected)
        and (resolved != "application/octet-stream")
    ):
    ...
    doc = PassportDocument(
        ...
        content_type=resolved or next(iter(expected), "application/octet-stream"),
    ...
    write_audit_log(
        ...
        actor_user_id=user.id,
        organization_id=organization_id,
        entity_type="passport_document",
        entity_id=str(row.id),
        request_id=request_id,
        ip_address=ip_address,
        user_agent=user_agent,
        metadata={"documentType": document_type, "isPublic": is_public},
    )
    db.commit()
    db.refresh(doc)
...
def _active_public_passport(db: Session, public_slug: str) -> DigitalProductPassport | None:
    published = db.execute(
        ...
        .where(
            DigitalProductPassport.public_slug == public_slug,
            DigitalProductPassport.status == "published",
        )
        .order_by(DigitalProductPassport.version.desc())
    ).scalar_one_or_none()
    if published:
        return published
    revoked = db.execute(
        ...
        .where(
            DigitalProductPassport.public_slug == public_slug,
            DigitalProductPassport.status == "revoked",
        )
        .order_by(DigitalProductPassport.version.desc())
    ).scalar_one_or_none()
    return revoked
...
def get_public_passport(db: Session, public_slug: str) -> dict[str, Any]:
    ...
    if row is None:
        any_row = db.execute(
            select(DigitalProductPassport)
            .where(DigitalProductPassport.public_slug == public_slug.strip().lower())
            .order_by(DigitalProductPassport.version.desc())
        ).scalar_one_or_none()
        if any_row is None:
    ...
    sections = [
        {
            ...
            "structuredData": s.structured_data_json,
            "displayOrder": s.display_order,
        }
        for s in _sections(db, row.id)
        if s.is_public
    ]
    footprint = None
    if row.product_carbon_footprint_id:
        ...
        if fp and fp.status == "approved":
            footprint = {
                "totalKgCO2e": str(fp.total_kg_co2e),
                "cradleToGateKgCO2e": str(fp.cradle_to_gate_kg_co2e)
                if fp.cradle_to_gate_kg_co2e is not None
                else None,
                "usePhaseKgCO2e": str(fp.use_phase_kg_co2e)
                if fp.use_phase_kg_co2e is not None
                else None,
                "endOfLifeKgCO2e": str(fp.end_of_life_kg_co2e)
                if fp.end_of_life_kg_co2e is not None
                else None,
                "functionalUnitQuantity": str(fp.functional_unit_quantity),
                "functionalUnitCode": fp.functional_unit_code,
            }
    write_audit_log(
        db,
        action="public_passport.accessed",
        organization_id=row.organization_id,
        entity_type="digital_product_passport",
        entity_id=str(row.id),
        metadata={"publicSlug": public_slug},
    )
    db.commit()
    return {
        "status": row.status,
        ...
        "product": {
            ...
            "recyclabilityPercentage": str(product.recyclability_percentage)
            if product.recyclability_percentage is not None
            else None,
            "recycledContentPercentage": str(product.recycled_content_percentage)
            if product.recycled_content_percentage is not None
            else None,
            "repairabilityScore": product.repairability_score,
        },
        "manufacturer": {
            "organizationName": org.name if org else None,
            "organizationSlug": org.slug if org else None,
        },
        "sections": sections,
...
def list_public_documents(db: Session, public_slug: str) -> list[dict[str, Any]]:
    ...
    docs = list(
        db.execute(
            select(PassportDocument).where(
                PassportDocument.passport_id == row.id, PassportDocument.is_public.is_(True)
            )
        )
        .scalars()
        .all()
...
def get_public_document_bytes(
    db: Session, public_slug: str, document_id: uuid.UUID
) -> tuple[PassportDocument, bytes]:
    row = _active_public_passport(db, public_slug.strip().lower())
    if row is None or row.status not in {"published", "revoked"}:
        raise NotFoundError("Passport not found.")
