...
class DigitalProductPassport(Base, UUIDPrimaryKeyMixin, TimestampMixin):
    ...
    __table_args__ = (
        UniqueConstraint("passport_code", name="uq_dpp_passport_code"),
        UniqueConstraint("public_slug", name="uq_dpp_public_slug"),
        Index("ix_dpp_organization_id", "organization_id"),
        Index("ix_dpp_product_id", "product_id"),
        Index("ix_dpp_product_batch_id", "product_batch_id"),
        Index("ix_dpp_public_slug", "public_slug"),
        Index("ix_dpp_passport_code", "passport_code"),
        Index("ix_dpp_status", "status"),
        Index("ix_dpp_published_at", "published_at"),
    )
    ...
    product_carbon_footprint_id: Mapped[uuid.UUID | None] = mapped_column(
        UUID(as_uuid=True),
        ForeignKey("product_carbon_footprints.id", ondelete="SET NULL"),
        nullable=True,
    )
    ...
    supersedes_passport_id: Mapped[uuid.UUID | None] = mapped_column(
        UUID(as_uuid=True),
        ForeignKey("digital_product_passports.id", ondelete="SET NULL"),
        nullable=True,
    )
...
class DigitalProductPassportSection(Base, UUIDPrimaryKeyMixin, TimestampMixin):
    ...
    __table_args__ = (
        UniqueConstraint("passport_id", "section_code", name="uq_dpp_section_code"),
        Index("ix_dpp_sections_passport_id", "passport_id"),
    )
    passport_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True),
        ForeignKey("digital_product_passports.id", ondelete="CASCADE"),
        nullable=False,
    )
...
class PassportDocument(Base, UUIDPrimaryKeyMixin, TimestampMixin):
    ...
    __table_args__ = (
        Index("ix_passport_documents_organization_id", "organization_id"),
        Index("ix_passport_documents_passport_id", "passport_id"),
    )
    organization_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True), ForeignKey("organizations.id", ondelete="CASCADE"), nullable=False
    )
