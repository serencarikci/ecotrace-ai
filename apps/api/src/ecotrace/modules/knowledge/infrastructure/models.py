...
class KnowledgeDocument(Base, UUIDPrimaryKeyMixin, TimestampMixin):
    ...
    __table_args__ = (
        UniqueConstraint("organization_id", "slug", name="uq_knowledge_documents_org_slug"),
        ...
        Index("ix_knowledge_documents_language", "language_code"),
    )
...
class KnowledgeDocumentVersion(Base, UUIDPrimaryKeyMixin, TimestampMixin):
    ...
    __table_args__ = (
        ...
        Index("ix_knowledge_document_versions_status", "status"),
    )
...
class KnowledgeChunk(Base, UUIDPrimaryKeyMixin, TimestampMixin):
    ...
    __table_args__ = (
        ...
        Index("ix_knowledge_chunks_document_version_id", "document_version_id"),
        Index("ix_knowledge_chunks_language", "language_code"),
...
class KnowledgeEmbedding(Base, UUIDPrimaryKeyMixin, TimestampMixin):
    ...
    __table_args__ = (
        UniqueConstraint("chunk_id", "model_name", name="uq_knowledge_embeddings_chunk_model"),
        Index("ix_knowledge_embeddings_organization_id", "organization_id"),
        Index("ix_knowledge_embeddings_chunk_id", "chunk_id"),
        Index("ix_knowledge_embeddings_model_name", "model_name"),
    )
    organization_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True), ForeignKey("organizations.id", ondelete="CASCADE"), nullable=False
    )
...
class ChatFolder(Base, UUIDPrimaryKeyMixin, TimestampMixin):
    ...
    __table_args__ = (
        ...
        Index("ix_chat_folders_organization_id", "organization_id"),
        Index("ix_chat_folders_owner_user_id", "owner_user_id"),
    )
    organization_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True), ForeignKey("organizations.id", ondelete="CASCADE"), nullable=False
    )
...
class ChatConversation(Base, UUIDPrimaryKeyMixin, TimestampMixin):
    ...
    __table_args__ = (
        ...
        Index("ix_chat_conversations_pinned", "is_pinned"),
    )
    organization_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True), ForeignKey("organizations.id", ondelete="CASCADE"), nullable=False
    )
...
class ProviderConfig(Base, UUIDPrimaryKeyMixin, TimestampMixin):
    ...
    __table_args__ = (
        ...
        Index("ix_provider_configs_organization_id", "organization_id"),
    )
    organization_id: Mapped[uuid.UUID | None] = mapped_column(
        UUID(as_uuid=True), ForeignKey("organizations.id", ondelete="CASCADE"), nullable=True
    )
...
class OrganizationAiSettings(Base, UUIDPrimaryKeyMixin, TimestampMixin):
    ...
    __table_args__ = (
        UniqueConstraint("organization_id", name="uq_organization_ai_settings_org"),
        Index("ix_organization_ai_settings_organization_id", "organization_id"),
    )
    organization_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True), ForeignKey("organizations.id", ondelete="CASCADE"), nullable=False
    )
    preferred_provider: Mapped[str] = mapped_column(
        String(64), nullable=False, default="local_grounded"
    )
    preferred_embedding_model: Mapped[str] = mapped_column(
        String(128), nullable=False, default="local-hash-384"
    )
...
class RetrievalLog(Base, UUIDPrimaryKeyMixin, TimestampMixin):
    ...
    __table_args__ = (
        Index("ix_retrieval_logs_organization_id", "organization_id"),
...
class ConversationFeedback(Base, UUIDPrimaryKeyMixin, TimestampMixin):
    ...
    __table_args__ = (
        ...
        Index("ix_conversation_feedback_organization_id", "organization_id"),
    )
    organization_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True), ForeignKey("organizations.id", ondelete="CASCADE"), nullable=False
    )
...
class AiMemory(Base, UUIDPrimaryKeyMixin, TimestampMixin):
    ...
    __table_args__ = (
        Index("ix_ai_memories_organization_id", "organization_id"),
    ...
    conversation_id: Mapped[uuid.UUID | None] = mapped_column(
        UUID(as_uuid=True), ForeignKey("chat_conversations.id", ondelete="CASCADE"), nullable=True
    )
