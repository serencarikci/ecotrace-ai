...
def extract_text_from_bytes(
    *, content: bytes, file_name: str, content_type: str, ocr_engine: Any | None = None
    ...
    if name.endswith(".zip") or content_type == "application/zip":
        ...
        if ocr_engine is not None:
            text = ocr_engine.extract_text(
                content=content, content_type=content_type, file_name=file_name
            )
    ...
    with zipfile.ZipFile(io.BytesIO(content)) as zf:
        for info in zf.infolist()[:50]:
            if info.is_dir():
                continue
            data = zf.read(info)
            text, _ = extract_text_from_bytes(
                content=data,
