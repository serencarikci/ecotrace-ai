...
def chunk_text(
    text: str, *, strategy: str = "markdown", chunk_size: int = 800, overlap: int = 120
) -> list[TextChunk]:
...
def _chunk_fixed(text: str, *, chunk_size: int, overlap: int) -> list[TextChunk]:
    ...
    while start < len(text):
        ...
        if piece:
            chunks.append(
                TextChunk(content=piece, chunk_index=index, token_estimate=estimate_tokens(piece))
            )
            index += 1
        if end >= len(text):
            break
...
def _chunk_semantic(text: str, *, chunk_size: int, overlap: int) -> list[TextChunk]:
    ...
    for sentence in sentences:
        ...
        if size + len(sentence) > chunk_size and buf:
            ...
            chunks.append(
                TextChunk(
                    content=content, chunk_index=index, token_estimate=estimate_tokens(content)
                )
            )
            index += 1
