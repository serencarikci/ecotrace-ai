...
from ecotrace.modules.knowledge.application.ingestion import (
...
def list_documents(
    ...
    *,
    ...
    if q:
        like = f"%{q}%"
        stmt = stmt.where(
            or_(KnowledgeDocument.title.ilike(like), KnowledgeDocument.slug.ilike(like))
        )
...
def get_document(
    db: Session, user: User, organization_id: uuid.UUID, document_id: uuid.UUID
) -> dict[str, Any]:
    require_ai_read(db, user, organization_id)
...
def upload_document(
    ...
    *,
    ...
    text, meta = extract_text_from_bytes(
        content=content, file_name=file_name, content_type=content_type, ocr_engine=ocr
    )
    ...
    while db.execute(
        select(KnowledgeDocument.id).where(
            KnowledgeDocument.organization_id == organization_id, KnowledgeDocument.slug == slug
        )
    ).scalar_one_or_none():
        slug = f"{base_slug}-{i}"
        i += 1
    ...
    doc = KnowledgeDocument(
        ...
        slug=slug,
        document_type=document_type or meta.get("document_type") or "other",
        status="processing",
        language_code=str(meta.get("language") or "en"),
        tags=tags or [],
        permissions_json={"roles": ["organization_member"], "visibility": "organization"},
        metadata_json={"ingestion": meta},
    ...
    write_audit_log(
        ...
        organization_id=organization_id,
        metadata={"title": doc.title, "slug": doc.slug},
    )
    db.flush()
    return _serialize_document(doc, include_versions=True)
...
def index_document_version(
    ...
    *,
    ...
    pieces = chunk_text(
        text,
        strategy=settings.ai_chunking_strategy,
    ...
    if publish:
        existing = db.execute(
            select(KnowledgeChunk).where(
                KnowledgeChunk.document_id == document.id, KnowledgeChunk.is_active.is_(True)
            )
        ).scalars()
        for row in existing:
            row.is_active = False
    ...
    for piece in pieces:
        chunk = KnowledgeChunk(
            ...
            tags=document.tags,
            permissions_json=document.permissions_json,
            metadata_json={
        ...
        db.add(
            KnowledgeEmbedding(
                ...
                model_name=getattr(settings, "ai_embedding_model", embedder.name),
...
def list_chunks(
    ...
    *,
    ...
    stmt = select(KnowledgeChunk).where(
        KnowledgeChunk.organization_id == organization_id, KnowledgeChunk.is_active.is_(True)
    )
    if document_id:
...
def _serialize_document(
    doc: KnowledgeDocument, *, include_versions: bool = False
) -> dict[str, Any]:
