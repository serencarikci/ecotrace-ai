...
def _pick_candidates(
    ...
    for precedence, status, reason_ok, reason_ambiguous in (
        ...
        (
            "DEFAULT_REFERENCE",
            "RESOLVED_DEFAULT",
            "Default reference selected because no compatible active primary value was available.",
    ...
    return (
        "UNRESOLVED",
        None,
        "Resolution failed because no compatible active primary or default value was available.",
    )
    ...
    *,
    ...
    for row in rows:
        if row.organization_id not in (None, organization_id):
            continue
        precedence = "ORG_PRIMARY" if row.data_source_type == "PRIMARY" else "DEFAULT_REFERENCE"
