...
class MappingManifest:
    ...
    def formula_entries(self) -> list[ManifestEntry]:
        return [
            e for e in self.entries if e.direction in {"FORMULA", "OUTPUT"} and e.expected_formula
        ]
...
def load_manifest() -> MappingManifest:
    ...
    for patch in (steel_patch_path(), process_f_patch_path()):
        ...
        raw_entries = _merge_patch_entries(raw_entries, list(patch_payload.get("entries") or []))
