...
def _run_response(
    ...
    if include_total:
        ...
        if len(units) == 1:
            ...
            same_total = sum(
                (r.result_value for r in results if r.result_value is not None), Decimal("0")
            )
    ...
    if generic is None:
        ef = db.execute(
            select(CbamFactorDefinition).where(
                CbamFactorDefinition.code == "GENERIC_EMISSION_FACTOR"
            )
        ).scalar_one()
        embedded = db.execute(
            select(CbamFactorDefinition).where(
...
def list_calculation_definitions(
    ...
    *,
    ...
    stmt = select(CbamCalculationDefinition).where(CbamCalculationDefinition.status != "ARCHIVED")
...
def execute_calculation_run(
    ...
    def_stmt = select(CbamCalculationDefinition).where(CbamCalculationDefinition.status == "ACTIVE")
...
def recalculate_result(
    ...
    if resolution is None or not resolution.is_current:
        resolution = db.execute(
            select(CbamFactorResolution).where(
                CbamFactorResolution.organization_id == organization_id,
                CbamFactorResolution.reporting_period_binding_id == run.reporting_period_binding_id,
