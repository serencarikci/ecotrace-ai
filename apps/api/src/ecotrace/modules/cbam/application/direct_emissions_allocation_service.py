...
def _active_fuels_by_code(db: Session) -> dict[str, CbamStationaryCombustionFuel]:
    rows = db.execute(
        select(CbamStationaryCombustionFuel).where(CbamStationaryCombustionFuel.status == "ACTIVE")
    ).scalars()
...
def _resolve_context(
    ...
    sc_summary = get_stationary_combustion_period_summary(db, user, organization_id, binding_id)
    ...
    profile_ids = {
        p.product_profile_version_id for p in prod_rows if p.product_profile_version_id is not None
    }
    ...
    for prod in prod_rows:
        ...
        if profile is None:
            profile = (
                profiles.get(prod.product_profile_version_id)
                if prod.product_profile_version_id
                else None
            )
        if profile is None:
            blocking.append("PRODUCTION_PROFILE_LINK_MISSING")
...
def assert_monthly_basis_not_referenced(
    ...
    exists = db.execute(
        select(CbamDeaMonthlyBasisSnapshot.id)
        .where(
...
def assert_production_record_not_referenced(
    ...
    exists = db.execute(
        select(CbamDeaProductAllocation.id)
        .where(
            CbamDeaProductAllocation.organization_id == organization_id,
            CbamDeaProductAllocation.production_record_ids.contains([str(production_record_id)]),
        )
        .limit(1)
    ).scalar_one_or_none()
    if exists is not None:
