...
class CbamProductProfileVersion(Base, UUIDPrimaryKeyMixin, TimestampMixin):
    ...
    steel_mill_identification_number: Mapped[str | None] = mapped_column(
...
class CbamMonthlyProductionBasis(Base, UUIDPrimaryKeyMixin, TimestampMixin):
    ...
    total_production_quantity: Mapped[Decimal | None] = mapped_column(
...
class CbamDeaMonthlyBasisSnapshot(Base, UUIDPrimaryKeyMixin, TimestampMixin):
    ...
    reporting_period_binding_id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), nullable=False)
    ...
    quantity_unit: Mapped[str] = mapped_column(String(32), nullable=False)
    normalized_total_production_tonnes: Mapped[Decimal] = mapped_column(Numeric(36, 18), nullable=False)
    normalized_cbam_quantity_tonnes: Mapped[Decimal] = mapped_column(Numeric(36, 18), nullable=False)
...
class CbamDeaProductAllocation(Base, UUIDPrimaryKeyMixin, TimestampMixin):
    ...
    raw_allocated_fossil_co2_tonnes: Mapped[Decimal] = mapped_column(Numeric(36, 18), nullable=False)
    final_allocated_fossil_co2_tonnes: Mapped[Decimal] = mapped_column(Numeric(24, 8), nullable=False)
...
class CbamIeaProductAllocation(Base, UUIDPrimaryKeyMixin, TimestampMixin):
    ...
    electricity_rounding_adjustment: Mapped[Decimal] = mapped_column(Numeric(36, 18), nullable=False)
...
class CbamPurchasedPrecursor(Base, UUIDPrimaryKeyMixin, TimestampMixin):
    ...
    __table_args__ = (
        ...
        CheckConstraint(
            'quantity IS NULL OR quantity >= 0', name='ck_cbam_purch_prec_qty_nonneg'
        ),
        CheckConstraint(
            'non_cbam_quantity IS NULL OR non_cbam_quantity >= 0',
    ...
    electricity_intensity_source_code: Mapped[str | None] = mapped_column(
...
class CbamProductEmbeddedEmissionsProduct(Base, UUIDPrimaryKeyMixin, TimestampMixin):
    ...
    __table_args__ = (
        ...
        CheckConstraint(
            'denominator_tonnes > 0', name='ck_cbam_pee_products_denominator_positive'
        ),
    ...
    has_exported_electricity: Mapped[bool | None] = mapped_column(Boolean, nullable=True)
    exported_electricity_mwh: Mapped[Decimal | None] = mapped_column(
        Numeric(36, 18), nullable=True
    )
    exported_electricity_emission_factor: Mapped[Decimal | None] = mapped_column(
        Numeric(36, 18), nullable=True
...
class CbamProductEmbeddedEmissionsPrecursorContribution(
    ...
    contribution_direct_tco2e_raw: Mapped[Decimal] = mapped_column(
        Numeric(36, 18), nullable=False
    )
...
class CbamProductEmbeddedEmissionsInternalContribution(
    Base, UUIDPrimaryKeyMixin, TimestampMixin
):
    ...
    __table_args__ = (
        UniqueConstraint(
            'result_id', 'product_use_id', name='uq_cbam_pee_internal_result_use'
        ),
    ...
    product_use_unit: Mapped[str] = mapped_column(String(32), nullable=False)
    quantity_tonnes: Mapped[Decimal] = mapped_column(Numeric(36, 18), nullable=False)
    consumer_denominator_tonnes: Mapped[Decimal] = mapped_column(
        Numeric(36, 18), nullable=False
    )
