...
def scan(db: Session, user: User, organization_id: uuid.UUID) -> dict[str, Any]:
    ...
    if period_count == 0:
        checks.append(
            ("missing_reporting_periods", "organization", "Missing reporting periods", "medium")
        )
    failed_imports = db.execute(
        select(func.count())
        .select_from(ImportJob)
        .where(
            ImportJob.organization_id == organization_id, ImportJob.status.in_(["failed", "error"])
        )
    ).scalar_one()
    if failed_imports:
        checks.append(
            ("failed_imports", "data_import", f"{failed_imports} failed import job(s)", "high")
        )
    ...
    backlog = db.execute(
        ...
        .where(
            ...
            ActivityRecord.status.in_(["draft", "submitted"]),
        )
    ).scalar_one()
    if backlog > 20:
        checks.append(
            (
                "unapproved_activity_backlog",
                "activity_record",
                f"Unapproved activity backlog: {backlog}",
                "medium",
            )
        )
    for issue_type, entity_type, title, severity in checks:
        ...
        exists = db.execute(
            select(DataQualityIssue.id).where(
                ...
                DataQualityIssue.fingerprint == fp,
            )
        ).scalar_one_or_none()
        if exists:
            continue
        db.add(
            DataQualityIssue(
                ...
                title=title,
                description=title,
                evidence_json={"detectedBy": "intelligence-data-quality-scan"},
                status="open",
    ...
    write_audit_log(
        db,
        action="data_quality.scan.completed",
...
def get_issue(
    db: Session, user: User, organization_id: uuid.UUID, issue_id: uuid.UUID
) -> dict[str, Any]:
    require_automation_read(db, user, organization_id)
...
def assign(
    db: Session, user: User, organization_id: uuid.UUID, issue_id: uuid.UUID, assignee_id: uuid.UUID
) -> dict[str, Any]:
    require_automation_write(db, user, organization_id)
...
def resolve(
    db: Session, user: User, organization_id: uuid.UUID, issue_id: uuid.UUID, notes: str | None
) -> dict[str, Any]:
    require_automation_write(db, user, organization_id)
    row = _get(db, organization_id, issue_id)
    row.status = "resolved"
    ...
    write_audit_log(
        db,
        action="data_quality.issue.resolved",
        actor_user_id=user.id,
        organization_id=organization_id,
...
def dismiss(
    db: Session, user: User, organization_id: uuid.UUID, issue_id: uuid.UUID, reason: str
) -> dict[str, Any]:
    require_automation_write(db, user, organization_id)
    if not reason.strip():
...
def _get(db: Session, organization_id: uuid.UUID, issue_id: uuid.UUID) -> DataQualityIssue:
    row = db.execute(
        select(DataQualityIssue).where(
            DataQualityIssue.id == issue_id, DataQualityIssue.organization_id == organization_id
        )
    ).scalar_one_or_none()
    if row is None:
