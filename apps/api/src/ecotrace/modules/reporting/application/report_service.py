...
def executive_report(
    ...
    *,
    ...
    dashboard = query_service.dashboard(
        db, user, organization_id, inventory_id=inventory_id, allow_provisional=allow_provisional
    )
    recommendations = query_service.recommendations(
        db, user, organization_id, inventory_id=inventory_id, allow_provisional=allow_provisional
    )
    targets = target_service.list_targets(
        db, user, organization_id, page=1, page_size=50, status="active"
    )
    return {
        "metadata": _meta(organization_id, "executive"),
        "dashboard": dashboard,
        "activeTargets": [t.model_dump(mode="json", by_alias=True) for t in targets.items],
...
def inventory_summary_report(
    ...
    *,
    ...
    return {
        "metadata": _meta(organization_id, "inventory_summary"),
        "summary": dashboard["summary"],
        "scopeDistribution": dashboard["scopeDistribution"],
        "categoryDistribution": dashboard["categoryDistribution"],
        "facilityTotals": dashboard["facilityTotals"],
        "inventoryMetadata": dashboard["metadata"],
    }


def target_progress_report(
    db: Session,
    user: User,
    organization_id: uuid.UUID,
    *,
    ...
    for target in targets.items:
        rows.append(
            target_service.target_progress(
                db,
                user,
                organization_id,
                target.id,
...
def scenario_comparison_report(
    db: Session, user: User, organization_id: uuid.UUID, *, scenario_id: uuid.UUID | None = None
) -> dict[str, Any]:
        ...
        items.append(
            {
                ...
                "latestRun": None
                if latest is None
                else latest.model_dump(mode="json", by_alias=True),
            }
        )
        ...
        for item in report.get("items", []):
            ...
            writer.writerow(
                [
                    ...
                    progress.get("currentValue"),
                    progress.get("targetValue"),
                ]
            )
        ...
        for item in report.get("items", []):
            ...
            writer.writerow(
                [
                    ...
                    scenario.get("name"),
                    run.get("runNumber"),
                    run.get("baselineTotalKgCo2e"),
                    run.get("scenarioTotalKgCo2e"),
                    run.get("reductionKgCo2e"),
                    run.get("reductionPercentage"),
                ]
            )
