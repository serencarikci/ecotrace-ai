...
def notify_org_admins(
    db: Session, organization_id: uuid.UUID, *, title: str, message: str, notification_type: str
) -> dict[str, Any]:
...
def update_preferences(
    ...
    row = db.execute(
        select(NotificationPreference).where(
            ...
            == str(payload.get("notificationType") or "default"),
        )
    ).scalar_one_or_none()
    if row is None:
        row = NotificationPreference(
            user_id=user.id,
            organization_id=organization_id,
            notification_type=str(payload.get("notificationType") or "default"),
        )
        db.add(row)
    for key, attr in {
