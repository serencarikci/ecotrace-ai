...
def _to_user_summary(user: User) -> UserSummary:
    return UserSummary(
        id=user.id, email=user.email, full_name=user.full_name, roles=_user_role_codes(user)
    )
    ...
    *,
    ...
    if _lockout_active(email):
        ...
        raise AuthenticationError(
            "Too many failed login attempts. Try again later.", code="ACCOUNT_LOCKOUT"
        )
    ...
    access_token, expires_in = create_access_token(
        subject=str(user.id), roles=_user_role_codes(user)
    )
...
def refresh_tokens(
    ...
    *,
    ...
    if stored.revoked_at is not None:
        ...
        raise AuthenticationError(
            "Refresh token reuse detected. Please sign in again.", code="TOKEN_REUSE_DETECTED"
        )
    ...
    *,
    ...
    if stored is not None and stored.revoked_at is None:
        ...
        write_audit_log(
            db,
            action="auth.logout",
            actor_user_id=actor_user_id or stored.user_id,
            entity_type="refresh_token",
            entity_id=str(stored.id),
            request_id=request_id,
            ip_address=ip_address,
