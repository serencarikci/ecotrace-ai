...
TEMPLATES: dict[str, dict[str, Any]] = {
    "monthly_carbon_report": {
        ...
        "triggerConfig": {"expression": "monthly", "day": 1},
        "actionType": "generate_report",
        "actionConfig": {"reportType": "carbon_inventory_summary"},
    },
    "weekly_anomaly_scan": {
        "triggerType": "schedule",
        "triggerConfig": {"expression": "weekly", "weekday": 1},
        "actionType": "run_anomaly_detection",
        "actionConfig": {},
    },
    "daily_data_quality_scan": {
        "triggerType": "schedule",
        "triggerConfig": {"expression": "daily"},
        "actionType": "run_data_quality_scan",
        "actionConfig": {},
    },
    "target_risk_alert": {
        "triggerType": "schedule",
        "triggerConfig": {"expression": "weekly"},
        "actionType": "create_alert",
        "actionConfig": {"alertType": "target_risk"},
    },
    "failed_import_alert": {
        "triggerType": "completed_import",
        "triggerConfig": {"onFailure": True},
        "actionType": "create_alert",
        "actionConfig": {"alertType": "import_failure"},
    },
    "quarterly_supplier_review": {
        "triggerType": "schedule",
        "triggerConfig": {"expression": "quarterly"},
        "actionType": "start_agent_execution",
        "actionConfig": {"agentCode": "supplier_review"},
    },
    "regulatory_effective_date_alert": {
        "triggerType": "schedule",
        "triggerConfig": {"expression": "daily"},
        "actionType": "create_alert",
        "actionConfig": {"alertType": "document_expiration"},
    },
}
...
def create_rule(
    ...
    *,
    ...
    row = AutomationRule(
        ...
        trigger_config_json=trigger_config or {},
        condition_config_json=condition_config or {},
        action_type=action_type,
        action_config_json=action_config or {},
        approval_required=approval_required,
        status="draft",
        created_by_user_id=user.id,
        next_run_at=_next_run(trigger_type, trigger_config or {}),
    )
    db.add(row)
    db.flush()
    write_audit_log(
...
def update_rule(
    ...
    for key, attr in {
        "name": "name",
        "description": "description",
        "triggerConfig": "trigger_config_json",
        "conditionConfig": "condition_config_json",
        "actionConfig": "action_config_json",
...
def activate_rule(
    ...
    write_audit_log(
        db,
        action="automation.rule.activated",
        actor_user_id=user.id,
        organization_id=organization_id,
        entity_type="automation_rule",
        entity_id=str(row.id),
...
def run_rule(
    db: Session, user: User, organization_id: uuid.UUID, rule_id: uuid.UUID, *, manual: bool = True
) -> dict[str, Any]:
    require_automation_write(db, user, organization_id)
    row = _get(db, organization_id, rule_id)
    ...
    execution_key = hashlib.sha256(f"{row.id}:{day_key}:{row.action_type}".encode()).hexdigest()[
        :40
    ]
    existing = db.execute(
        select(AutomationRuleExecution).where(
            AutomationRuleExecution.organization_id == organization_id,
            AutomationRuleExecution.execution_key == execution_key,
        )
    ).scalar_one_or_none()
    if existing:
    ...
    job = JobExecution(
        organization_id=organization_id,
        job_type=f"automation:{row.action_type}",
        job_reference_id=row.id,
        execution_key=f"job-{execution_key}",
        status="running",
        scheduled_for=datetime.now(UTC),
        started_at=datetime.now(UTC),
        attempt_number=1,
        max_attempts=settings.job_max_attempts,
        worker_id="api-manual" if manual else "scheduler",
        trace_id=uuid.uuid4().hex,
    )
    db.add(job)
    exec_row = AutomationRuleExecution(
        organization_id=organization_id,
    ...
    write_audit_log(
        ...
        entity_type="automation_rule",
        entity_id=str(row.id),
        metadata={"executionKey": execution_key},
    )
    db.flush()
...
def list_rule_executions(
    ...
    rows = db.execute(
        ...
        .where(
            AutomationRuleExecution.organization_id == organization_id,
            AutomationRuleExecution.automation_rule_id == rule_id,
        )
...
def _dispatch_action(
    db: Session, user: User, organization_id: uuid.UUID, row: AutomationRule
) -> dict[str, Any]:
    ...
    if action == "create_alert":
        ...
        alert = alert_service.create_alert(
            db,
            organization_id,
            alert_type=str(cfg.get("alertType") or "automation"),
            source_type="automation_rule",
            source_id=row.id,
            title=f"Automation: {row.name}",
            message=row.description or row.name,
            severity="medium",
        )
    ...
    if action == "generate_report":
        ...
        return report_service.generate_now(
            db,
            user,
            organization_id,
            report_type=str(cfg.get("reportType") or "executive_sustainability_summary"),
    ...
    if action == "start_agent_execution":
        ...
        return agent_service.execute_agent(
            ...
            user,
            organization_id,
            str(cfg.get("agentCode") or "carbon_analysis"),
            prompt=f"Automation rule {row.code} triggered review.",
    ...
    if action == "notify_users":
        ...
        return notification_service.notify_org_admins(
            db,
            organization_id,
            title=f"Automation {row.name}",
            message=row.description or "Automation completed",
...
def schedule_preview(expression: str) -> str:
    mapping = {
        ...
        "weekly": "Every week (Monday) at 00:00 UTC",
        "monthly": "Monthly on day 1 at 00:00 UTC",
        "quarterly": "Every quarter on day 1 at 00:00 UTC",
        "annual": "Every year on Jan 1 at 00:00 UTC",
...
def _get(db: Session, organization_id: uuid.UUID, rule_id: uuid.UUID) -> AutomationRule:
    row = db.execute(
        select(AutomationRule).where(
            AutomationRule.id == rule_id, AutomationRule.organization_id == organization_id
        )
    ).scalar_one_or_none()
    if row is None:
...
def _ser(row: AutomationRule) -> dict[str, Any]:
    ...
    return {
        ...
        "conditionConfig": row.condition_config_json,
        "actionType": row.action_type,
        "actionConfig": row.action_config_json,
        ...
        "schedulePreview": schedule_preview(str(expr)) if expr else None,
    }
