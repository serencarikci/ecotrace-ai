...
def create_alert(
    ...
    *,
    ...
    existing = db.execute(
        select(Alert).where(Alert.organization_id == organization_id, Alert.fingerprint == fp)
    ).scalar_one_or_none()
    if existing:
...
def get_alert(
    db: Session, user: User, organization_id: uuid.UUID, alert_id: uuid.UUID
) -> dict[str, Any]:
    require_automation_read(db, user, organization_id)
...
def assign(
    db: Session, user: User, organization_id: uuid.UUID, alert_id: uuid.UUID, assignee_id: uuid.UUID
) -> dict[str, Any]:
    require_automation_write(db, user, organization_id)
    row = _get(db, organization_id, alert_id)
...
def resolve(
    db: Session, user: User, organization_id: uuid.UUID, alert_id: uuid.UUID, notes: str | None
) -> dict[str, Any]:
    require_automation_write(db, user, organization_id)
    row = _get(db, organization_id, alert_id)
    row.status = "resolved"
