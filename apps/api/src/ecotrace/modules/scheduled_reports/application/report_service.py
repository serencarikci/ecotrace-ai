...
def create_scheduled(
    ...
    row = ScheduledReport(
        ...
        report_type=str(payload.get("reportType") or "executive_sustainability_summary"),
        schedule_expression=str(payload.get("scheduleExpression") or "monthly"),
        timezone=str(payload.get("timezone") or "UTC"),
        report_config_json=payload.get("reportConfig") or {},
        output_format=str(payload.get("outputFormat") or "json"),
        recipient_user_ids_json=payload.get("recipientUserIds") or [],
        approval_required=bool(payload.get("approvalRequired") or False),
        status="draft",
        created_by_user_id=user.id,
        next_generation_at=datetime.now(UTC),
    )
    db.add(row)
    db.flush()
    write_audit_log(
...
def get_scheduled(
    db: Session, user: User, organization_id: uuid.UUID, report_id: uuid.UUID
) -> dict[str, Any]:
    require_automation_read(db, user, organization_id)
...
def update_scheduled(
    ...
    for key, attr in {
        ...
        "reportConfig": "report_config_json",
        "recipientUserIds": "recipient_user_ids_json",
...
def generate_now(
    ...
    *,
    ...
    status = (
        "pending_approval"
        if scheduled_report_id and _needs_approval(db, scheduled_report_id)
        else "ready"
    )
    row = GeneratedReport(
        organization_id=organization_id,
        ...
        title=f"{report_type.replace('_', ' ').title()} — {datetime.now(UTC).date()}",
        ...
        content_preview_json={"keys": list(payload.keys())[:20]},
    )
    db.add(row)
...
def download(
    ...
    write_audit_log(
        db,
        action="generated_report.downloaded",
        actor_user_id=user.id,
        organization_id=organization_id,
        entity_type="generated_report",
        entity_id=str(row.id),
    )
    media = {"json": "application/json", "csv": "text/csv", "pdf": "application/pdf"}.get(
        row.output_format, "application/octet-stream"
    )
...
def _build_report_payload(
    db: Session, user: User, organization_id: uuid.UUID, report_type: str
) -> dict[str, Any]:
    try:
        ...
        return {
            ...
            "warning": f"Fallback payload due to: {exc}",
    ...
    return {
        ...
        "organizationId": str(organization_id),
        "generatedAt": datetime.now(UTC).isoformat(),
        "summary": f"EcoTrace {report_type} snapshot",
    }
...
def _get_sched(db: Session, organization_id: uuid.UUID, report_id: uuid.UUID) -> ScheduledReport:
    row = db.execute(
        select(ScheduledReport).where(
            ScheduledReport.id == report_id, ScheduledReport.organization_id == organization_id
        )
    ).scalar_one_or_none()
    if row is None:
...
def _ser_sched(row: ScheduledReport) -> dict[str, Any]:
    return {
        ...
        "nextGenerationAt": row.next_generation_at.isoformat() if row.next_generation_at else None,
    }
...
def _ser_gen(row: GeneratedReport) -> dict[str, Any]:
    return {
        "id": str(row.id),
        "scheduledReportId": str(row.scheduled_report_id) if row.scheduled_report_id else None,
        "reportType": row.report_type,
        "title": row.title,
        "outputFormat": row.output_format,
        ...
        "preview": row.content_preview_json,
    }
