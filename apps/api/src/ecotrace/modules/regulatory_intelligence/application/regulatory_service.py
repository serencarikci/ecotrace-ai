...
def list_documents(db: Session, user: User) -> list[dict[str, Any]]:
    _ = user
    rows = db.execute(
        select(RegulatoryDocument).order_by(RegulatoryDocument.updated_at.desc())
    ).scalars()
...
def create_document(db: Session, user: User, payload: dict[str, Any]) -> dict[str, Any]:
    ...
    row = RegulatoryDocument(
        jurisdiction_code=str(payload["jurisdictionCode"]),
        authority_name=str(payload["authorityName"]),
        regulation_code=str(payload["regulationCode"]),
        title=str(payload["title"]),
        description=payload.get("description"),
        category=str(payload.get("category") or "climate"),
        source_url=payload.get("sourceUrl"),
        published_at=_parse_date(payload.get("publishedAt")),
        effective_from=_parse_date(payload.get("effectiveFrom")),
        effective_to=_parse_date(payload.get("effectiveTo")),
        version=str(payload.get("version") or "1"),
        status=str(payload.get("status") or "active"),
        is_demo=bool(payload.get("isDemo", True)),
        applicability_tags_json=payload.get("applicabilityTags") or [],
    )
    db.add(row)
    db.flush()
    write_audit_log(
        db,
        action="regulatory.document.created",
...
def list_assessments(db: Session, user: User, organization_id: uuid.UUID) -> list[dict[str, Any]]:
    ...
    rows = db.execute(
        select(OrganizationRegulatoryAssessment).where(
            OrganizationRegulatoryAssessment.organization_id == organization_id
        )
    ).scalars()
...
def get_assessment(
    db: Session, user: User, organization_id: uuid.UUID, assessment_id: uuid.UUID
) -> dict[str, Any]:
    require_automation_read(db, user, organization_id)
...
def review_assessment(
    ...
    *,
    ...
    write_audit_log(
        db,
        action="regulatory.assessment.reviewed",
        actor_user_id=user.id,
        organization_id=organization_id,
        entity_type="organization_regulatory_assessment",
        entity_id=str(row.id),
    )
    db.flush()
    return _ser_assessment(row)
...
def scan_org(db: Session, user: User, organization_id: uuid.UUID) -> dict[str, Any]:
    ...
    docs = list(
        db.execute(
            select(RegulatoryDocument).where(RegulatoryDocument.status == "active")
        ).scalars()
    )
    created = 0
    for doc in docs:
        existing = db.execute(
            select(OrganizationRegulatoryAssessment).where(
                OrganizationRegulatoryAssessment.organization_id == organization_id,
                OrganizationRegulatoryAssessment.regulatory_document_id == doc.id,
            )
        ).scalar_one_or_none()
        if existing:
            continue
        upcoming = bool(
            doc.effective_from and doc.effective_from <= date.today() + timedelta(days=90)
        )
        row = OrganizationRegulatoryAssessment(
            organization_id=organization_id,
            regulatory_document_id=doc.id,
            applicability_status="unknown",
            relevance_score=Decimal("0.5"),
            review_status="pending",
            impact_summary="AI-assisted classification requires human review.",
            action_required=upcoming,
            due_date=doc.effective_from,
            citations_json=[
                {
                    "label": "R1",
                    "documentName": doc.title,
                    "recordId": str(doc.id),
                    "databaseSource": "regulatory_document",
                }
            ],
        )
        db.add(row)
        ...
        if upcoming:
            ...
            alert_service.create_alert(
                ...
                source_id=doc.id,
                title=f"Upcoming regulatory effective date: {doc.regulation_code}",
                message=f"{doc.title} may become effective soon.",
                severity="medium",
            )
    db.flush()
...
def _get_assessment(
    ...
    row = db.execute(
        select(OrganizationRegulatoryAssessment).where(
            OrganizationRegulatoryAssessment.id == assessment_id,
            OrganizationRegulatoryAssessment.organization_id == organization_id,
        )
    ).scalar_one_or_none()
    if row is None:
...
def _ser_assessment(row: OrganizationRegulatoryAssessment) -> dict[str, Any]:
    return {
        "id": str(row.id),
        "regulatoryDocumentId": str(row.regulatory_document_id),
        "applicabilityStatus": row.applicability_status,
        "relevanceScore": float(row.relevance_score) if row.relevance_score is not None else None,
