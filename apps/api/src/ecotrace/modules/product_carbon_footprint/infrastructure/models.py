...
class ProductCarbonFootprint(Base, UUIDPrimaryKeyMixin, TimestampMixin):
    ...
    __table_args__ = (
        Index("ix_pcf_organization_id", "organization_id"),
        Index("ix_pcf_product_id", "product_id"),
        Index("ix_pcf_product_variant_id", "product_variant_id"),
        Index("ix_pcf_product_batch_id", "product_batch_id"),
        Index("ix_pcf_status", "status"),
        UniqueConstraint(
            "organization_id",
            "product_id",
            "lca_study_id",
    ...
    lca_study_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True), ForeignKey("lca_studies.id", ondelete="RESTRICT"), nullable=False
    )
    calculation_run_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True),
        ForeignKey("lca_calculation_runs.id", ondelete="RESTRICT"),
        nullable=False,
    )
