...
def get_footprint(
    db: Session, organization_id: uuid.UUID, footprint_id: uuid.UUID
...
def list_footprints(
    ...
    *,
...
) -> Page[FootprintResponse]:
    ensure_org_access(db, user, organization_id)
    stmt = select(ProductCarbonFootprint).where(
        ProductCarbonFootprint.organization_id == organization_id
    )
    if product_id:
    ...
    rows = list(
        db.execute(
            stmt.order_by(ProductCarbonFootprint.created_at.desc())
    ...
    return paginate(
        [FootprintResponse.model_validate(r) for r in rows],
        page=page,
        page_size=page_size,
...
def get_footprint_detail(
    db: Session, user: User, organization_id: uuid.UUID, footprint_id: uuid.UUID
) -> FootprintResponse:
    ensure_org_access(db, user, organization_id)
    return FootprintResponse.model_validate(get_footprint(db, organization_id, footprint_id))


def approve_footprint(
    db: Session,
    user: User,
    organization_id: uuid.UUID,
    ...
    *,
    ...
    others = list(
        db.execute(
            select(ProductCarbonFootprint).where(
                ProductCarbonFootprint.organization_id == organization_id,
                ProductCarbonFootprint.product_id == row.product_id,
                ProductCarbonFootprint.product_variant_id == row.product_variant_id,
                ProductCarbonFootprint.product_batch_id == row.product_batch_id,
                ProductCarbonFootprint.lca_study_id == row.lca_study_id,
                ProductCarbonFootprint.id != row.id,
    ...
    write_audit_log(
        db,
        action="product_carbon_footprint.approved",
        actor_user_id=user.id,
        organization_id=organization_id,
        entity_type="product_carbon_footprint",
