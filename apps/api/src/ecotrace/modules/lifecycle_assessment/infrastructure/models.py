...
class LcaStudy(Base, UUIDPrimaryKeyMixin, TimestampMixin):
    ...
    __table_args__ = (
        UniqueConstraint("organization_id", "code", name="uq_lca_studies_org_code"),
        Index("ix_lca_studies_organization_id", "organization_id"),
        Index("ix_lca_studies_product_id", "product_id"),
        Index("ix_lca_studies_product_variant_id", "product_variant_id"),
        Index("ix_lca_studies_product_batch_id", "product_batch_id"),
        Index("ix_lca_studies_status", "status"),
        Index("ix_lca_studies_study_type", "study_type"),
    )
...
class LcaFunctionalUnit(Base, UUIDPrimaryKeyMixin, TimestampMixin):
    ...
    lca_study_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True), ForeignKey("lca_studies.id", ondelete="CASCADE"), nullable=False
    )
...
class LcaSystemBoundary(Base, UUIDPrimaryKeyMixin, TimestampMixin):
    ...
    excluded_processes_json: Mapped[list[Any] | dict[str, Any] | None] = mapped_column(
        JSONB, nullable=True
    )
...
class LcaInventoryInput(Base, UUIDPrimaryKeyMixin, TimestampMixin):
    ...
    __table_args__ = (
        Index("ix_lca_inventory_inputs_lca_study_id", "lca_study_id"),
        Index("ix_lca_inventory_inputs_lifecycle_stage", "lifecycle_stage"),
        Index("ix_lca_inventory_inputs_input_type", "input_type"),
        Index("ix_lca_inventory_inputs_material_id", "material_id"),
        Index("ix_lca_inventory_inputs_supplier_id", "supplier_id"),
        Index("ix_lca_inventory_inputs_activity_type_id", "activity_type_id"),
    )
    lca_study_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True), ForeignKey("lca_studies.id", ondelete="CASCADE"), nullable=False
    )
    ...
    allocation_factor: Mapped[Decimal] = mapped_column(
        Numeric(12, 8), nullable=False, default=Decimal("1")
    )
...
class LcaCalculationRun(Base, UUIDPrimaryKeyMixin, TimestampMixin):
    ...
    __table_args__ = (
        UniqueConstraint("lca_study_id", "run_number", name="uq_lca_run_number"),
        Index("ix_lca_calculation_runs_lca_study_id", "lca_study_id"),
        Index("ix_lca_calculation_runs_status", "status"),
        Index("ix_lca_calculation_runs_created_at", "created_at"),
    )
    lca_study_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True), ForeignKey("lca_studies.id", ondelete="CASCADE"), nullable=False
    )
    ...
    error_summary_json: Mapped[dict[str, Any] | list[Any] | None] = mapped_column(
        JSONB, nullable=True
    )
...
class LcaCalculationItem(Base, UUIDPrimaryKeyMixin, TimestampMixin):
    ...
    __table_args__ = (
        Index("ix_lca_calculation_items_calculation_run_id", "calculation_run_id"),
        Index("ix_lca_calculation_items_lifecycle_stage", "lifecycle_stage"),
        Index("ix_lca_calculation_items_material_id", "material_id"),
        Index("ix_lca_calculation_items_supplier_id", "supplier_id"),
        Index("ix_lca_calculation_items_activity_type_id", "activity_type_id"),
    )
    calculation_run_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True),
        ForeignKey("lca_calculation_runs.id", ondelete="CASCADE"),
        nullable=False,
    )
    lca_study_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True), ForeignKey("lca_studies.id", ondelete="CASCADE"), nullable=False
    )
    inventory_input_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True),
        ForeignKey("lca_inventory_inputs.id", ondelete="RESTRICT"),
        nullable=False,
    )
    lifecycle_stage: Mapped[str] = mapped_column(String(64), nullable=False)
    ...
    factor_source_id: Mapped[uuid.UUID | None] = mapped_column(
        UUID(as_uuid=True),
        ForeignKey("emission_factor_sources.id", ondelete="SET NULL"),
        nullable=True,
    )
...
class LcaDataQualityAssessment(Base, UUIDPrimaryKeyMixin, TimestampMixin):
    ...
    __table_args__ = (
        Index("ix_lca_dq_lca_study_id", "lca_study_id"),
        Index("ix_lca_dq_inventory_input_id", "inventory_input_id"),
    )
    lca_study_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True), ForeignKey("lca_studies.id", ondelete="CASCADE"), nullable=False
    )
    inventory_input_id: Mapped[uuid.UUID | None] = mapped_column(
        UUID(as_uuid=True), ForeignKey("lca_inventory_inputs.id", ondelete="CASCADE"), nullable=True
    )
