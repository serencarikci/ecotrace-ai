...
def validate_study_completeness(db: Session, study: LcaStudy) -> dict[str, Any]:
    ...
    if fu is None:
        errors.append(
            {"code": "missing_functional_unit", "message": "Primary functional unit required."}
        )
    elif fu.quantity <= 0:
        errors.append(
            {
                "code": "invalid_functional_unit",
                "message": "Functional unit quantity must be positive.",
            }
        )
    boundary = db.execute(
    ...
    inputs = list(
        db.execute(select(LcaInventoryInput).where(LcaInventoryInput.lca_study_id == study.id))
        .scalars()
        .all()
    )
    if not inputs:
        errors.append(
            {"code": "missing_inventory", "message": "At least one inventory input required."}
        )
    for inp in inputs:
            ...
            errors.append(
                {
                    ...
                    "message": f"Input {inp.id} allocation factor out of range.",
                    "inventoryInputId": str(inp.id),
                }
            )
        if inp.allocation_method == "custom" and (
            not (inp.metadata_json or {}).get("allocationReason")
        ):
            errors.append(
                {
                    "code": "custom_allocation_reason",
                    "message": "Custom allocation requires a reason.",
                    "inventoryInputId": str(inp.id),
                }
            )
        if inp.allocation_method == "economic" and (not (inp.metadata_json or {}).get("currency")):
            errors.append(
                {
                    "code": "economic_allocation_currency",
                    "message": "Economic allocation requires monetary data and currency.",
                    "inventoryInputId": str(inp.id),
                }
            )
            ...
            errors.append(
                {
                    ...
                    "message": "Source reference required for supplier-specific/database inputs.",
                    "inventoryInputId": str(inp.id),
                }
            )
            ...
            errors.append(
                {
                    ...
                    "message": "Activity type required for factor matching.",
                    "inventoryInputId": str(inp.id),
                }
            )
    return {
        "valid": len(errors) == 0,
        "blockingErrorCount": len(errors),
        "errors": errors,
        "inventoryInputCount": len(inputs),
        "disclaimer": DISCLAIMER,
    }


def run_lca_calculation(
    db: Session,
    user: User,
    study: LcaStudy,
    *,
    ...
    if study.status == "approved":
        raise BusinessRuleError(
            "Approved studies are immutable. Use recalculate to create a new run context."
        )
    ...
    run_number = (
        db.execute(
            select(func.coalesce(func.max(LcaCalculationRun.run_number), 0)).where(
                LcaCalculationRun.lca_study_id == study.id
            )
        ).scalar_one()
        + 1
    )
    for prev in db.execute(
        select(LcaCalculationRun).where(
            LcaCalculationRun.lca_study_id == study.id,
            LcaCalculationRun.status.in_(["completed", "completed_with_errors"]),
        )
    ).scalars():
        prev.status = "superseded"
    gwp = _load_gwp_map(db, "IPCC_AR6")
    run = LcaCalculationRun(
        lca_study_id=study.id,
        run_number=run_number,
        status="running",
        started_at=datetime.now(UTC),
        triggered_by_user_id=user.id,
        inventory_input_count=len(inputs),
        calculated_input_count=0,
        skipped_input_count=0,
        failed_input_count=0,
        engine_version=LCA_ENGINE_VERSION,
    ...
    write_audit_log(
        db,
        action="lca.calculation_started",
        actor_user_id=user.id,
        organization_id=study.organization_id,
        entity_type="lca_calculation_run",
        entity_id=str(run.id),
        request_id=request_id,
        ip_address=ip_address,
        user_agent=user_agent,
        metadata={"studyId": str(study.id), "runNumber": run_number},
    )
    total_kg = Decimal("0")
    ...
    activity_types = (
        {
            a.id: a
            for a in db.execute(select(ActivityType).where(ActivityType.id.in_(activity_ids)))
            .scalars()
            .all()
        }
        if activity_ids
    ...
    for inp in inputs:
        ...
        snapshot: dict[str, Any] = {
            "allocationMethod": inp.allocation_method,
            "allocationFactor": str(inp.allocation_factor),
            "sourceType": inp.source_type,
            "sourceReference": inp.source_reference,
            "gwpDataset": "IPCC_AR6",
            "gwpSnapshot": {k: str(v) for k, v in gwp.items()},
            "disclaimer": DISCLAIMER,
        }
        ...
        try:
            ...
            match = match_emission_factor(
                db,
                organization_id=study.organization_id,
                activity_type_id=inp.activity_type_id,
                activity_date=inp.valid_from,
                activity_unit_code=inp.unit_code,
            ...
            if match.selected is None:
                ...
                errors.extend(
                    match.errors
                    or [{"code": "no_factor", "message": "No compatible emission factor."}]
                )
                ...
                allocated_qty = (normalized_qty * inp.allocation_factor).quantize(
                    Decimal("0.00000001")
                )
                result = compute_emission_result(
                    normalized_quantity=allocated_qty,
        ...
        items.append(
            LcaCalculationItem(
                ...
                input_quantity=inp.quantity,
                input_unit_code=inp.unit_code,
                normalized_quantity=normalized_qty,
                normalized_unit_code=normalized_unit,
                factor_value=factor_value,
                factor_unit_code=factor_unit,
                allocation_factor=inp.allocation_factor,
                allocated_quantity=allocated_qty,
                total_kg_co2e=total_item,
                functional_unit_kg_co2e=fu_item,
    ...
    run.result_summary_json = {
        "byLifecycleStage": {k: str(q_kg(v)) for k, v in by_stage.items()},
        "byMaterial": {k: str(q_kg(v)) for k, v in by_material.items()},
        "bySupplier": {k: str(q_kg(v)) for k, v in by_supplier.items()},
        "byFacility": {k: str(q_kg(v)) for k, v in by_facility.items()},
        "cradleToGateKgCO2e": str(cradle),
        "usePhaseKgCO2e": str(use_phase),
        "endOfLifeKgCO2e": str(eol),
        "biogenicCO2Kg": str(q_kg(biogenic_total)),
        "disclaimer": DISCLAIMER,
    }
    ...
    if run.status in {"completed", "completed_with_errors"}:
        ...
        pcf = ProductCarbonFootprint(
            ...
            product_variant_id=study.product_variant_id,
            product_batch_id=study.product_batch_id,
            functional_unit_quantity=fu.quantity,
            functional_unit_code=fu.unit_code,
            total_kg_co2e=fu_total,
            cradle_to_gate_kg_co2e=cradle,
            use_phase_kg_co2e=use_phase,
            end_of_life_kg_co2e=eol,
            biogenic_co2_kg=q_kg(biogenic_total),
            status="calculated",
        )
        db.add(pcf)
    write_audit_log(
        db,
        action="lca.calculation_completed",
        actor_user_id=user.id,
        organization_id=study.organization_id,
        entity_type="lca_calculation_run",
        entity_id=str(run.id),
        ...
        ip_address=ip_address,
        user_agent=user_agent,
        metadata={"status": run.status, "totalKgCO2e": str(run.total_kg_co2e)},
    )
    return run
