...
from ecotrace.modules.lifecycle_assessment.application.calculation_engine import (
    run_lca_calculation,
    validate_study_completeness,
)
...
from ecotrace.modules.products.application.product_service import (
    get_batch,
    get_product,
    get_variant,
)
from ecotrace.modules.products.application.validators import (
    require_allocation_factor,
    require_dq_score,
...
def list_studies(
    ...
    *,
    ...
    rows = list(
        db.execute(
            stmt.order_by(LcaStudy.code.asc()).offset((page - 1) * page_size).limit(page_size)
        )
        .scalars()
        .all()
    )
    return paginate(
        [StudyResponse.model_validate(r) for r in rows],
        page=page,
        page_size=page_size,
...
def create_study(
    ...
    *,
    ...
    if db.execute(
        select(LcaStudy.id).where(
            LcaStudy.organization_id == organization_id, LcaStudy.code == code
        )
    ).scalar_one_or_none():
    ...
    row = LcaStudy(
        ...
        goal=payload.goal,
        intended_application=payload.intended_application,
    ...
    write_audit_log(
        db,
        action="lca_study.created",
        actor_user_id=user.id,
        organization_id=organization_id,
        entity_type="lca_study",
...
def get_study_detail(
    db: Session, user: User, organization_id: uuid.UUID, study_id: uuid.UUID
) -> StudyResponse:
    ensure_org_access(db, user, organization_id)
    return StudyResponse.model_validate(get_study(db, organization_id, study_id))


def upsert_functional_unit(
    db: Session,
    user: User,
    organization_id: uuid.UUID,
    study_id: uuid.UUID,
    payload: FunctionalUnitUpsert,
    *,
    request_id: str | None = None,
    ip_address: str | None = None,
    user_agent: str | None = None,
) -> FunctionalUnitResponse:
    require_product_write(db, user, organization_id)
    ...
    existing = db.execute(
        select(LcaFunctionalUnit).where(
            LcaFunctionalUnit.lca_study_id == study.id, LcaFunctionalUnit.is_primary.is_(True)
        )
    ).scalar_one_or_none()
    ...
    if existing:
        ...
        row = LcaFunctionalUnit(
            lca_study_id=study.id,
            description=payload.description,
            quantity=payload.quantity,
            unit_code=payload.unit_code,
            reference_flow_description=payload.reference_flow_description,
            normalization_basis=payload.normalization_basis,
            is_primary=True,
        )
        db.add(row)
    write_audit_log(
        ...
        actor_user_id=user.id,
        organization_id=organization_id,
        entity_type="lca_functional_unit",
    ...
    *,
    ...
    for stage in payload.included_stages_json:
        ...
        if stage not in allowed and study.study_type == "cradle_to_gate":
            raise ValidationAppError(
                f"Stage '{stage}' is not compatible with cradle_to_gate study type."
            )
    ...
    if excluded:
        if isinstance(excluded, list):
            ...
            raise ValidationAppError("Excluded processes must include a reason.")
    existing = db.execute(
        select(LcaSystemBoundary).where(LcaSystemBoundary.lca_study_id == study.id)
    ).scalar_one_or_none()
    if existing:
        for field in (
            "boundary_type",
    ...
    write_audit_log(
        ...
        actor_user_id=user.id,
        organization_id=organization_id,
        entity_type="lca_system_boundary",
        entity_id=str(study.id),
        request_id=request_id,
        ip_address=ip_address,
        user_agent=user_agent,
        metadata={"boundaryType": payload.boundary_type},
    )
    db.commit()
    db.refresh(row)
...
def _validate_inventory(
    db: Session, organization_id: uuid.UUID, study: LcaStudy, payload: InventoryCreate
) -> None:
    ...
    if payload.allocation_method == "economic" and (
        not (payload.metadata_json or {}).get("currency")
    ):
...
def list_inventory(
    ...
    *,
    ...
    rows = list(
        db.execute(
            stmt.order_by(LcaInventoryInput.created_at.asc())
    ...
    return paginate(
        [InventoryResponse.model_validate(r) for r in rows],
        page=page,
        page_size=page_size,
        total_items=int(total),
    )
...
def add_inventory(
    ...
    *,
    ...
    dup = db.execute(
        select(LcaInventoryInput.id).where(
            LcaInventoryInput.lca_study_id == study.id,
            LcaInventoryInput.lifecycle_stage == payload.lifecycle_stage,
            LcaInventoryInput.input_type == payload.input_type,
            LcaInventoryInput.material_id == payload.material_id,
            LcaInventoryInput.activity_type_id == payload.activity_type_id,
            LcaInventoryInput.supplier_id == payload.supplier_id,
            LcaInventoryInput.description == payload.description,
            LcaInventoryInput.unit_code == payload.unit_code,
            LcaInventoryInput.source_type == payload.source_type,
            LcaInventoryInput.source_reference == payload.source_reference,
        )
    ).scalar_one_or_none()
    if dup:
    ...
    write_audit_log(
        ...
        actor_user_id=user.id,
        organization_id=organization_id,
        entity_type="lca_inventory_input",
        entity_id=str(study.id),
        request_id=request_id,
        ip_address=ip_address,
        user_agent=user_agent,
...
def validate_study(
    ...
    *,
    ...
    write_audit_log(
        ...
        ip_address=ip_address,
        user_agent=user_agent,
        metadata={"blockingErrorCount": result["blockingErrorCount"]},
    )
    db.commit()
    return result
...
def list_runs(
    ...
    rows = list(
        db.execute(
            select(LcaCalculationRun)
            .where(LcaCalculationRun.lca_study_id == study_id)
            .order_by(LcaCalculationRun.run_number.desc())
        )
        .scalars()
        .all()
    )
...
def get_results(
    ...
    items = list(
        db.execute(
            select(LcaCalculationItem)
            .where(LcaCalculationItem.calculation_run_id == run.id)
            .order_by(LcaCalculationItem.total_kg_co2e.desc().nullslast())
            .limit(50)
        )
        .scalars()
        .all()
    ...
    return {
        "run": RunResponse.model_validate(run).model_dump(by_alias=True),
        "topItems": [ItemResponse.model_validate(i).model_dump(by_alias=True) for i in items],
...
def create_data_quality(
    ...
    *,
    ...
    _assert_editable(study)
    for field, value in (
        ("temporalScore", payload.temporal_score),
        ("geographicScore", payload.geographic_score),
        ("technologicalScore", payload.technological_score),
        ("completenessScore", payload.completeness_score),
    ...
    overall = (
        Decimal(payload.temporal_score)
        + Decimal(payload.geographic_score)
        + Decimal(payload.technological_score)
        + Decimal(payload.completeness_score)
        + Decimal(payload.reliability_score)
    ) / Decimal("5")
    row = LcaDataQualityAssessment(
        lca_study_id=study.id,
        inventory_input_id=payload.inventory_input_id,
        temporal_score=payload.temporal_score,
        geographic_score=payload.geographic_score,
        technological_score=payload.technological_score,
    ...
    write_audit_log(
        ...
        actor_user_id=user.id,
        organization_id=organization_id,
        entity_type="lca_data_quality_assessment",
        entity_id=str(study.id),
        request_id=request_id,
        ip_address=ip_address,
        user_agent=user_agent,
...
def get_study_scope(
    ...
    study = get_study(db, organization_id, study_id)
    fu = db.execute(
        select(LcaFunctionalUnit).where(LcaFunctionalUnit.lca_study_id == study.id)
    ).scalar_one_or_none()
    boundary = db.execute(
        select(LcaSystemBoundary).where(LcaSystemBoundary.lca_study_id == study.id)
    ).scalar_one_or_none()
    return {
        "study": StudyResponse.model_validate(study).model_dump(by_alias=True),
        "functionalUnit": FunctionalUnitResponse.model_validate(fu).model_dump(by_alias=True)
        if fu
        else None,
        "systemBoundary": BoundaryResponse.model_validate(boundary).model_dump(by_alias=True)
        if boundary
        else None,
        "disclaimer": DISCLAIMER,
    }
