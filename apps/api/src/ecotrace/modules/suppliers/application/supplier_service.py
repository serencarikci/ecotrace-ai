...
def _validate_email_url(email: str | None, website: str | None) -> None:
    if email and (not EMAIL_RE.match(email.strip())):
        raise ValidationAppError(
            ...
            details=[{"field": "contactEmail", "message": "Must be a valid email."}],
        )
    if website:
        ...
        if parsed.scheme not in {"http", "https"} or not parsed.netloc:
            raise ValidationAppError(
                "Invalid website URL.",
                details=[{"field": "website", "message": "Must be http(s) URL."}],
            )
...
def list_suppliers(
    ...
    *,
    ...
    rows = list(
        db.execute(
            stmt.order_by(Supplier.code.asc()).offset((page - 1) * page_size).limit(page_size)
        )
        .scalars()
        .all()
    )
    return paginate(
        [SupplierResponse.model_validate(r) for r in rows],
        page=page,
        page_size=page_size,
...
def create_supplier(
    ...
    *,
    ...
    _validate_fields(
        ...
        rating=payload.sustainability_rating,
    )
    exists = db.execute(
    ...
    row = Supplier(
        ...
        supplier_type=payload.supplier_type,
        status=payload.status,
        sustainability_rating=payload.sustainability_rating,
...
def update_supplier(
    ...
    *,
    ...
    _validate_fields(
        ...
        status=status,
        country_code=country,
        email=data.get("contact_email", row.contact_email),
        website=data.get("website", row.website),
        rating=data.get("sustainability_rating", row.sustainability_rating),
    )
