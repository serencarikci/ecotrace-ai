...
class ScenarioModel(Base, UUIDPrimaryKeyMixin, TimestampMixin):
    ...
    __table_args__ = (
        UniqueConstraint("organization_id", "code", name="uq_scenario_org_code"),
        Index("ix_scenario_models_organization_id", "organization_id"),
        Index("ix_scenario_models_baseline_inventory_id", "baseline_inventory_id"),
        Index("ix_scenario_models_scenario_type", "scenario_type"),
        Index("ix_scenario_models_status", "status"),
    )
    ...
    baseline_inventory_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True), ForeignKey("carbon_inventories.id", ondelete="RESTRICT"), nullable=False
    )
...
class ScenarioAssumption(Base, UUIDPrimaryKeyMixin, TimestampMixin):
    ...
    scenario_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True), ForeignKey("scenario_models.id", ondelete="CASCADE"), nullable=False
    )
...
class ScenarioRun(Base, UUIDPrimaryKeyMixin, TimestampMixin):
    ...
    __table_args__ = (
        UniqueConstraint("scenario_id", "run_number", name="uq_scenario_run_number"),
...
class ScenarioRunItem(Base, UUIDPrimaryKeyMixin, TimestampMixin):
    ...
    __table_args__ = (
        Index("ix_scenario_run_items_scenario_run_id", "scenario_run_id"),
        Index("ix_scenario_run_items_activity_record_id", "activity_record_id"),
        Index("ix_scenario_run_items_facility_id", "facility_id"),
        Index("ix_scenario_run_items_activity_type_id", "activity_type_id"),
        Index("ix_scenario_run_items_scope", "scope"),
        Index("ix_scenario_run_items_category", "category"),
    )
    ...
    baseline_calculation_item_id: Mapped[uuid.UUID | None] = mapped_column(
        UUID(as_uuid=True),
        ForeignKey("carbon_calculation_items.id", ondelete="SET NULL"),
        nullable=True,
    )
    activity_record_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True), ForeignKey("activity_records.id", ondelete="RESTRICT"), nullable=False
    )
    facility_id: Mapped[uuid.UUID | None] = mapped_column(
        UUID(as_uuid=True), ForeignKey("facilities.id", ondelete="SET NULL"), nullable=True
    )
