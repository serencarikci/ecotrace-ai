...
def list_scenarios(
    ...
    *,
    ...
    count_stmt = (
        ...
        .where(ScenarioModel.organization_id == organization_id)
    )
    if status:
    ...
    rows = (
        db.execute(
            stmt.order_by(ScenarioModel.code).offset((page - 1) * page_size).limit(page_size)
        )
        .scalars()
        .all()
    )
    return paginate(
        [_scenario_response(r) for r in rows], page=page, page_size=page_size, total_items=total
    )
    ...
    *,
    ...
    existing = db.execute(
        select(ScenarioModel).where(
            ScenarioModel.organization_id == organization_id,
            ScenarioModel.code == payload.code.strip(),
        )
    ).scalar_one_or_none()
    if existing:
    ...
    row = ScenarioModel(
        ...
        baseline_inventory_id=payload.baseline_inventory_id,
        reporting_period_id=payload.reporting_period_id or inventory.reporting_period_id,
        status="draft",
...
def get_scenario(
    db: Session, user: User, organization_id: uuid.UUID, scenario_id: uuid.UUID
) -> ScenarioResponse:
    require_org_read(db, user, organization_id)
...
def add_assumption(
    ...
    row = ScenarioAssumption(
        ...
        scenario_value=payload.scenario_value,
        ...
        errors.append(
            {
                ...
                "message": f"Inventory status {inventory.status} is not usable for scenarios.",
            }
        )
    assumptions = (
        db.execute(select(ScenarioAssumption).where(ScenarioAssumption.scenario_id == scenario_id))
        .scalars()
        .all()
    )
    if not assumptions:
            ...
            errors.append(
                {
                    ...
                    "message": f"Assumption {assumption.parameter_code} has no change value.",
                }
            )
    valid = not errors
...
def _matches_assumption(
    activity: ActivityRecord, item: CarbonCalculationItem, assumption: ScenarioAssumption
) -> bool:
...
def _apply_quantity(
    baseline_qty: Decimal, assumption: ScenarioAssumption
) -> tuple[Decimal, dict[str, Any]]:
    applied: dict[str, Any] = {
        "assumptionId": str(assumption.id),
        "parameterCode": assumption.parameter_code,
...
def _recalculate_item(item: CarbonCalculationItem, scenario_qty: Decimal) -> dict[str, Any]:
    ...
    try:
        result = compute_emission_result(
            normalized_quantity=scenario_qty,
            normalized_unit_code=item.normalized_unit_code or item.activity_unit_code,
            factor_value=_dec(factor.get("factorValue"))
            if factor.get("factorValue") is not None
            else item.factor_value,
            factor_unit_code=factor.get("unitCode")
            or item.factor_unit_code
            or item.activity_unit_code,
            co2_factor=_dec(factor.get("co2Factor")),
            ch4_factor=_dec(factor.get("ch4Factor")),
            n2o_factor=_dec(factor.get("n2oFactor")),
            biogenic_co2_factor=_dec(factor.get("biogenicCo2Factor")),
            ch4_gwp=_dec(gwp.get("CH4")),
            n2o_gwp=_dec(gwp.get("N2O")),
            other_gases_json=item.other_gases_json,
        )
        return result
    except Exception:
        ...
        if baseline_qty and baseline_qty != 0 and (item.total_kg_co2e is not None):
            ...
            return {
                "total_kg_co2e": scaled,
                "total_t_co2e": kg_to_tonnes(scaled),
                "formula": "scaled_from_baseline",
                "co2_kg": None,
                "ch4_kg": None,
                "n2o_kg": None,
        ...
        return {
            "total_kg_co2e": item.total_kg_co2e or Decimal("0"),
            "total_t_co2e": kg_to_tonnes(item.total_kg_co2e or Decimal("0")),
            "formula": "unchanged",
            "co2_kg": item.co2_kg,
            "ch4_kg": item.ch4_kg,
            "n2o_kg": item.n2o_kg,
            "biogenic_co2_kg": item.biogenic_co2_kg,
            "other_gases_json": item.other_gases_json,
        }
...
def calculate_scenario(
    ...
    *,
    ...
    next_number = (
        db.execute(
            select(func.coalesce(func.max(ScenarioRun.run_number), 0)).where(
    ...
    for item in items:
        ...
        run_items.append(
            ScenarioRunItem(
                ...
                applied_assumption_json=applied,
    ...
    run.result_summary_json = {
        ...
        "reductionPercentage": None if pct is None else str(pct),
        "itemCount": len(run_items),
    ...
    write_audit_log(
        ...
        ip_address=ip_address,
        user_agent=user_agent,
        metadata={"scenarioId": str(scenario_id), "runNumber": next_number},
    )
...
def list_runs(
    ...
    rows = (
        db.execute(
            ...
            .order_by(ScenarioRun.run_number.desc())
        )
        .scalars()
        .all()
    )
...
def get_run(
    db: Session, user: User, organization_id: uuid.UUID, scenario_id: uuid.UUID, run_id: uuid.UUID
) -> ScenarioRunResponse:
    require_org_read(db, user, organization_id)
    _get_scenario(db, organization_id, scenario_id)
