...
class CarbonInventory(Base, UUIDPrimaryKeyMixin, TimestampMixin):
    ...
    __table_args__ = (Index('ix_carbon_inventories_organization_id', 'organization_id'), Index('ix_carbon_inventories_reporting_period_id', 'reporting_period_id'), Index('ix_carbon_inventories_status', 'status'), Index('ix_carbon_inventories_created_at', 'created_at'))
    organization_id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), ForeignKey('organizations.id', ondelete='CASCADE'), nullable=False)
    reporting_period_id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), ForeignKey('reporting_periods.id', ondelete='RESTRICT'), nullable=False)
    ...
    partial_calculation: Mapped[bool] = mapped_column(Boolean, nullable=False, default=False, server_default='false')
    calculated_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True), nullable=True)
    calculated_by_user_id: Mapped[uuid.UUID | None] = mapped_column(UUID(as_uuid=True), ForeignKey('users.id', ondelete='SET NULL'), nullable=True)
...
class CarbonCalculationRun(Base, UUIDPrimaryKeyMixin, TimestampMixin):
    ...
    __table_args__ = (UniqueConstraint('inventory_id', 'run_number', name='uq_carbon_runs_inventory_run'), Index('ix_carbon_calculation_runs_inventory_id', 'inventory_id'), Index('ix_carbon_calculation_runs_status', 'status'), Index('ix_carbon_calculation_runs_created_at', 'created_at'))
    inventory_id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), ForeignKey('carbon_inventories.id', ondelete='CASCADE'), nullable=False)
...
class CarbonCalculationItem(Base, UUIDPrimaryKeyMixin, TimestampMixin):
    ...
    __table_args__ = (Index('ix_carbon_calc_items_inventory_id', 'inventory_id'), Index('ix_carbon_calc_items_calculation_run_id', 'calculation_run_id'), Index('ix_carbon_calc_items_activity_record_id', 'activity_record_id'), Index('ix_carbon_calc_items_emission_factor_id', 'emission_factor_id'), Index('ix_carbon_calc_items_scope', 'scope'), Index('ix_carbon_calc_items_category', 'category'), Index('ix_carbon_calc_items_status', 'status'), UniqueConstraint('calculation_run_id', 'activity_record_id', name='uq_carbon_calc_items_run_activity'))
    calculation_run_id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), ForeignKey('carbon_calculation_runs.id', ondelete='CASCADE'), nullable=False)
    inventory_id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), ForeignKey('carbon_inventories.id', ondelete='CASCADE'), nullable=False)
    ...
    emission_factor_id: Mapped[uuid.UUID | None] = mapped_column(UUID(as_uuid=True), ForeignKey('emission_factors.id', ondelete='RESTRICT'), nullable=True)
    factor_source_id: Mapped[uuid.UUID | None] = mapped_column(UUID(as_uuid=True), ForeignKey('emission_factor_sources.id', ondelete='RESTRICT'), nullable=True)
