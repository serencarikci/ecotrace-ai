    ...
    rows = (
        db.execute(
            select(GwpValue).where(
                GwpValue.assessment_report_code == dataset_code,
    ...
    for record in records:
        ...
        try:
            ...
            snapshot = {
                ...
                "disclaimer": (
                    "Demo/reference emission factors are not authoritative and "
                    "must not be used for regulatory reporting."
                    if factor.is_demo
                    else None
                ),
            }
...
def get_item_detail(
    ...
    return {
        ...
        "gasBreakdown": {
            ...
            "totalTCo2e": (
                str(kg_to_tonnes(item.total_kg_co2e))
