...
from ecotrace.shared.application.org_access import (
    ensure_org_access,
    require_manage_structure,
    require_org_read,
    require_period_manage,
...
def _get_inventory(
    db: Session, organization_id: uuid.UUID, inventory_id: uuid.UUID
) -> CarbonInventory:
    ...
    rows = (
        db.execute(
            select(GwpValue).where(
                GwpValue.assessment_report_code == dataset_code, GwpValue.is_active.is_(True)
            )
        )
        .scalars()
        .all()
...
def list_inventories(
    ...
    *,
    ...
    count_stmt = (
        select(func.count())
        .select_from(CarbonInventory)
        .where(CarbonInventory.organization_id == organization_id)
    )
    if status:
    ...
    rows = (
        db.execute(
            stmt.order_by(CarbonInventory.created_at.desc())
    ...
    return paginate(
        [_inventory_response(r) for r in rows], page=page, page_size=page_size, total_items=total
    )
...
def create_inventory(
    ...
    *,
    ...
    version = (
        db.execute(
            select(func.coalesce(func.max(CarbonInventory.version), 0)).where(
                CarbonInventory.organization_id == organization_id,
                CarbonInventory.reporting_period_id == payload.reporting_period_id,
            )
        ).scalar_one()
        + 1
    )
    row = CarbonInventory(
        ...
        status="draft",
        calculation_methodology_version=payload.calculation_methodology_version,
        gwp_dataset_code=payload.gwp_dataset_code,
        version=version,
    )
    db.add(row)
    db.flush()
...
def get_inventory(
    db: Session, user: User, organization_id: uuid.UUID, inventory_id: uuid.UUID
) -> InventoryResponse:
    require_org_read(db, user, organization_id)
...
def _extract_dimensions(record: ActivityRecord) -> dict[str, str | None]:
    ...
    return {
        "technology_code": meta.get("technologyCode") or meta.get("technology_code"),
        "fuel_type": meta.get("fuelType") or meta.get("fuel_type"),
        "transportation_mode": meta.get("transportationMode") or meta.get("transportation_mode"),
    }
...
def validate_inventory(
    ...
    *,
    ...
    records = list(
        db.execute(
            select(ActivityRecord).where(
                ...
                ActivityRecord.reporting_period_id == inventory.reporting_period_id,
    ...
    summary: dict[str, Any] = {
        ...
        "warningCount": 0,
        "activityRecordCount": len(records),
    }
    ...
    for record in records:
        ...
        match = match_emission_factor(
            ...
            technology_code=dims["technology_code"],
            fuel_type=dims["fuel_type"],
            transportation_mode=dims["transportation_mode"],
        )
        if match.ambiguous:
            summary["ambiguousFactors"].append(
                {
                    **entry,
                    "alternatives": [str(a.id) for a in match.alternatives],
                    "errors": match.errors,
                }
            )
            summary["blockingErrorCount"] += 1
        ...
        if any(v is not None for v in (factor.ch4_factor, factor.n2o_factor)):
            ...
            if factor.n2o_factor is not None and "N2O" not in gwp:
                ...
                summary["blockingErrorCount"] += 1
                continue
        summary["valid"].append(
            {
                **entry,
                "emissionFactorId": str(factor.id),
                "matchingPriority": match.priority,
    ...
    write_audit_log(
        ...
        ip_address=ip_address,
        user_agent=user_agent,
        metadata={"blockingErrorCount": summary["blockingErrorCount"]},
    )
    db.commit()
    return summary
    ...
    *,
    ...
    if inventory.status == "approved":
        raise BusinessRuleError(
            "Approved inventories cannot be recalculated in place. Use recalculate."
        )
    ...
    if validation["blockingErrorCount"] > 0 and (not partial):
        raise BusinessRuleError(
            "Blocking validation errors exist. Resolve them or enable partialCalculation.",
            details=[{"blockingErrorCount": validation["blockingErrorCount"]}],
        )
    run_number = (
        db.execute(
            select(func.coalesce(func.max(CarbonCalculationRun.run_number), 0)).where(
    ...
    write_audit_log(
        ...
        actor_user_id=user.id,
        organization_id=organization_id,
        entity_type="carbon_calculation_run",
        entity_id=str(run.id),
        request_id=request_id,
        ip_address=ip_address,
        user_agent=user_agent,
        metadata={"inventoryId": str(inventory.id), "runNumber": run_number},
    )
    records = list(
        db.execute(
            select(ActivityRecord).where(
    ...
    activity_types = (
        {
            a.id: a
            for a in db.execute(select(ActivityType).where(ActivityType.id.in_(activity_type_ids)))
            .scalars()
            .all()
        }
    ...
    for record in records:
                ...
                CarbonCalculationItem(
                    **base_item,
                    ...
                    validation_errors_json=[
                        {"code": "unapproved", "message": "Activity not approved."}
                    ],
                )
            )
            continue
            ...
            items.append(
                CarbonCalculationItem(
                    **base_item,
                    status="failed",
                    validation_errors_json=[
                        {"code": "validation_failed", "message": "Failed pre-validation."}
                    ],
                )
            )
            continue
        ...
        if match.ambiguous or match.selected is None:
            ...
            items.append(
                CarbonCalculationItem(
                    **base_item,
                    status="failed",
                    matching_reason=match.explanation,
        ...
        try:
            activity_unit = get_unit(db, record.normalized_unit_code)
            factor_unit = get_unit(db, factor.unit_code)
            qty_in_factor_unit = convert_between(
                record.normalized_quantity, activity_unit, factor_unit
            )
            result = compute_emission_result(
                normalized_quantity=qty_in_factor_unit,
            ...
            snapshot = {
                ...
                "methodologyVersion": inventory.calculation_methodology_version,
                "gwpDatasetCode": inventory.gwp_dataset_code,
                "gwpValues": gwp_snapshot,
                "activityRecordId": str(record.id),
                "activityTypeId": str(record.activity_type_id),
                "activityTypeCode": activity_type.code if activity_type is not None else None,
                ...
                "quantityInFactorUnit": str(qty_in_factor_unit),
                "factor": {
                    "id": str(factor.id),
                    "code": factor.code,
                    ...
                    "unitCode": factor.unit_code,
                    "factorValue": str(factor.factor_value)
                    if factor.factor_value is not None
                    else None,
                    "co2Factor": str(factor.co2_factor) if factor.co2_factor is not None else None,
                    "ch4Factor": str(factor.ch4_factor) if factor.ch4_factor is not None else None,
                    "n2oFactor": str(factor.n2o_factor) if factor.n2o_factor is not None else None,
                    "biogenicCo2Factor": str(factor.biogenic_co2_factor)
                    if factor.biogenic_co2_factor is not None
                    else None,
                    "isDemo": factor.is_demo,
                },
                "factorSource": {
                    ...
                    "licenseName": source.license_name if source else None,
                    "isDemo": source.is_demo if source else None,
                },
                "matchingPriority": match.priority,
                "matchingReason": match.reason,
                "formula": result["formula"],
                "snapshotAt": datetime.now(UTC).isoformat(),
                "disclaimer": "Demo/reference emission factors are not authoritative and must not be used for regulatory reporting."
                if factor.is_demo
                else None,
            }
            items.append(
                CarbonCalculationItem(
                    **base_item,
                    ...
                    other_gases_json=result["other_gases_json"],
            ...
            items.append(
                CarbonCalculationItem(
                    **base_item,
                    ...
                    factor_source_id=factor.source_id if factor else None,
                    status="failed",
                    validation_errors_json=err,
                )
            )
    ...
    run.error_summary_json = {
        "errors": errors,
        "validation": {
            "blockingErrorCount": validation["blockingErrorCount"],
            "missingFactors": len(validation["missingFactors"]),
            "ambiguousFactors": len(validation["ambiguousFactors"]),
        },
    }
    ...
    write_audit_log(
        db,
        action="calculation.completed" if run.status != "failed" else "calculation.failed",
        actor_user_id=user.id,
        organization_id=organization_id,
        entity_type="carbon_calculation_run",
        entity_id=str(run.id),
...
def submit_review(
    ...
    *,
    ...
    if inventory.latest_run_id and (
        inventory.partial_calculation
        or (inventory.error_summary_json and inventory.error_summary_json.get("errors"))
    ):
        run = db.get(CarbonCalculationRun, inventory.latest_run_id)
        if run and run.status != "completed":
            raise BusinessRuleError(
                "Inventories with unresolved calculation errors cannot be submitted."
            )
    inventory.status = "under_review"
    write_audit_log(
        db,
        action="inventory.submitted_for_review",
        actor_user_id=user.id,
        organization_id=organization_id,
        entity_type="carbon_inventory",
        entity_id=str(inventory.id),
    ...
    *,
    ...
    existing = db.execute(
        select(CarbonInventory).where(
            CarbonInventory.organization_id == organization_id,
            CarbonInventory.reporting_period_id == inventory.reporting_period_id,
            CarbonInventory.status == "approved",
            CarbonInventory.id != inventory.id,
        )
    ).scalar_one_or_none()
    if existing:
...
def recalculate_inventory(
    ...
    *,
    ...
    inventory = _get_inventory(db, organization_id, inventory_id)
    if inventory.status == "approved":
        clone = CarbonInventory(
            organization_id=organization_id,
            reporting_period_id=inventory.reporting_period_id,
            name=f"{inventory.name} (recalc)",
            description=inventory.description,
            status="draft",
            calculation_methodology_version=inventory.calculation_methodology_version,
            gwp_dataset_code=inventory.gwp_dataset_code,
        ...
        write_audit_log(
            ...
            ip_address=ip_address,
            user_agent=user_agent,
            metadata={"fromInventoryId": str(inventory.id)},
        )
        db.commit()
...
def list_runs(
    ...
    rows = (
        db.execute(
            select(CarbonCalculationRun)
            .where(CarbonCalculationRun.inventory_id == inventory_id)
            .order_by(CarbonCalculationRun.run_number.desc())
        )
        .scalars()
        .all()
    )
...
def list_items(
    ...
    *,
    ...
    stmt = select(CarbonCalculationItem).where(
        CarbonCalculationItem.inventory_id == inventory_id,
        CarbonCalculationItem.calculation_run_id == target_run,
    )
    count_stmt = (
        select(func.count())
        .select_from(CarbonCalculationItem)
        .where(
            CarbonCalculationItem.inventory_id == inventory_id,
            CarbonCalculationItem.calculation_run_id == target_run,
        )
    ...
    rows = (
        db.execute(
            stmt.order_by(CarbonCalculationItem.created_at)
            .offset((page - 1) * page_size)
            .limit(page_size)
        )
        .scalars()
    ...
    return paginate(
        [_item_response(r) for r in rows], page=page, page_size=page_size, total_items=total
    )


def get_item_detail(
    db: Session, user: User, organization_id: uuid.UUID, item_id: uuid.UUID
) -> dict[str, Any]:
    require_org_read(db, user, organization_id)
    ...
    return {
        ...
        "activity": {
            "id": str(activity.id) if activity else None,
            "quantity": str(activity.quantity) if activity else None,
            "unitCode": activity.unit_code if activity else None,
            "normalizedQuantity": str(activity.normalized_quantity) if activity else None,
            "normalizedUnitCode": activity.normalized_unit_code if activity else None,
            ...
            "facilityId": str(activity.facility_id) if activity and activity.facility_id else None,
            "facilityName": facility.name if facility else None,
            "facilityCountry": facility.country_code if facility else None,
        },
        "factor": factor_snapshot(factor) if factor else None,
        "source": {
            "id": str(source.id) if source else None,
            "code": source.code if source else None,
            "name": source.name if source else None,
        ...
        "gasBreakdown": {
            "co2Kg": str(item.co2_kg) if item.co2_kg is not None else None,
            "ch4Kg": str(item.ch4_kg) if item.ch4_kg is not None else None,
            "n2oKg": str(item.n2o_kg) if item.n2o_kg is not None else None,
            "biogenicCo2Kg": str(item.biogenic_co2_kg)
            if item.biogenic_co2_kg is not None
            else None,
            "totalKgCo2e": str(item.total_kg_co2e) if item.total_kg_co2e is not None else None,
            "totalTCo2e": str(kg_to_tonnes(item.total_kg_co2e))
            if isinstance(item.total_kg_co2e, Decimal)
            else None,
        },
        "matchingExplanation": item.matching_reason,
        "matchingPriority": item.matching_priority,
        "snapshot": item.calculation_snapshot_json,
        "validation": item.validation_errors_json,
    }
...
def factor_snapshot(factor: EmissionFactor) -> dict[str, Any]:
    return {
        ...
        "geographyCode": factor.geography_code,
        "unitCode": factor.unit_code,
        "factorValue": str(factor.factor_value) if factor.factor_value is not None else None,
...
def inventory_summary(
    ...
    if not inventory.latest_run_id:
        return {
            ...
            "activityTypeTotals": [],
            "greenhouseGasTotals": {},
            "itemCounts": {"calculated": 0, "failed": 0, "skipped": 0},
            "errorCounts": 0,
        }
    items = list(
        db.execute(
            select(CarbonCalculationItem).where(
                CarbonCalculationItem.calculation_run_id == inventory.latest_run_id
            )
        )
        .scalars()
        .all()
    ...
    gas_totals = {
        ...
        "ch4Kg": Decimal("0"),
        "n2oKg": Decimal("0"),
        "biogenicCo2Kg": Decimal("0"),
    }
    activity_ids = {i.activity_record_id for i in calc_items}
    activities = (
        {
            a.id: a
            for a in db.execute(select(ActivityRecord).where(ActivityRecord.id.in_(activity_ids)))
            .scalars()
            .all()
        }
        if activity_ids
    ...
    facilities = (
        {
            f.id: f
            for f in db.execute(select(Facility).where(Facility.id.in_(facility_ids)))
            .scalars()
            .all()
        }
        if facility_ids
    ...
    types = (
        {
            t.id: t
            for t in db.execute(select(ActivityType).where(ActivityType.id.in_(type_ids)))
            .scalars()
            .all()
        }
        if type_ids
    ...
    for item in calc_items:
        ...
        if item.category:
            category_map[item.category] = (
                category_map.get(item.category, Decimal("0")) + item.total_kg_co2e
            )
    ...
    return {
        "inventoryId": str(inventory.id),
        "status": inventory.status,
        "partialCalculation": inventory.partial_calculation,
        "totalKgCo2e": str(total),
        "totalTCo2e": str(kg_to_tonnes(total)),
        "scope1TotalKgCo2e": str(scope_totals["scope_1"]),
        "scope2TotalKgCo2e": str(scope_totals["scope_2"]),
        "scope3TotalKgCo2e": str(scope_totals["scope_3"]),
        "scope1TotalTCo2e": str(kg_to_tonnes(scope_totals["scope_1"])),
        "scope2TotalTCo2e": str(kg_to_tonnes(scope_totals["scope_2"])),
        "scope3TotalTCo2e": str(kg_to_tonnes(scope_totals["scope_3"])),
        "categoryTotals": [
            {"category": k, "kgCo2e": str(v), "tCo2e": str(kg_to_tonnes(v))}
            for k, v in sorted(category_map.items())
        ],
        "facilityTotals": [
            {"facility": k, "kgCo2e": str(v), "tCo2e": str(kg_to_tonnes(v))}
            for k, v in sorted(facility_map.items())
        ],
        "activityTypeTotals": [
            {"activityType": k, "kgCo2e": str(v), "tCo2e": str(kg_to_tonnes(v))}
            for k, v in sorted(activity_map.items())
        ],
        "greenhouseGasTotals": {k: str(v) for k, v in gas_totals.items()},
        "itemCounts": {
            "calculated": sum(1 for i in items if i.status == "calculated"),
            "failed": sum(1 for i in items if i.status == "failed"),
            "skipped": sum(1 for i in items if i.status == "skipped"),
        },
        "errorCounts": sum(1 for i in items if i.status == "failed"),
    }


def preview_factor_match(
    db: Session, user: User, organization_id: uuid.UUID, payload: MatchPreviewRequest
) -> dict[str, Any]:
    ...
    if activity_type_id is None or unit_code is None:
        raise ValidationAppError(
            "activityTypeId and unitCode are required when no activityRecordId."
        )
    match = match_emission_factor(
        db,
        organization_id=organization_id,
    ...
    return {
        "selectedFactor": factor_snapshot(match.selected) if match.selected else None,
        "matchingPriority": match.priority,
        "matchingExplanation": match.explanation or match.reason,
        "alternativeFactors": [factor_snapshot(f) for f in match.alternatives],
        "ambiguityStatus": match.ambiguous,
        "validationErrors": match.errors,
    }
