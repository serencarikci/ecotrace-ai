...
def list_records(
    ...
    *,
    ...
    stmt = select(ActivityRecord).where(
        ActivityRecord.organization_id == organization_id, ActivityRecord.is_archived.is_(False)
    )
    if facility_id:
    ...
    if search:
        like = f"%{search.strip()}%"
        stmt = stmt.where(
            or_(ActivityRecord.source_reference.ilike(like), ActivityRecord.description.ilike(like))
        )
...
def create_record(
    ...
    *,
    ...
    normalized, normalized_unit = normalize_quantity(
        db, quantity=payload.quantity, unit_code=payload.unit_code, activity_type=activity_type
    )
    record = ActivityRecord(
    ...
    db.flush()
    _add_revision(
        db, record, change_type="created", user=user, previous=None, new=_snapshot(record)
    )
    write_audit_log(
        db,
...
def update_record(
    ...
    *,
    ...
    _add_revision(
        db, record, change_type="updated", user=user, previous=previous, new=_snapshot(record)
    )
    write_audit_log(
        db,
...
def correct_record(
    ...
    *,
    ...
    if record.quantity < 0:
        raise ValidationAppError("Quantity must be greater than or equal to zero.")
    normalized, normalized_unit = normalize_quantity(
        db, quantity=record.quantity, unit_code=record.unit_code, activity_type=activity_type
    )
    record.normalized_quantity = normalized
    record.normalized_unit_code = normalized_unit
...
def archive_record(
    ...
    *,
    ...
    if not codes.intersection(
        {ROLE_SYSTEM_ADMIN, ROLE_ORGANIZATION_ADMIN, ROLE_ANALYST, ROLE_SUSTAINABILITY_MANAGER}
    ):
