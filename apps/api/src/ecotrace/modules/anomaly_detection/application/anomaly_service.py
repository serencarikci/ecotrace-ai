...
from ecotrace.modules.anomaly_detection.application.detectors import (
    fingerprint,
    percentage_change,
    severity_from_score,
    z_score,
...
def list_rules(db: Session, user: User, organization_id: uuid.UUID) -> list[dict[str, Any]]:
    ...
    rows = db.execute(
        select(AnomalyDetectionRule).where(AnomalyDetectionRule.organization_id == organization_id)
    ).scalars()
...
def create_rule(
    ...
    row = AnomalyDetectionRule(
        organization_id=organization_id,
        code=str(payload["code"]),
        name=str(payload["name"]),
        description=payload.get("description"),
        metric_type=str(payload.get("metricType") or "activity_quantity"),
        detection_method=str(payload.get("detectionMethod") or "z_score"),
        threshold_config_json=payload.get("thresholdConfig") or {"z": 3.0, "pct": 50.0},
        scope_config_json=payload.get("scopeConfig") or {},
        severity_mapping_json=payload.get("severityMapping") or {},
        minimum_data_points=int(payload.get("minimumDataPoints") or 5),
        lookback_period=int(payload.get("lookbackPeriod") or 12),
        is_active=True,
    )
    db.add(row)
    db.flush()
    write_audit_log(
        db,
        action="anomaly.rule.created",
        actor_user_id=user.id,
        organization_id=organization_id,
        entity_type="anomaly_detection_rule",
...
def update_rule(
    db: Session, user: User, organization_id: uuid.UUID, rule_id: uuid.UUID, payload: dict[str, Any]
) -> dict[str, Any]:
    require_automation_manage(db, user, organization_id)
    row = db.execute(
        select(AnomalyDetectionRule).where(
            AnomalyDetectionRule.id == rule_id,
            AnomalyDetectionRule.organization_id == organization_id,
        )
    ).scalar_one_or_none()
    if row is None:
...
def run_org_scan(db: Session, user: User, organization_id: uuid.UUID) -> dict[str, Any]:
    ...
    rules = list(
        db.execute(
            select(AnomalyDetectionRule).where(
                AnomalyDetectionRule.organization_id == organization_id,
                AnomalyDetectionRule.is_active.is_(True),
            )
        ).scalars()
    )
    created = 0
...
def list_anomalies(db: Session, user: User, organization_id: uuid.UUID) -> list[dict[str, Any]]:
    ...
    rows = db.execute(
        select(AnomalyEvent)
        .where(AnomalyEvent.organization_id == organization_id)
...
def get_anomaly(
    db: Session, user: User, organization_id: uuid.UUID, anomaly_id: uuid.UUID
) -> dict[str, Any]:
    require_automation_read(db, user, organization_id)
...
def resolve(
    db: Session, user: User, organization_id: uuid.UUID, anomaly_id: uuid.UUID, notes: str | None
) -> dict[str, Any]:
    require_automation_write(db, user, organization_id)
    row = _get_event(db, organization_id, anomaly_id)
    row.status = "resolved"
...
def dismiss(
    db: Session, user: User, organization_id: uuid.UUID, anomaly_id: uuid.UUID, reason: str
) -> dict[str, Any]:
    require_automation_write(db, user, organization_id)
    if not reason.strip():
...
def _detect_for_rule(
    db: Session, user: User, organization_id: uuid.UUID, rule: AnomalyDetectionRule
) -> dict[str, Any]:
    _ = user
    ...
    rows = list(
        db.execute(
            ...
            .limit(max(rule.lookback_period, 12))
        ).scalars()
    )
    ...
    if abs(score) < thr and (
        pct is None or abs(pct) < float((rule.threshold_config_json or {}).get("pct") or 50)
    ):
        return {"created": 0}
    fp = fingerprint(
        organization_id, rule.code, "activity", round(observed, 4), datetime.now(UTC).date()
    )
    existing = db.execute(
        select(AnomalyEvent).where(
            AnomalyEvent.organization_id == organization_id, AnomalyEvent.fingerprint == fp
        )
    ).scalar_one_or_none()
    if existing:
    ...
    event = AnomalyEvent(
        ...
        metric_code=rule.metric_type,
        observed_value=Decimal(str(observed)),
        expected_value=Decimal(str(round(expected, 8))),
        deviation_value=Decimal(str(round(observed - expected, 8))),
        deviation_percentage=Decimal(str(round(pct or 0.0, 8))) if pct is not None else None,
        anomaly_score=Decimal(str(round(abs(score), 8))),
        severity=severity_from_score(abs(score), rule.severity_mapping_json),
        status="open",
        ...
        evidence_json={
            ...
            "history": history[:12],
            "note": "Statistical anomaly indicator; not automatically an error.",
        },
    )
    db.add(event)
    db.flush()
    ...
    alert_service.create_alert(
        ...
        message=f"Detected {rule.metric_type} deviation (score={abs(score):.2f}).",
...
def _default_activity_scan(db: Session, user: User, organization_id: uuid.UUID) -> int:
    rule = AnomalyDetectionRule(
        organization_id=organization_id,
        code="default-activity-z",
        name="Default activity z-score",
        description="Auto-created scan rule",
        metric_type="activity_quantity",
        detection_method="z_score",
        threshold_config_json={"z": 2.5, "pct": 40},
...
def _get_event(db: Session, organization_id: uuid.UUID, anomaly_id: uuid.UUID) -> AnomalyEvent:
    row = db.execute(
        select(AnomalyEvent).where(
            AnomalyEvent.id == anomaly_id, AnomalyEvent.organization_id == organization_id
        )
    ).scalar_one_or_none()
    if row is None:
...
def _ser_rule(row: AnomalyDetectionRule) -> dict[str, Any]:
    return {
        ...
        "thresholdConfig": row.threshold_config_json,
...
def _ser_event(row: AnomalyEvent) -> dict[str, Any]:
    return {
        ...
        "observedValue": float(row.observed_value) if row.observed_value is not None else None,
        "expectedValue": float(row.expected_value) if row.expected_value is not None else None,
        "deviationValue": float(row.deviation_value) if row.deviation_value is not None else None,
        "deviationPercentage": float(row.deviation_percentage)
        if row.deviation_percentage is not None
        else None,
        ...
        "assignedToUserId": str(row.assigned_to_user_id) if row.assigned_to_user_id else None,
