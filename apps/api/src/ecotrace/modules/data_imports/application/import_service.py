...
def list_jobs(
    ...
    *,
    ...
    rows = list(
        db.execute(
            stmt.order_by(ImportJob.created_at.desc())
    ...
    return paginate(
        [ImportJobResponse.model_validate(r) for r in rows],
        page=page,
        page_size=page_size,
...
def list_rows(
    ...
    *,
    ...
    rows = list(
        db.execute(
            stmt.order_by(ImportJobRow.row_number).offset((page - 1) * page_size).limit(page_size)
        )
        .scalars()
        .all()
    )
    return paginate(
        [ImportJobRowResponse.model_validate(r) for r in rows],
        page=page,
        page_size=page_size,
        total_items=int(total),
    )
    ...
    *,
    ...
    return "|".join(
        [
            ...
            activity_date.isoformat() if activity_date else "",
    ...
    *,
    ...
    if missing:
        raise ValidationAppError(
            ...
            details=[{"field": "headers", "message": f"Missing: {', '.join(missing)}"}],
        )
    rows = list(reader)
...
def _lookup_activity_type(db: Session, code: str) -> ActivityType | None:
    return db.execute(
        select(ActivityType).where(ActivityType.code == code, ActivityType.is_active.is_(True))
    ).scalar_one_or_none()


def _lookup_period(
    db: Session, organization_id: uuid.UUID, code: str | None, activity_date: date | None
    ...
    if code:
        return db.execute(
            select(ReportingPeriod).where(
                ...
                ReportingPeriod.status != "archived",
            )
        ).scalar_one_or_none()
    ...
    return db.execute(
        ...
        .where(
            ReportingPeriod.organization_id == organization_id,
            ReportingPeriod.status.in_(("open", "under_review")),
            ReportingPeriod.start_date <= activity_date,
            ReportingPeriod.end_date >= activity_date,
        )
    ...
    *,
    ...
    for row in rows:
        ...
        if activity_date is None and (not period_start_raw):
            errors.append(
                {"field": "activityDate", "message": "Required when periodStart is empty."}
            )
        try:
        ...
        period = _lookup_period(
            db, organization_id, reporting_period_code, activity_date or period_start
        )
        if period is None:
            ...
            try:
                ...
                errors.append(
                    {"field": "reportingPeriodCode", "message": str(getattr(exc, "message", exc))}
                )
        if facility and production_line_code:
            line = db.execute(
                select(ProductionLine).where(
                    ...
                    ProductionLine.code == production_line_code,
                    ProductionLine.is_active.is_(True),
                )
            ).scalar_one_or_none()
            if line is None:
                errors.append(
                    {"field": "productionLineCode", "message": "Unknown production line."}
                )
            else:
        ...
        if data_source_code:
            ds = db.execute(
                select(DataSource).where(
                    ...
                    DataSource.code == data_source_code,
        ...
        if activity_type and quantity is not None and unit_code and (not errors):
            try:
                normalized_quantity, normalized_unit_code = normalize_quantity(
            ...
            if key in seen_keys:
                ...
                existing = db.execute(
                    select(ActivityRecord.id).where(
                        ...
                        ActivityRecord.facility_id == facility.id,
                        ActivityRecord.activity_type_id == activity_type.id,
                        ActivityRecord.activity_date == activity_date,
                        ActivityRecord.period_start == period_start,
                        ActivityRecord.period_end == period_end,
                        ActivityRecord.source_reference == source_reference,
                        ActivityRecord.quantity == quantity,
                        ActivityRecord.unit_code == unit_code,
        ...
        if errors:
            ...
            row.normalized_data_json = {
                ...
                "activityTypeId": str(activity_type.id),
                "reportingPeriodId": str(period.id),
                "productionLineId": str(production_line_id) if production_line_id else None,
                "equipmentId": str(equipment_id) if equipment_id else None,
                "dataSourceId": str(data_source_id) if data_source_id else None,
                ...
                "unitCode": unit_code,
                "normalizedQuantity": format(normalized_quantity, "f")
                if normalized_quantity is not None
                else None,
                "normalizedUnitCode": normalized_unit_code,
    ...
    rows = list(
        db.execute(
            select(ImportJobRow).where(
                ...
                ImportJobRow.validation_status == "valid",
                ImportJobRow.activity_record_id.is_(None),
            )
        )
        .scalars()
        .all()
    ...
    try:
        for row in rows:
            ...
            record = ActivityRecord(
                organization_id=organization_id,
                facility_id=uuid.UUID(data["facilityId"]),
                production_line_id=uuid.UUID(data["productionLineId"])
                if data.get("productionLineId")
                else None,
                equipment_id=uuid.UUID(data["equipmentId"]) if data.get("equipmentId") else None,
                data_source_id=uuid.UUID(data["dataSourceId"])
                if data.get("dataSourceId")
                else None,
                activity_type_id=uuid.UUID(data["activityTypeId"]),
                reporting_period_id=uuid.UUID(data["reportingPeriodId"]),
                activity_date=date.fromisoformat(data["activityDate"])
                if data.get("activityDate")
                else None,
                period_start=date.fromisoformat(data["periodStart"])
                if data.get("periodStart")
                else None,
                period_end=date.fromisoformat(data["periodEnd"]) if data.get("periodEnd") else None,
                quantity=Decimal(data["quantity"]),
                unit_code=data["unitCode"],
                normalized_quantity=Decimal(data["normalizedQuantity"]),
                normalized_unit_code=data["normalizedUnitCode"],
                status="draft",
                source_reference=data.get("sourceReference"),
                description=data.get("description"),
                notes=data.get("notes"),
                metadata_json={"importJobId": str(job.id), "importRowNumber": row.row_number},
                created_by_user_id=user.id,
        ...
        job.completed_at = datetime.now(UTC)
        job.status = (
            "completed" if failed == 0 and job.invalid_rows == 0 else "completed_with_errors"
        )
        write_audit_log(
            db,
            ...
            ip_address=ip_address,
            user_agent=user_agent,
            metadata={"importedRows": imported},
        )
        db.commit()
    except Exception:
        db.rollback()
