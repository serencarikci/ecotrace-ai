...
class ImportJobRow(Base, UUIDPrimaryKeyMixin):
    ...
    __table_args__ = (
        UniqueConstraint("import_job_id", "row_number", name="uq_import_job_rows_job_row"),
        Index("ix_import_job_rows_import_job_id", "import_job_id"),
    )
    ...
    activity_record_id: Mapped[uuid.UUID | None] = mapped_column(
        UUID(as_uuid=True), ForeignKey("activity_records.id", ondelete="SET NULL"), nullable=True
    )
