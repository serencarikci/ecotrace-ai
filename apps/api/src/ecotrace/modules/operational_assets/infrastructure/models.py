...
class ProductionLine(Base, UUIDPrimaryKeyMixin, TimestampMixin):
    ...
    __table_args__ = (UniqueConstraint('facility_id', 'code', name='uq_production_lines_facility_code'), Index('ix_production_lines_organization_id', 'organization_id'), Index('ix_production_lines_facility_id', 'facility_id'), CheckConstraint('capacity_value IS NULL OR capacity_value > 0', name='capacity_positive'))
...
class Equipment(Base, UUIDPrimaryKeyMixin, TimestampMixin):
    ...
    __table_args__ = (UniqueConstraint('facility_id', 'code', name='uq_equipment_facility_code'), Index('ix_equipment_organization_id', 'organization_id'), Index('ix_equipment_facility_id', 'facility_id'), Index('ix_equipment_production_line_id', 'production_line_id'), Index('ix_equipment_equipment_type', 'equipment_type'), Index('ix_equipment_is_active', 'is_active'), CheckConstraint('decommissioning_date IS NULL OR commissioning_date IS NULL OR decommissioning_date >= commissioning_date', name='equip_dates'))
    organization_id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), ForeignKey('organizations.id', ondelete='CASCADE'), nullable=False)
    facility_id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), ForeignKey('facilities.id', ondelete='CASCADE'), nullable=False)
...
class DataSource(Base, UUIDPrimaryKeyMixin, TimestampMixin):
    ...
    __table_args__ = (UniqueConstraint('organization_id', 'code', name='uq_data_sources_org_code'), Index('ix_data_sources_organization_id', 'organization_id'), Index('ix_data_sources_facility_id', 'facility_id'), Index('ix_data_sources_equipment_id', 'equipment_id'), Index('ix_data_sources_source_type', 'source_type'))
    organization_id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), ForeignKey('organizations.id', ondelete='CASCADE'), nullable=False)
