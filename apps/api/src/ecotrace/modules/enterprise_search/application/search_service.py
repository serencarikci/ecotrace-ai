...
def enterprise_search(
    ...
    *,
    ...
    if mode in {"semantic", "hybrid", "full-text"}:
        evidence, metrics = hybrid_retrieve(
            db,
            user,
            organization_id,
            query=q,
            ...
            results.append(
                {
                    ...
                    "recordId": item.document_id,
                }
            )
            facets["sources"]["document"] = facets["sources"].get("document", 0) + 1
            facets["documentTypes"][item.document_type] = (
                facets["documentTypes"].get(item.document_type, 0) + 1
            )
        retrieval_metrics = metrics
    else:
        retrieval_metrics = {}
    like = f"%{q}%"
    for model, source, title_attr in (
        (Product, "product", "name"),
        (CarbonInventory, "carbon_inventory", "name"),
        (DigitalProductPassport, "passport", "title"),
        (SustainabilityTarget, "target", "name"),
        (ScenarioModel, "scenario", "name"),
        (KnowledgeDocument, "knowledge_document", "title"),
    ):
        stmt = (
            select(model)
            .where(model.organization_id == organization_id)
            .where(getattr(model, title_attr).ilike(like))
            .limit(5)
        )
...
def autocomplete(
    db: Session, user: User, organization_id: uuid.UUID, *, prefix: str, limit: int = 10
) -> list[dict[str, str]]:
    require_ai_read(db, user, organization_id)
    ...
    products = db.execute(
        select(Product.name)
        .where(Product.organization_id == organization_id, Product.name.ilike(like))
        .limit(limit)
    ).scalars()
...
def list_saved_searches(
    ...
    rows = db.execute(
        select(SavedSearch).where(
            SavedSearch.organization_id == organization_id, SavedSearch.owner_user_id == user.id
        )
    ).scalars()
    return [
        {
...
def conversation_analytics(db: Session, user: User, organization_id: uuid.UUID) -> dict[str, Any]:
    ...
    messages = db.execute(
        ...
        .where(ChatMessage.organization_id == organization_id)
    ).scalar_one()
    avg_latency = db.execute(
        select(func.avg(RetrievalLog.llm_latency_ms)).where(
    ...
    cache_hits = db.execute(
        ...
        .where(RetrievalLog.organization_id == organization_id, RetrievalLog.cache_hit.is_(True))
    ).scalar_one()
    ...
    return {
        ...
        "messages": messages,
        "avgLlmLatencyMs": float(avg_latency or 0),
        "cacheHitRate": cache_hits / total_logs if total_logs else 0.0,
        "retrievalLogs": total_logs,
    }
