...
def list_materials(
    ...
    *,
    ...
    rows = list(
        db.execute(
            stmt.order_by(Material.code.asc()).offset((page - 1) * page_size).limit(page_size)
        )
        .scalars()
        .all()
    )
    return paginate(
        [MaterialResponse.model_validate(r) for r in rows],
        page=page,
        page_size=page_size,
...
def create_material(
    ...
    *,
    ...
    _validate(
        ...
        renewable=payload.renewable_content_percentage,
        density=payload.density_value,
        supplier_id=payload.supplier_id,
        country=country,
    )
    if db.execute(
        select(Material.id).where(
            Material.organization_id == organization_id, Material.code == code
        )
    ).scalar_one_or_none():
    ...
    row = Material(
        ...
        renewable_content_percentage=payload.renewable_content_percentage,
...
def update_material(
    ...
    *,
    ...
    _validate(
        db,
        organization_id,
        category=data.get("material_category", row.material_category),
        ...
        renewable=data.get("renewable_content_percentage", row.renewable_content_percentage),
        density=data.get("density_value", row.density_value),
...
def get_material_detail(
    db: Session, user: User, organization_id: uuid.UUID, material_id: uuid.UUID
) -> MaterialResponse:
    ensure_org_access(db, user, organization_id)
