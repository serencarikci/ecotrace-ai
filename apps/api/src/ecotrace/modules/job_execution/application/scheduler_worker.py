    ...
    for rule in rules:
        ...
        exists = db.execute(
            select(JobExecution.id).where(
                JobExecution.organization_id == rule.organization_id,
                JobExecution.execution_key == key,
            )
        ).scalar_one_or_none()
        if exists:
        ...
        job = JobExecution(
            ...
            job_type=f"automation:{rule.action_type}",
            job_reference_id=rule.id,
            execution_key=key,
            status="scheduled",
            scheduled_for=now,
            attempt_number=1,
            ...
            result_summary_json={
                "note": "Due automation captured by scheduler. Trigger manual run or API worker.",
                "ruleCode": rule.code,
            },
        )
        db.add(job)
