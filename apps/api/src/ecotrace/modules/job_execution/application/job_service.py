...
def retry_job(
    ...
    row.error_details_json = {
        **(row.error_details_json or {}),
        "retryScheduledInSeconds": backoff,
        "maxAttempts": settings.job_max_attempts,
    }
    write_audit_log(
        db,
        action="job.retried",
...
def try_acquire_lock(db: Session, job: JobExecution, worker_id: str) -> bool:
    if (
        job.locked_at
        and job.locked_at > datetime.now(UTC) - timedelta(minutes=10)
        and job.worker_id
...
def recent_failures(db: Session, user: User, *, limit: int = 20) -> list[dict[str, Any]]:
    ...
    rows = db.execute(
        select(JobExecution)
        .where(JobExecution.status.in_(["failed", "timed_out"]))
        .order_by(JobExecution.created_at.desc())
        .limit(limit)
    ).scalars()
    return [_ser(r) for r in rows]
...
def _get(db: Session, organization_id: uuid.UUID, execution_id: uuid.UUID) -> JobExecution:
    row = db.execute(
        select(JobExecution).where(
            JobExecution.id == execution_id, JobExecution.organization_id == organization_id
        )
    ).scalar_one_or_none()
    if row is None:
...
def _ser(row: JobExecution) -> dict[str, Any]:
    return {
        ...
        "jobReferenceId": str(row.job_reference_id) if row.job_reference_id else None,
