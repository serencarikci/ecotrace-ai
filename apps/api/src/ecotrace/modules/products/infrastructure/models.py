...
class Product(Base, UUIDPrimaryKeyMixin, TimestampMixin):
    ...
    __table_args__ = (
        ...
        Index("ix_products_product_category", "product_category"),
    ...
    recycled_content_percentage: Mapped[Decimal | None] = mapped_column(
...
class ProductVariant(Base, UUIDPrimaryKeyMixin, TimestampMixin):
    ...
    __table_args__ = (
        ...
        Index("ix_product_variants_organization_id", "organization_id"),
...
class ProductBatch(Base, UUIDPrimaryKeyMixin, TimestampMixin):
    ...
    __table_args__ = (
        UniqueConstraint("organization_id", "batch_code", name="uq_product_batches_org_code"),
        Index("ix_product_batches_organization_id", "organization_id"),
        Index("ix_product_batches_product_id", "product_id"),
        Index("ix_product_batches_facility_id", "facility_id"),
        Index("ix_product_batches_production_date", "production_date"),
        Index("ix_product_batches_status", "status"),
        Index("ix_product_batches_batch_code", "batch_code"),
    )
    organization_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True), ForeignKey("organizations.id", ondelete="CASCADE"), nullable=False
    )
...
class BillOfMaterials(Base, UUIDPrimaryKeyMixin, TimestampMixin):
    ...
    __table_args__ = (
        ...
        Index("ix_bills_of_materials_organization_id", "organization_id"),
        Index("ix_bills_of_materials_product_id", "product_id"),
        Index("ix_bills_of_materials_status", "status"),
        Index("ix_bills_of_materials_valid_from", "valid_from"),
        Index("ix_bills_of_materials_valid_to", "valid_to"),
        Index("ix_bills_of_materials_version", "version"),
    )
    organization_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True), ForeignKey("organizations.id", ondelete="CASCADE"), nullable=False
    )
...
class BillOfMaterialItem(Base, UUIDPrimaryKeyMixin, TimestampMixin):
    ...
    __table_args__ = (
        Index("ix_bom_items_bill_of_material_id", "bill_of_material_id"),
        Index("ix_bom_items_material_id", "material_id"),
        Index("ix_bom_items_component_product_id", "component_product_id"),
    )
    bill_of_material_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True), ForeignKey("bills_of_materials.id", ondelete="CASCADE"), nullable=False
    )
...
class ProductSustainabilityIndicator(Base, UUIDPrimaryKeyMixin, TimestampMixin):
    ...
    __table_args__ = (
        Index("ix_product_indicators_organization_id", "organization_id"),
        Index("ix_product_indicators_product_id", "product_id"),
        Index("ix_product_indicators_indicator_code", "indicator_code"),
        Index("ix_product_indicators_status", "status"),
    )
    organization_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True), ForeignKey("organizations.id", ondelete="CASCADE"), nullable=False
    )
