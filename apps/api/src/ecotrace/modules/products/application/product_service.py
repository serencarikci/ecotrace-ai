...
from ecotrace.modules.products.application.validators import (
    require_non_negative,
    require_percentage,
    require_positive,
    require_repairability,
)
...
from ecotrace.shared.application.org_access import (
    ensure_org_access,
    require_product_approve,
    require_product_manage,
...
def _validate_product_fields(
    ...
    *,
    ...
    unit_code: str,
    weight: Decimal | None,
    recyclability: Decimal | None,
    recycled: Decimal | None,
    repairability: int | None,
    country: str | None,
) -> None:
...
def list_products(
    ...
    *,
    ...
    if search:
        like = f"%{search.strip()}%"
        stmt = stmt.where(
            or_(Product.name.ilike(like), Product.code.ilike(like), Product.sku.ilike(like))
        )
    if product_type:
    ...
    rows = list(
        db.execute(
            stmt.order_by(Product.code.asc()).offset((page - 1) * page_size).limit(page_size)
        )
        .scalars()
        .all()
    )
    return paginate(
        [ProductResponse.model_validate(r) for r in rows],
        page=page,
        page_size=page_size,
...
def create_product(
    ...
    *,
    ...
    _validate_product_fields(
        ...
        unit_code=payload.default_unit_code,
        weight=payload.weight_value,
        recyclability=payload.recyclability_percentage,
        recycled=payload.recycled_content_percentage,
        repairability=payload.repairability_score,
        country=country,
    )
    if db.execute(
        select(Product.id).where(Product.organization_id == organization_id, Product.code == code)
    ).scalar_one_or_none():
    ...
    row = Product(
        ...
        weight_unit_code=payload.weight_unit_code,
        expected_lifetime_value=payload.expected_lifetime_value,
        expected_lifetime_unit=payload.expected_lifetime_unit,
        recyclability_percentage=payload.recyclability_percentage,
        recycled_content_percentage=payload.recycled_content_percentage,
        repairability_score=payload.repairability_score,
...
def update_product(
    ...
    *,
    ...
    _validate_product_fields(
        db,
        product_type=data.get("product_type", row.product_type),
        unit_code=data.get("default_unit_code", row.default_unit_code),
        weight=data.get("weight_value", row.weight_value),
        recyclability=data.get("recyclability_percentage", row.recyclability_percentage),
        recycled=data.get("recycled_content_percentage", row.recycled_content_percentage),
        repairability=data.get("repairability_score", row.repairability_score),
...
def get_product_detail(
    db: Session, user: User, organization_id: uuid.UUID, product_id: uuid.UUID
) -> ProductResponse:
    ensure_org_access(db, user, organization_id)
...
def list_variants(
    ...
    rows = list(
        db.execute(
            ...
            .order_by(ProductVariant.code.asc())
        )
        .scalars()
        .all()
    )
    ...
    *,
    ...
    if db.execute(
        select(ProductVariant.id).where(
            ProductVariant.product_id == product_id, ProductVariant.code == code
        )
    ).scalar_one_or_none():
    ...
    write_audit_log(
        ...
        ip_address=ip_address,
        user_agent=user_agent,
        metadata={"productId": str(product_id), "code": code},
    )
    db.commit()
    db.refresh(row)
...
def get_variant_detail(
    db: Session, user: User, organization_id: uuid.UUID, variant_id: uuid.UUID
) -> VariantResponse:
    ensure_org_access(db, user, organization_id)
...
def list_batches(
    ...
    *,
    ...
    rows = list(
        db.execute(
            stmt.order_by(ProductBatch.batch_code.asc())
    ...
    return paginate(
        [BatchResponse.model_validate(r) for r in rows],
        page=page,
        page_size=page_size,
        total_items=int(total),
    )
    ...
    *,
    ...
    if db.execute(
        select(ProductBatch.id).where(
            ProductBatch.organization_id == organization_id, ProductBatch.batch_code == code
        )
    ).scalar_one_or_none():
    ...
    row = ProductBatch(
        ...
        weight_unit_code=payload.weight_unit_code,
        status=payload.status,
        traceability_reference=payload.traceability_reference,
        metadata_json=payload.metadata_json,
    )
    db.add(row)
    db.flush()
    ...
    *,
    ...
    if row.status in {"completed", "released"}:
        raise BusinessRuleError(
            "Completed and released batches are immutable except through controlled transitions."
        )
    data = payload.model_dump(exclude_unset=True)
    _validate_batch_refs(
        db,
        ...
        variant_id=data.get("product_variant_id", row.product_variant_id),
        facility_id=data.get("facility_id", row.facility_id),
        production_line_id=data.get("production_line_id", row.production_line_id),
        production_date=data.get("production_date", row.production_date),
        expiration_date=data.get("expiration_date", row.expiration_date),
...
def transition_batch(
    ...
    *,
    ...
    if payload.status not in allowed:
        raise BusinessRuleError(
            f"Cannot transition batch from '{row.status}' to '{payload.status}'."
        )
    previous = row.status
...
def get_batch_detail(
    db: Session, user: User, organization_id: uuid.UUID, batch_id: uuid.UUID
) -> BatchResponse:
    ensure_org_access(db, user, organization_id)
    return BatchResponse.model_validate(get_batch(db, organization_id, batch_id))


def _component_cycle_exists(
    db: Session, organization_id: uuid.UUID, root_product_id: uuid.UUID, component_id: uuid.UUID
) -> bool:
    ...
    while stack:
        ...
        bom_ids = list(
            db.execute(
                select(BillOfMaterials.id).where(
                    ...
                    BillOfMaterials.product_id == current,
                    BillOfMaterials.status.in_(["approved", "draft", "under_review"]),
                )
            )
            .scalars()
            .all()
        ...
        comps = list(
            db.execute(
                select(BillOfMaterialItem.component_product_id).where(
                    BillOfMaterialItem.bill_of_material_id.in_(bom_ids),
                    BillOfMaterialItem.component_product_id.is_not(None),
                )
            )
            .scalars()
            .all()
...
def _validate_bom_item(
    db: Session, organization_id: uuid.UUID, product_id: uuid.UUID, item: BomItemCreate
) -> None:
    has_mat = item.material_id is not None
    has_comp = item.component_product_id is not None
    if has_mat == has_comp:
        raise ValidationAppError(
            "Each BOM item must reference either a material or a component product."
        )
...
def _bom_to_response(db: Session, bom: BillOfMaterials) -> BomResponse:
    items = list(
        db.execute(
            ...
            .where(BillOfMaterialItem.bill_of_material_id == bom.id)
            .order_by(BillOfMaterialItem.created_at.asc())
        )
        .scalars()
        .all()
    )
...
def create_bom(
    ...
    *,
    ...
    version = (
        db.execute(
            select(func.coalesce(func.max(BillOfMaterials.version), 0)).where(
                BillOfMaterials.product_id == product_id
            )
        ).scalar_one()
        + 1
    ...
    for item in payload.items:
        db.add(
            BillOfMaterialItem(
                ...
                source_country_code=item.source_country_code.upper()
                if item.source_country_code
                else None,
...
def update_bom(
    ...
    *,
    ...
    if items is not None:
        for item in items:
            _validate_bom_item(
                db, organization_id, bom.product_id, BomItemCreate.model_validate(item)
            )
        existing = list(
            db.execute(
                select(BillOfMaterialItem).where(BillOfMaterialItem.bill_of_material_id == bom.id)
            )
            .scalars()
            .all()
        )
        ...
        for item in items:
            ...
            db.add(
                BillOfMaterialItem(
                    ...
                    waste_percentage=model.waste_percentage,
                    recycled_content_percentage=model.recycled_content_percentage,
                    source_country_code=model.source_country_code.upper()
                    if model.source_country_code
                    else None,
                    transport_distance_km=model.transport_distance_km,
    ...
    *,
    ...
    if bom.valid_from:
        overlapping = list(
            db.execute(
                select(BillOfMaterials).where(
                    ...
                    BillOfMaterials.status == "approved",
                )
            )
            .scalars()
            .all()
        ...
        for other in overlapping:
            ...
            if a0 <= b1 and b0 <= a1:
                raise BusinessRuleError(
                    "Only one active approved BOM version may apply at a given date."
                )
    for other in (
        db.execute(
            select(BillOfMaterials).where(
...
def clone_bom_version(
    ...
    *,
    ...
    items = list(
        db.execute(
            select(BillOfMaterialItem).where(BillOfMaterialItem.bill_of_material_id == source.id)
        )
        .scalars()
        .all()
    )
    payload = BomCreate(
        ...
        items=[
            BomItemCreate(
                ...
                recycled_content_percentage=i.recycled_content_percentage,
                source_country_code=i.source_country_code,
                transport_distance_km=i.transport_distance_km,
    ...
    write_audit_log(
        ...
        ip_address=ip_address,
        user_agent=user_agent,
        metadata={"sourceBomId": str(source.id)},
    )
    db.commit()
    return result
...
def get_bom_detail(
    db: Session, user: User, organization_id: uuid.UUID, bom_id: uuid.UUID
) -> BomResponse:
    ensure_org_access(db, user, organization_id)
