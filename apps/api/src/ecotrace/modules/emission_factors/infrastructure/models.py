...
class GwpValue(Base, UUIDPrimaryKeyMixin, TimestampMixin):
    ...
    __table_args__ = (
        Index("ix_gwp_values_assessment_report_code", "assessment_report_code"),
        Index("ix_gwp_values_gas_code", "gas_code"),
        Index("ix_gwp_values_is_active", "is_active"),
        UniqueConstraint(
...
class EmissionFactorSource(Base, UUIDPrimaryKeyMixin, TimestampMixin):
    ...
    __table_args__ = (
        UniqueConstraint("code", name="uq_emission_factor_sources_code"),
        Index("ix_emission_factor_sources_is_active", "is_active"),
    )
...
class EmissionFactor(Base, UUIDPrimaryKeyMixin, TimestampMixin):
    ...
    __table_args__ = (
        UniqueConstraint("code", "version", name="uq_emission_factors_code_version"),
        CheckConstraint("factor_value IS NULL OR factor_value >= 0", name="factor_value_nonneg"),
        CheckConstraint("co2_factor IS NULL OR co2_factor >= 0", name="co2_factor_nonneg"),
        CheckConstraint("ch4_factor IS NULL OR ch4_factor >= 0", name="ch4_factor_nonneg"),
        CheckConstraint("n2o_factor IS NULL OR n2o_factor >= 0", name="n2o_factor_nonneg"),
        CheckConstraint(
            "biogenic_co2_factor IS NULL OR biogenic_co2_factor >= 0",
            name="biogenic_co2_factor_nonneg",
        ),
        CheckConstraint(
            "uncertainty_percentage IS NULL OR "
            "(uncertainty_percentage >= 0 AND uncertainty_percentage <= 100)",
            name="uncertainty_pct_range",
        ),
        CheckConstraint(
            "valid_to IS NULL OR valid_from IS NULL OR valid_to >= valid_from",
            name="factor_valid_range",
        ),
        Index("ix_emission_factors_source_id", "source_id"),
        Index("ix_emission_factors_activity_type_id", "activity_type_id"),
        Index("ix_emission_factors_scope", "scope"),
        Index("ix_emission_factors_category", "category"),
        Index("ix_emission_factors_geography_code", "geography_code"),
        Index("ix_emission_factors_valid_from", "valid_from"),
        Index("ix_emission_factors_valid_to", "valid_to"),
        Index("ix_emission_factors_status", "status"),
        Index("ix_emission_factors_is_active", "is_active"),
    )
    ...
    supersedes_factor_id: Mapped[uuid.UUID | None] = mapped_column(
        UUID(as_uuid=True),
...
class OrganizationEmissionFactorPreference(Base, UUIDPrimaryKeyMixin, TimestampMixin):
    ...
    __table_args__ = (
        Index("ix_org_ef_pref_organization_id", "organization_id"),
        Index("ix_org_ef_pref_activity_type_id", "activity_type_id"),
        Index("ix_org_ef_pref_emission_factor_id", "emission_factor_id"),
        Index("ix_org_ef_pref_valid_from", "valid_from"),
        Index("ix_org_ef_pref_valid_to", "valid_to"),
        CheckConstraint(
            "valid_to IS NULL OR valid_from IS NULL OR valid_to >= valid_from",
