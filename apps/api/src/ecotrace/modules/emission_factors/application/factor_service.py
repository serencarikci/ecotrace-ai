...
from ecotrace.modules.carbon_accounting.application.matching_service import (
    find_overlapping_active_factors,
)
...
def _to_response(db: Session, row: EmissionFactor) -> FactorResponse:
    usage = db.execute(
        ...
        .where(CarbonCalculationItem.emission_factor_id == row.id)
    ).scalar_one()
...
def list_factors(
    ...
    *,
    ...
    rows = (
        db.execute(
            stmt.order_by(EmissionFactor.code, EmissionFactor.version.desc())
    ...
    return paginate(
        [_to_response(db, r) for r in rows], page=page, page_size=page_size, total_items=total
    )


def and_valid_on(valid_on: date) -> Any:
    return (EmissionFactor.valid_from.is_(None) | (EmissionFactor.valid_from <= valid_on)) & (
        EmissionFactor.valid_to.is_(None) | (EmissionFactor.valid_to >= valid_on)
    )
    ...
    *,
    ...
    existing = db.execute(
        select(EmissionFactor).where(
            EmissionFactor.code == code, EmissionFactor.version == payload.version
        )
    ).scalar_one_or_none()
    if existing:
    ...
    row = EmissionFactor(
        ...
        geography_code=(payload.geography_code or "GLOBAL").upper(),
        facility_type=payload.facility_type,
        technology_code=payload.technology_code,
        ...
        other_gases_json=payload.other_gases_json,
        biogenic_co2_factor=payload.biogenic_co2_factor,
        ...
        or {"disclaimer": "Demo/reference data — not for regulatory reporting."},
    )
    db.add(row)
    db.flush()
    write_audit_log(
        db,
        action="factor.draft_created",
...
def update_draft(
    ...
    *,
    ...
    merged = {
        "factor_value": data.get("factor_value", row.factor_value),
        "co2_factor": data.get("co2_factor", row.co2_factor),
        "ch4_factor": data.get("ch4_factor", row.ch4_factor),
        "n2o_factor": data.get("n2o_factor", row.n2o_factor),
        "biogenic_co2_factor": data.get("biogenic_co2_factor", row.biogenic_co2_factor),
        "uncertainty_percentage": data.get("uncertainty_percentage", row.uncertainty_percentage),
...
def activate_factor(
    ...
    *,
    ...
    overlaps = find_overlapping_active_factors(
        db,
    ...
    if supersede_previous:
        previous = db.execute(
            select(EmissionFactor).where(
                EmissionFactor.code == row.code,
                EmissionFactor.status == "active",
                EmissionFactor.id != row.id,
            )
        ).scalar_one_or_none()
        if previous:
    ...
    if overlaps:
        raise ConflictError(
            "Activating this factor would create an ambiguous overlap with other active factors.",
            details=[
                {"factorId": str(o.id), "code": o.code, "version": o.version} for o in overlaps
            ],
        )
    row.status = "active"
    row.is_active = True
    write_audit_log(
        ...
        metadata={
            "code": row.code,
            "version": row.version,
            "supersededId": str(previous.id) if previous else None,
        },
    )
    db.commit()
    db.refresh(row)
...
def clone_version(
    ...
    *,
    ...
    max_version = db.execute(
        select(func.max(EmissionFactor.version)).where(EmissionFactor.code == source.code)
    ).scalar_one()
    ...
    row = EmissionFactor(
        ...
        technology_code=source.technology_code,
        fuel_type=source.fuel_type,
        transportation_mode=source.transportation_mode,
        vehicle_type=source.vehicle_type,
        unit_code=source.unit_code,
        factor_value=source.factor_value,
        co2_factor=source.co2_factor,
        ch4_factor=source.ch4_factor,
        n2o_factor=source.n2o_factor,
        other_gases_json=source.other_gases_json,
        biogenic_co2_factor=source.biogenic_co2_factor,
        uncertainty_percentage=source.uncertainty_percentage,
        ...
        supersedes_factor_id=source.id,
    ...
    write_audit_log(
        ...
        ip_address=ip_address,
        user_agent=user_agent,
        metadata={"fromId": str(source.id), "version": new_version},
    )
    db.commit()
    db.refresh(row)
    return _to_response(db, row)
...
def list_versions(db: Session, user: User, factor_id: uuid.UUID) -> list[FactorResponse]:
    ...
    rows = (
        db.execute(
            ...
            .where(EmissionFactor.code == row.code)
