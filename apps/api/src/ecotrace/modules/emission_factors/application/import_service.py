...
TEMPLATE_CSV = (
    "sourceCode,factorCode,factorName,activityTypeCode,scope,category,unitCode,"
    "factorValue,geographyCode,validFrom,validTo,version,subcategory,co2Factor,"
    "ch4Factor,n2oFactor,uncertaintyPercentage,methodology,metadata\n"
    "DEMO_GRID,EF-ELEC-TR-DEMO,Demo TR Electricity,purchased_electricity,scope_2,"
    "purchased_electricity,kWh,0.442,TR,2024-01-01,2024-12-31,1,,,,,\n"
)
    ...
    *,
    ...
    if missing:
        raise ValidationAppError(
            ...
            details=[{"field": c, "message": "Required"} for c in missing],
        )
    ...
    units = {
        u.code: u for u in db.execute(select(Unit).where(Unit.is_active.is_(True))).scalars().all()
    }
    ...
    for idx, row in enumerate(reader, start=2):
        ...
        if unit_code not in units:
            ...
            activity_code in activity_types
            and units[unit_code].dimension != activity_types[activity_code].allowed_unit_dimension
        ):
        ...
        existing = db.execute(
            select(EmissionFactor).where(
                EmissionFactor.code == factor_code, EmissionFactor.version == version
            )
        ).scalar_one_or_none()
        if existing:
        ...
        valid_rows.append(
            {
                ...
                "code": factor_code,
                "name": (row.get("factorName") or "").strip(),
                "activity_type_id": activity_types[activity_code].id,
                ...
                "ch4_factor": ch4,
                "n2o_factor": n2o,
                "uncertainty_percentage": unc,
                "geography_code": ((row.get("geographyCode") or "GLOBAL").strip().upper()),
                ...
                "metadata_json": {
                    ...
                    "disclaimer": "Demo/reference data — not for regulatory reporting.",
                    "rawMetadata": (row.get("metadata") or "").strip() or None,
                },
            }
        )

    job = EmissionFactorImportJob(
    ...
    if execute:
            ...
            db.add(
                EmissionFactor(
                    **payload,
                    ...
                    is_active=False,
                    is_demo=True,
                    description="Imported draft factor (demo).",
                )
            )
            created += 1
        job.status = "executed"
        ...
        write_audit_log(
            db,
            action="factor.import_executed",
            actor_user_id=user.id,
            entity_type="emission_factor_import_job",
            entity_id=str(job.id),
            request_id=request_id,
            ip_address=ip_address,
            user_agent=user_agent,
            metadata={"created": created, "filename": filename},
        )
    ...
    return FactorImportResult(
        ...
        error_summary_json=job.error_summary_json,
    )
