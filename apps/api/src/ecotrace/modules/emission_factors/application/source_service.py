...
def list_sources(
    ...
    *,
    ...
    if search:
        ...
        filt = or_(
            EmissionFactorSource.code.ilike(pattern),
            EmissionFactorSource.name.ilike(pattern),
            EmissionFactorSource.publisher.ilike(pattern),
        )
    ...
    rows = (
        db.execute(
            stmt.order_by(EmissionFactorSource.code).offset((page - 1) * page_size).limit(page_size)
        )
        .scalars()
        .all()
    )
    return paginate(
        [_to_response(r) for r in rows], page=page, page_size=page_size, total_items=total
    )
...
def create_source(
    ...
    *,
    ...
    existing = db.execute(
        select(EmissionFactorSource).where(EmissionFactorSource.code == code)
    ).scalar_one_or_none()
    if existing:
    ...
    write_audit_log(
        db,
        action="factor_source.created",
        actor_user_id=user.id,
        entity_type="emission_factor_source",
