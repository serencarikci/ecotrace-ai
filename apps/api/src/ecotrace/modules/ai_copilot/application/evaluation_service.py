    ...
    *,
    ...
    for row in dataset:
        ...
        per_query.append(
            {
                "question": question,
                "precision": precision_at_k(relevant, retrieved, 5),
                "recall": recall_at_k(relevant, retrieved, 5),
                "mrr": mean_reciprocal_rank(relevant, retrieved),
                "ndcg": ndcg_at_k(relevant, retrieved, 5),
                "faithfulness": faithfulness_score(
                    answer=answer, evidence_texts=[e.content for e in evidence]
                ),
                "answerRelevance": recall_at_k(relevant, retrieved, 5),
                "citationAccuracy": citation_accuracy(
                    answer=" ".join(labels), citation_labels=labels
                ),
                "latencyMs": latency,
                "hallucinationRate": 0.0 if evidence else 1.0,
                "contextPrecision": precision_at_k(relevant, retrieved, 5),
            }
        )
    ...
    run = EvaluationRun(
        ...
        metrics_json={"aggregate": metrics, "perQuery": per_query},
        notes=notes,
    ...
    return [
        {
            ...
            "name": r.name,
            "status": r.status,
            "metrics": (r.metrics_json or {}).get("aggregate"),
