...
from ecotrace.modules.retrieval.application.hybrid_search import (
...
def list_conversations(
    ...
    *,
    ...
    stmt = select(ChatConversation).where(
        ...
        or_(ChatConversation.owner_user_id == user.id, ChatConversation.is_shared_org.is_(True)),
    )
    ...
    rows = db.execute(
        stmt.order_by(ChatConversation.is_pinned.desc(), ChatConversation.updated_at.desc())
        .offset((page - 1) * page_size)
        .limit(page_size)
    ).scalars()
    return paginate(
        [_serialize_conversation(c) for c in rows],
        page=page,
        page_size=page_size,
        total_items=total,
    )
...
def delete_conversation(
    db: Session, user: User, organization_id: uuid.UUID, conversation_id: uuid.UUID
) -> dict[str, Any]:
    require_ai_write(db, user, organization_id)
    conv = _get_owned_conversation(db, user, organization_id, conversation_id)
    conv.status = "deleted"
...
def export_conversation(
    ...
    return {
        ...
        "messages": list_messages(db, user, organization_id, conversation_id),
...
def create_folder(
    ...
    require_ai_write(db, user, organization_id)
    folder = ChatFolder(
        organization_id=organization_id, owner_user_id=user.id, name=name.strip()[:255]
    )
    db.add(folder)
    db.flush()
...
def chat(
    ...
    *,
    ...
    db.add(user_msg)
    db.flush()
    result = _generate_grounded_answer(
        db,
        user=user,
        organization_id=organization_id,
        conversation=conv,
    ...
    if stream:
        return _stream_and_persist(
            db, conversation=conv, organization_id=organization_id, language=language, result=result
        )
    assistant = _persist_assistant(db, conv=conv, organization_id=organization_id, result=result)
    db.flush()
    return {
        "conversationId": str(conv.id),
        "userMessage": _serialize_message(user_msg),
...
def _generate_grounded_answer(
    ...
    *,
    ...
    if settings.ai_enable_response_cache and cache_key in _RESPONSE_CACHE:
        ...
        return cached
    evidence, retrieval_metrics = hybrid_retrieve(
        db,
        user,
        organization_id,
        query=question,
        language=None if org_settings.language_code == "auto" else org_settings.language_code,
    )
        ...
        citations.append(
            {
                ...
                "databaseSource": block["databaseSource"],
    ...
    system_parts = [
        org_settings.system_prompt or _default_system_prompt(language),
        build_context_prompt(evidence, language=language),
    ]
    if tool_blocks:
    ...
    if not grounded:
        ...
        llm_result = provider.complete(
            ...
            max_tokens=org_settings.max_tokens,
            top_p=org_settings.top_p,
            language=language,
        )
    ...
    reasoning = {
        ...
        "tools": [t["action"] for t in tool_results],
        "grounded": grounded,
    }
    log = RetrievalLog(
        organization_id=organization_id,
        ...
        user_id=user.id,
        query_text=question,
        expanded_queries_json=retrieval_metrics.get("expandedQueries"),
        filters_json={"organizationId": str(organization_id)},
        candidate_chunk_ids=[e.chunk_id for e in evidence],
        reranked_chunk_ids=[e.chunk_id for e in evidence],
        retrieval_latency_ms=retrieval_metrics.get("retrievalLatencyMs"),
        embedding_latency_ms=retrieval_metrics.get("embeddingLatencyMs"),
        llm_latency_ms=llm_latency,
        prompt_tokens=llm_meta["promptTokens"],
        completion_tokens=llm_meta["completionTokens"],
        estimated_cost_usd=0.0,
        cache_hit=False,
        provider=llm_meta["provider"],
        tool_usage_json=tool_results,
    ...
    _upsert_memory(
        ...
        scope="short_term",
        key="last_question",
        value={"question": question, "language": language},
    )
    payload = {
        "content": content,
        "citations": citations,
        ...
        "promptTokens": llm_meta["promptTokens"],
        "completionTokens": llm_meta["completionTokens"],
        "latencyMs": llm_latency,
        "toolCalls": tool_results,
        "cacheHit": False,
        "retrievalLogId": str(log.id) if log.id else None,
    }
...
def _persist_assistant(
    db: Session, *, conv: ChatConversation, organization_id: uuid.UUID, result: dict[str, Any]
) -> dict[str, Any]:
    assistant = ChatMessage(
        ...
        metadata_json={"grounded": result["grounded"], "cacheHit": result.get("cacheHit", False)},
    )
    db.add(assistant)
    ...
    if result.get("retrievalLogId"):
        log = (
            db.get(RetrievalLog, uuid.UUID(result["retrievalLogId"]))
            if result["retrievalLogId"]
            else None
        )
        if log is None:
            logs = db.execute(
                select(RetrievalLog)
                .where(RetrievalLog.conversation_id == conv.id)
...
def _stream_and_persist(
    ...
    *,
    ...
    assistant = _persist_assistant(
        db, conv=conversation, organization_id=organization_id, result=result
    )
    meta = {
        "event": "meta",
...
def _get_or_create_ai_settings(db: Session, organization_id: uuid.UUID) -> OrganizationAiSettings:
    ...
    row = OrganizationAiSettings(
        organization_id=organization_id,
        preferred_provider=settings.ai_llm_provider,
        preferred_embedding_model=settings.ai_embedding_model,
        temperature=settings.ai_temperature,
        max_tokens=settings.ai_max_tokens,
        top_p=settings.ai_top_p,
        citation_mode=settings.ai_citation_mode,
        language_code="auto",
...
def update_ai_settings(
    ...
    mapping = {
        ...
        "preferredEmbeddingModel": "preferred_embedding_model",
...
def upsert_prompt_template(
    ...
    *,
    ...
    existing = list(
        db.execute(
            select(PromptTemplate).where(
                PromptTemplate.organization_id == organization_id, PromptTemplate.code == code
            )
        ).scalars()
    )
...
def _get_owned_conversation(
    ...
    conv = db.execute(
        select(ChatConversation).where(
            ...
            ChatConversation.status != "deleted",
        )
    ).scalar_one_or_none()
    if conv is None:
...
def _get_accessible_conversation(
    ...
    conv = db.execute(
        select(ChatConversation).where(
            ...
            ChatConversation.status != "deleted",
            or_(
                ChatConversation.owner_user_id == user.id, ChatConversation.is_shared_org.is_(True)
            ),
        )
    ).scalar_one_or_none()
    if conv is None:
...
def _serialize_conversation(conv: ChatConversation) -> dict[str, Any]:
    return {
        "id": str(conv.id),
        "organizationId": str(conv.organization_id),
        "title": conv.title,
        "status": conv.status,
        "languageCode": conv.language_code,
        "isPinned": conv.is_pinned,
        "isFavorite": conv.is_favorite,
        "isSharedOrg": conv.is_shared_org,
        "folderId": str(conv.folder_id) if conv.folder_id else None,
...
def _serialize_ai_settings(row: OrganizationAiSettings) -> dict[str, Any]:
    return {
        ...
        "preferredEmbeddingModel": row.preferred_embedding_model,
