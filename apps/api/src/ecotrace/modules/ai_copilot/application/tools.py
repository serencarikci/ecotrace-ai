...
def tool_evidence_blocks(tool_results: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for idx, item in enumerate(tool_results, start=1):
        ...
        blocks.append(
            {
                "label": f"T{idx}",
                "documentName": f"tool:{item['action']}",
                "documentId": None,
                "pageNumber": None,
                "chunkId": None,
                "databaseSource": "structured",
...
def _summarize_inventory(db: Session, organization_id: uuid.UUID, question: str) -> dict[str, Any]:
    ...
    inv = db.execute(
        ...
        .order_by(CarbonInventory.updated_at.desc())
        .limit(1)
    ).scalar_one_or_none()
    if inv is None:
...
def _highest_facility(db: Session, organization_id: uuid.UUID, question: str) -> dict[str, Any]:
    ...
    return {
        "summary": f"Organization has {count} facilities. Sample facilities: {names}. Detailed emission ranking requires an approved calculation run.",
        "recordId": str(facilities[0].id) if facilities else None,
    }
...
def _scope_breakdown(db: Session, organization_id: uuid.UUID, question: str) -> dict[str, Any]:
    ...
    return {
        "summary": f"{inv['summary']} Scope breakdown should be read from approved calculation analytics endpoints; chat tools only surface authorized structured summaries.",
        "recordId": inv.get("recordId"),
    }


def _summarize_product_footprint(
    db: Session, organization_id: uuid.UUID, question: str
) -> dict[str, Any]:
    _ = question
    row = db.execute(
        select(ProductCarbonFootprint)
        .where(ProductCarbonFootprint.organization_id == organization_id)
        .order_by(ProductCarbonFootprint.updated_at.desc())
        .limit(1)
    ).scalar_one_or_none()
    if row is None:
    ...
    return {
        "summary": f"Latest PCF status={row.status}, total_kg_co2e={value}.",
        "recordId": str(row.id),
    }
...
def _summarize_passport(db: Session, organization_id: uuid.UUID, question: str) -> dict[str, Any]:
    ...
    row = db.execute(
        ...
        .where(DigitalProductPassport.organization_id == organization_id)
        .order_by(DigitalProductPassport.updated_at.desc())
        .limit(1)
    ).scalar_one_or_none()
    if row is None:
        return {"summary": "No digital product passport found.", "recordId": None}
    return {
        "summary": f"Passport '{row.title}' code={row.passport_code} status={row.status} public_slug={row.public_slug}.",
        "recordId": str(row.id),
        "url": f"/passport/{row.public_slug}",
    }
...
def _target_progress(db: Session, organization_id: uuid.UUID, question: str) -> dict[str, Any]:
    ...
    return {
        "summary": "Targets: " + ("; ".join(parts) if parts else "none"),
        "recordId": str(rows[0].id) if rows else None,
    }
...
def _sustainability_summary(
    ...
    return {
        "summary": f"{inv['summary']} | {targets['summary']} | {passport['summary']}",
        "recordId": inv.get("recordId"),
    }
...
def _find_related_documents(
    ...
    top = scored[:5]
    summary = (
        "; ".join((f"{d.title} ({d.document_type})" for _, d in top)) or "No related documents"
    )
...
def _compare_inventories(db: Session, organization_id: uuid.UUID, question: str) -> dict[str, Any]:
    ...
    a, b = (rows[0], rows[1])
    return {
        "summary": f"Comparing '{a.name}' ({a.status}) vs '{b.name}' ({b.status}).",
        "recordId": str(a.id),
    }
...
_HANDLERS = {
    "summarize_inventory": _summarize_inventory,
    "compare_inventories": _compare_inventories,
    "explain_emission_increase": _explain_increase,
    "highest_emitting_facility": _highest_facility,
    "explain_scope_breakdown": _scope_breakdown,
    "summarize_product_footprint": _summarize_product_footprint,
    "summarize_passport": _summarize_passport,
    "compare_products": _compare_products,
    "compare_scenarios": _compare_scenarios,
    "explain_target_progress": _target_progress,
    "generate_sustainability_summary": _sustainability_summary,
    "find_related_documents": _find_related_documents,
    "locate_evidence": _locate_evidence,
}
