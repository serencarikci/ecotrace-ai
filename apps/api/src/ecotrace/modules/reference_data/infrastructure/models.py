...
class Unit(Base, UUIDPrimaryKeyMixin, TimestampMixin):
    __tablename__ = 'units'
    __table_args__ = (UniqueConstraint('code', name='uq_units_code'), CheckConstraint('conversion_factor_to_base > 0', name='conversion_positive'))
...
class ActivityType(Base, UUIDPrimaryKeyMixin, TimestampMixin):
    ...
    requires_facility: Mapped[bool] = mapped_column(Boolean, nullable=False, default=True, server_default='true')
    requires_equipment: Mapped[bool] = mapped_column(Boolean, nullable=False, default=False, server_default='false')
    is_active: Mapped[bool] = mapped_column(Boolean, nullable=False, default=True, server_default='true')
