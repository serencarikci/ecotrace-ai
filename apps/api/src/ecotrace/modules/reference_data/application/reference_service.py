...
def _validate_unit_fields(
    *, dimension: str, conversion_factor: Decimal, decimal_precision: int
) -> None:
    if dimension not in UNIT_DIMENSIONS:
        raise ValidationAppError(
            "Invalid unit dimension.",
            details=[{"field": "dimension", "message": "Unknown dimension code."}],
        )
...
def _validate_activity_type_fields(
    db: Session, *, category: str, default_unit_code: str, allowed_unit_dimension: str
) -> None:
    if category not in ACTIVITY_CATEGORIES:
        raise ValidationAppError(
            "Invalid activity category.",
            details=[{"field": "category", "message": "Unknown category code."}],
        )
    if allowed_unit_dimension not in UNIT_DIMENSIONS:
        raise ValidationAppError("Invalid allowed unit dimension.")
    unit = db.execute(
        select(Unit).where(Unit.code == default_unit_code, Unit.is_active.is_(True))
    ).scalar_one_or_none()
    if unit is None:
        raise ValidationAppError(
            "Default unit not found or inactive.",
            details=[{"field": "defaultUnitCode", "message": "Unknown unit."}],
        )
...
def list_units(
    ...
    *,
    ...
    if search:
        like = f"%{search.strip()}%"
        stmt = stmt.where(
            or_(Unit.code.ilike(like), Unit.name.ilike(like), Unit.symbol.ilike(like))
        )
    total = db.execute(select(func.count()).select_from(stmt.subquery())).scalar_one()
    rows = list(
        db.execute(stmt.order_by(Unit.code.asc()).offset((page - 1) * page_size).limit(page_size))
        .scalars()
        .all()
    )
    return paginate(
        [UnitResponse.model_validate(r) for r in rows],
        page=page,
        page_size=page_size,
...
def create_unit(
    ...
    *,
    ...
    _validate_unit_fields(
        ...
        conversion_factor=payload.conversion_factor_to_base,
    ...
    unit = Unit(
        ...
        base_unit_code=payload.base_unit_code.strip(),
        decimal_precision=payload.decimal_precision,
...
def update_unit(
    ...
    *,
    ...
    _validate_unit_fields(
        dimension=dimension, conversion_factor=factor, decimal_precision=precision
    )
...
def list_activity_types(
    ...
    *,
    ...
    rows = list(
        db.execute(
            stmt.order_by(ActivityType.code.asc()).offset((page - 1) * page_size).limit(page_size)
        )
        .scalars()
        .all()
    )
    return paginate(
        [ActivityTypeResponse.model_validate(r) for r in rows],
        page=page,
        page_size=page_size,
        total_items=int(total),
    )
...
def create_activity_type(
    ...
    *,
    ...
    _validate_activity_type_fields(
        db,
        category=payload.category,
        default_unit_code=payload.default_unit_code.strip(),
        allowed_unit_dimension=payload.allowed_unit_dimension,
    )
    exists = db.execute(
        select(ActivityType.id).where(ActivityType.code == code)
    ).scalar_one_or_none()
    if exists:
    ...
    activity_type = ActivityType(
        ...
        default_unit_code=payload.default_unit_code.strip(),
        allowed_unit_dimension=payload.allowed_unit_dimension,
        expected_value_type=payload.expected_value_type,
        data_frequency=payload.data_frequency,
        requires_facility=payload.requires_facility,
    ...
    write_audit_log(
        ...
        entity_id=str(activity_type.id),
        request_id=request_id,
        ip_address=ip_address,
        user_agent=user_agent,
        metadata={"code": activity_type.code},
    )
    db.commit()
...
def update_activity_type(
    ...
    *,
    ...
    _validate_activity_type_fields(
        db, category=category, default_unit_code=default_unit, allowed_unit_dimension=dimension
    )
    for key, value in data.items():
