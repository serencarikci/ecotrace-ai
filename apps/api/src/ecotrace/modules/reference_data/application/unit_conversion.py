...
def get_unit(db: Session, code: str) -> Unit:
    ...
    if unit is None:
        raise ValidationAppError(
            "Unknown or inactive unit.",
            details=[{"field": "unitCode", "message": f"Unit '{code}' is not valid."}],
        )
    return unit


def convert_to_base(quantity: Decimal, unit: Unit) -> Decimal:
    return (quantity * unit.conversion_factor_to_base).quantize(
...
def normalize_quantity(
    db: Session, *, quantity: Decimal, unit_code: str, activity_type: ActivityType
) -> tuple[Decimal, str]:
    unit = get_unit(db, unit_code)
    if unit.dimension != activity_type.allowed_unit_dimension:
        raise ValidationAppError(
            "Unit is not compatible with the activity type.",
            details=[
                {
                    "field": "unitCode",
                    "message": f"Unit dimension '{unit.dimension}' does not match '{activity_type.allowed_unit_dimension}'.",
                }
            ],
        )
