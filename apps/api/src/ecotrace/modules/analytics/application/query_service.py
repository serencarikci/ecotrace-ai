...
from ecotrace.core.analytics_constants import (
    AVAIL_AVAILABLE,
    AVAIL_UNAVAILABLE,
    PROGRESS_ACHIEVED,
    PROGRESS_AT_RISK,
    PROGRESS_AT_RISK_TOLERANCE,
    PROGRESS_MISSED,
...
def _trend(current: Decimal | None, previous: Decimal | None) -> dict[str, Any]:
    if current is None or previous is None:
        return {
            "currentValue": _d(current),
            "comparisonValue": _d(previous),
            "absoluteChange": None,
            "percentageChange": None,
            "trendDirection": TREND_UNAVAILABLE,
    ...
    return {
        ...
        "absoluteChange": str(change),
        "percentageChange": None if pct is None else str(pct),
...
def resolve_inventory(
    ...
    *,
    ...
    if inventory_id is not None:
        ...
        if (
            allow_provisional
            and inv.status not in {"approved", "calculated", "in_review"}
            and (inv.latest_run_id is None)
        ):
            raise ValidationAppError("Inventory has no completed calculation.")
        return inv
    stmt = (
        select(CarbonInventory)
        .where(
            CarbonInventory.organization_id == organization_id, CarbonInventory.status == "approved"
        )
        .order_by(CarbonInventory.approved_at.desc().nullslast(), CarbonInventory.created_at.desc())
        .limit(1)
    )
    ...
    if inv is None and allow_provisional:
        stmt = (
            ...
            .where(
                ...
                CarbonInventory.latest_run_id.is_not(None),
            )
            .order_by(CarbonInventory.calculated_at.desc().nullslast())
            .limit(1)
        )
        inv = db.execute(stmt).scalar_one_or_none()
    if inv is None:
...
def inventory_metadata(db: Session, inventory: CarbonInventory) -> dict[str, Any]:
    ...
    return {
        "organizationId": str(inventory.organization_id),
        "inventoryId": str(inventory.id),
        "inventoryName": inventory.name,
        "inventoryStatus": inventory.status,
        "reportingPeriodId": str(inventory.reporting_period_id),
        "reportingPeriodCode": period.code if period else None,
        "calculationRunId": str(inventory.latest_run_id) if inventory.latest_run_id else None,
        "calculatedAt": inventory.calculated_at.isoformat() if inventory.calculated_at else None,
        "methodologyVersion": inventory.calculation_methodology_version,
        "gwpDataset": inventory.gwp_dataset_code,
        "provisional": inventory.status != "approved",
        "engineVersion": run.engine_version if run else None,
    }
    ...
    return list(
        db.execute(
            select(CarbonCalculationItem).where(
                ...
                CarbonCalculationItem.status == "calculated",
                CarbonCalculationItem.total_kg_co2e.is_not(None),
            )
        )
        .scalars()
        .all()
...
def aggregate_items(
    db: Session, items: list[CarbonCalculationItem], facility_id: uuid.UUID | None = None
) -> dict[str, Any]:
    if facility_id is not None:
        ...
        activities = (
            {
                a.id: a
                for a in db.execute(
                    select(ActivityRecord).where(ActivityRecord.id.in_(activity_ids))
                )
                .scalars()
                .all()
            }
        ...
        items = [
            i
            for i in items
            if activities.get(i.activity_record_id)
            and activities[i.activity_record_id].facility_id == facility_id
        ]
    ...
    def ranked(mapping: dict[str, Decimal]) -> list[dict[str, Any]]:
        ...
        return [
            {
                "name": name,
                "totalKgCo2e": str(value),
                "totalTCo2e": str(kg_to_tonnes(value)),
                "sharePercentage": str(value / total * Decimal("100") if total else Decimal("0")),
            }
    ...
    return {
        ...
        "scopeTotals": {
            "scope1KgCo2e": str(scopes["scope_1"]),
            "scope2KgCo2e": str(scopes["scope_2"]),
            "scope3KgCo2e": str(scopes["scope_3"]),
            "scope1TCo2e": str(kg_to_tonnes(scopes["scope_1"])),
            "scope2TCo2e": str(kg_to_tonnes(scopes["scope_2"])),
            "scope3TCo2e": str(kg_to_tonnes(scopes["scope_3"])),
        },
        "categoryTotals": ranked(categories),
        "facilityTotals": ranked(facilities),
        "activityTypeTotals": ranked(activity_types),
        "factorSourceTotals": ranked(sources),
        "greenhouseGasTotals": {k: str(v) for k, v in gases.items()},
        "largestEmissionSource": largest_source,
        "highestEmittingFacility": highest_facility,
        "itemCount": len(items),
    }
...
def dashboard(
    ...
    *,
    ...
    comparison_inventory_id: uuid.UUID | None = None,
    allow_provisional: bool = False,
) -> dict[str, Any]:
    if allow_provisional:
        require_period_manage(db, user, organization_id)
    inventory = resolve_inventory(
        db, user, organization_id, inventory_id, allow_provisional=allow_provisional
    )
    ...
    if comparison_inventory_id:
        try:
            other = resolve_inventory(
                db,
                user,
                organization_id,
                comparison_inventory_id,
    ...
    approved_activity_count = db.execute(
        select(func.count())
        .select_from(ActivityRecord)
        .where(
            ActivityRecord.organization_id == organization_id, ActivityRecord.status == "approved"
        )
    ).scalar_one()
    error_count = 0
    if inventory.latest_run_id:
        error_count = db.execute(
            ...
            .where(
                CarbonCalculationItem.calculation_run_id == inventory.latest_run_id,
                CarbonCalculationItem.status == "failed",
            )
        ).scalar_one()
    return {
        "metadata": inventory_metadata(db, inventory),
        "summary": {
            "totalEmissionsKgCo2e": agg["totalKgCo2e"],
            "totalEmissionsTCo2e": agg["totalTCo2e"],
            "scope1KgCo2e": agg["scopeTotals"]["scope1KgCo2e"],
            "scope2KgCo2e": agg["scopeTotals"]["scope2KgCo2e"],
            "scope3KgCo2e": agg["scopeTotals"]["scope3KgCo2e"],
            "approvedActivityRecordCount": int(approved_activity_count),
            "calculationErrorCount": int(error_count),
            "largestEmissionSource": agg["largestEmissionSource"],
            "highestEmittingFacility": agg["highestEmittingFacility"],
        },
        "scopeDistribution": agg["scopeTotals"],
        "categoryDistribution": agg["categoryTotals"],
        "facilityTotals": agg["facilityTotals"],
        "activityTypeTotals": agg["activityTypeTotals"],
        "greenhouseGasTotals": agg["greenhouseGasTotals"],
        "comparison": comparison,
        "empty": agg["itemCount"] == 0,
    }
...
def breakdown(
    ...
    *,
    ...
    mapping = {
        "scopes": [
            {"name": "scope_1", "totalKgCo2e": agg["scopeTotals"]["scope1KgCo2e"]},
            {"name": "scope_2", "totalKgCo2e": agg["scopeTotals"]["scope2KgCo2e"]},
            {"name": "scope_3", "totalKgCo2e": agg["scopeTotals"]["scope3KgCo2e"]},
        ],
        "categories": agg["categoryTotals"],
        "facilities": agg["facilityTotals"],
        "activity-types": agg["activityTypeTotals"],
        "gases": [{"name": k, "totalKgCo2e": v} for k, v in agg["greenhouseGasTotals"].items()],
        "factor-sources": agg["factorSourceTotals"],
    }
...
def monthly_trends(
    ...
    *,
    ...
    for item in items:
        ...
        scope_buckets.setdefault(
            key, {"scope_1": Decimal("0"), "scope_2": Decimal("0"), "scope_3": Decimal("0")}
        )
        if item.scope in scope_buckets[key]:
            scope_buckets[key][item.scope] += kg
    points = []
    ordered = sorted(k for k in buckets if k != "unknown") + (
    ...
    for key in ordered:
        ...
        points.append(
            {
                "period": key,
                "totalKgCo2e": str(current),
                "scope1KgCo2e": str(scope_buckets[key]["scope_1"]),
                "scope2KgCo2e": str(scope_buckets[key]["scope_2"]),
                "scope3KgCo2e": str(scope_buckets[key]["scope_3"]),
                "comparison": _trend(current, prev),
            }
        )
        prev = current
    ...
    *,
    ...
    if current_value is None:
        return {
            ...
            "elapsedTimePercentage": None if elapsed_ratio is None else str(elapsed_ratio * 100),
            "status": PROGRESS_UNAVAILABLE,
            "expectedTrajectory": None,
            "actualTrajectory": None,
            "projectedCompletionStatus": PROGRESS_UNAVAILABLE,
        }
    span = (
        baseline_value - target_value if direction == "decrease" else target_value - baseline_value
    )
    moved = (
    ...
    remaining = (
        target_value - current_value if direction == "increase" else current_value - target_value
    )
    achieved = (
        current_value <= target_value if direction == "decrease" else current_value >= target_value
    )
    if achieved:
    ...
    return {
        ...
        "targetValue": str(target_value),
        "progressPercentage": str(progress),
        "remainingAmount": str(remaining),
        "elapsedTimePercentage": None
        if elapsed_ratio is None
        else str(elapsed_ratio * Decimal("100")),
        "status": status,
        "expectedTrajectory": None
        if elapsed_ratio is None
        else str(elapsed_ratio * Decimal("100")),
        "actualTrajectory": str(progress),
        "projectedCompletionStatus": status,
    }
    ...
    *,
    ...
    if agg["highestEmittingFacility"] and total > 0:
        ...
        if share >= Decimal("40"):
            recs.append(
                {
                    "code": "facility_concentration",
                    "title": "Investigate high-emitting facility",
                    "description": f"{agg['highestEmittingFacility']['name']} contributes {agg['highestEmittingFacility']['sharePercentage']}% of inventory emissions.",
                    "recommendationType": "facility_investigation",
                    "priority": "high" if share >= Decimal("50") else "medium",
                    "evidence": agg["highestEmittingFacility"],
                    "affectedFacility": agg["highestEmittingFacility"]["name"],
                    "affectedActivityType": None,
                    "estimatedReduction": None,
                    "sourceMetric": "facilityShare",
                    "generatedAt": inventory.calculated_at.isoformat()
                    if inventory.calculated_at
                    else None,
                }
            )
    ...
    if total > 0 and scope2 / total >= Decimal("0.30"):
        recs.append(
            {
                "code": "scope2_share",
                "title": "Review renewable electricity scenario",
                "description": "Scope 2 share exceeds 30% of total emissions in the selected inventory.",
                "recommendationType": "renewable_electricity_scenario",
                "priority": "medium",
                "evidence": {"scope2KgCo2e": str(scope2), "totalKgCo2e": str(total)},
                "affectedFacility": None,
                "affectedActivityType": None,
                "estimatedReduction": None,
                "sourceMetric": "scope2Share",
                "generatedAt": inventory.calculated_at.isoformat()
                if inventory.calculated_at
                else None,
            }
        )
    if agg["largestEmissionSource"] and agg["largestEmissionSource"]["name"] in {
        "purchased_electricity",
        "stationary_combustion",
    }:
        recs.append(
            {
                "code": "largest_source_efficiency",
                "title": "Prioritize efficiency on the largest emission source",
                "description": f"Largest category is {agg['largestEmissionSource']['name']} ({agg['largestEmissionSource']['sharePercentage']}%).",
                "recommendationType": "efficiency_review",
                "priority": "high",
                "evidence": agg["largestEmissionSource"],
                "affectedFacility": None,
                "affectedActivityType": None,
                "estimatedReduction": None,
                "sourceMetric": "categoryShare",
                "generatedAt": inventory.calculated_at.isoformat()
                if inventory.calculated_at
                else None,
            }
    ...
    active_targets = (
        db.execute(
            select(SustainabilityTarget).where(
                SustainabilityTarget.organization_id == organization_id,
                SustainabilityTarget.status == "active",
            )
        )
        .scalars()
        .all()
    ...
    for target in active_targets:
        progress = compute_target_progress(
            ...
            Decimal(agg["totalKgCo2e"])
            if target.target_type == "absolute_emission_reduction"
            else None,
            target.target_value,
            direction=target.target_direction,
            elapsed_ratio=Decimal("0.5"),
        )
        if progress["status"] in {PROGRESS_OFF_TRACK, PROGRESS_AT_RISK, PROGRESS_MISSED}:
            recs.append(
                {
                    "code": f"target_{target.code}",
                    "title": f"Review target {target.name}",
                    "description": f"Target progress status is {progress['status']}.",
                    "recommendationType": "target_review",
                    "priority": "critical" if progress["status"] == PROGRESS_OFF_TRACK else "high",
                    "evidence": progress,
                    "affectedFacility": None,
                    "affectedActivityType": None,
                    "estimatedReduction": None,
                    "sourceMetric": "targetProgress",
                    "generatedAt": inventory.calculated_at.isoformat()
                    if inventory.calculated_at
                    else None,
                }
...
def intensity_results(
    ...
    *,
    ...
    from ecotrace.modules.sustainability_targets.infrastructure.models import (
        IntensityMetricDefinition,
    )
    ...
    defs = (
        db.execute(
            select(IntensityMetricDefinition).where(
                IntensityMetricDefinition.organization_id == organization_id,
                IntensityMetricDefinition.is_active.is_(True),
            )
        )
        .scalars()
        .all()
    ...
    for definition in defs:
        ...
        denom_stmt = select(func.coalesce(func.sum(ActivityRecord.quantity), 0)).where(
        ...
        if definition.denominator_activity_type_id:
            denom_stmt = denom_stmt.where(
                ActivityRecord.activity_type_id == definition.denominator_activity_type_id
            )
            ...
            results.append(
                {
                    ...
                    "numeratorKgCo2e": str(numerator),
                    "denominator": str(denominator),
                    "intensityValue": None,
                    "availability": AVAIL_UNAVAILABLE,
                    "warning": "Denominator is zero; intensity cannot be calculated.",
                }
            )
            continue
        results.append(
            {
                ...
                "numeratorKgCo2e": str(numerator),
                "denominator": str(denominator),
                "intensityValue": str(numerator / denominator),
                "availability": AVAIL_AVAILABLE,
                "warning": None,
            }
        )
...
def kpi_results(
    ...
    *,
    ...
    defs = (
        db.execute(
            select(EnvironmentalKpiDefinition).where(
                EnvironmentalKpiDefinition.organization_id == organization_id,
                EnvironmentalKpiDefinition.is_active.is_(True),
            )
        )
        .scalars()
        .all()
    ...
    for definition in defs:
        ...
        if definition.kpi_type == "total_emissions":
            ...
            if intensity["items"]:
                value = (
                    Decimal(intensity["items"][0]["intensityValue"])
                    if intensity["items"][0]["intensityValue"] is not None
                    else None
                )
            ...
            stmt = select(func.coalesce(func.sum(ActivityRecord.quantity), 0)).where(
                ActivityRecord.organization_id == organization_id,
                ActivityRecord.status == "approved",
                ActivityRecord.is_archived.is_(False),
            )
        ...
        items.append(
            {
                ...
                "code": definition.code,
                "name": definition.name,
                "kpiType": definition.kpi_type,
                "unitCode": definition.unit_code,
                "targetDirection": definition.target_direction,
