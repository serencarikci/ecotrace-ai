...
class Settings(BaseSettings):
    ...
    cors_allowed_origins: Annotated[list[str], NoDecode] = Field(
        default_factory=lambda: ["http://localhost:4200"], alias="CORS_ALLOWED_ORIGINS"
    )
    ...
    scheduler_misfire_grace_seconds: int = Field(
        default=300, alias="SCHEDULER_MISFIRE_GRACE_SECONDS", ge=0
    )
    agent_max_tool_calls: int = Field(default=12, alias="AGENT_MAX_TOOL_CALLS", ge=1)
    agent_max_execution_seconds: int = Field(
        default=120, alias="AGENT_MAX_EXECUTION_SECONDS", ge=10
    )
    ...
    trusted_hosts: Annotated[list[str], NoDecode] = Field(
        default_factory=lambda: ["localhost", "127.0.0.1"], alias="TRUSTED_HOSTS"
    )
    ...
    retention_days_notifications: int = Field(
        default=90, alias="RETENTION_DAYS_NOTIFICATIONS", ge=1
    )
    retention_days_job_details: int = Field(default=180, alias="RETENTION_DAYS_JOB_DETAILS", ge=1)
    retention_days_agent_executions: int = Field(
        default=180, alias="RETENTION_DAYS_AGENT_EXECUTIONS", ge=1
    )
