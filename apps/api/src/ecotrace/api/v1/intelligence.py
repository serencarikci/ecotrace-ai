...
def get_execution(
    organization_id: UUID, execution_id: UUID, db: DbSession, user: CurrentUser
) -> Any:
...
def approve_action(
    ...
    result = agent_service.approve_action(
        db, user, organization_id, request_id, comment=payload.comment
    )
    db.commit()
    return result
...
def reject_action(
    ...
    result = agent_service.reject_action(
        db, user, organization_id, request_id, comment=payload.comment
    )
    db.commit()
    return result
...
def execute_action(
    organization_id: UUID, request_id: UUID, db: DbSession, user: CurrentUser
) -> Any:
...
def create_rule(
    organization_id: UUID, payload: AutomationCreate, db: DbSession, user: CurrentUser
) -> Any:
    result = automation_service.create_rule(
        db,
        user,
        organization_id,
        code=payload.code,
...
def patch_rule(
    organization_id: UUID, rule_id: UUID, payload: dict[str, Any], db: DbSession, user: CurrentUser
) -> Any:
...
def create_anomaly_rule(
    organization_id: UUID, payload: AnomalyRuleCreate, db: DbSession, user: CurrentUser
) -> Any:
    result = anomaly_service.create_rule(
        db, user, organization_id, payload.model_dump(by_alias=True)
    )
    db.commit()
    return result
...
def dismiss_anomaly(
    ...
    result = anomaly_service.dismiss(
        db, user, organization_id, anomaly_id, payload.reason or payload.notes or ""
    )
    db.commit()
    return result
...
def create_forecast(
    organization_id: UUID, payload: ForecastCreate, db: DbSession, user: CurrentUser
) -> Any:
    result = forecast_service.create_definition(
        db, user, organization_id, payload.model_dump(by_alias=True)
    )
    db.commit()
    return result
...
def get_forecast_def(
    organization_id: UUID, definition_id: UUID, db: DbSession, user: CurrentUser
) -> Any:
...
def run_forecast(
    ...
    result = forecast_service.run_forecast(
        db, user, organization_id, definition_id, backtest=backtest
    )
    db.commit()
    return result
...
def dq_assign(
    organization_id: UUID, issue_id: UUID, payload: AssignRequest, db: DbSession, user: CurrentUser
) -> Any:
...
def dq_resolve(
    organization_id: UUID, issue_id: UUID, payload: ResolveRequest, db: DbSession, user: CurrentUser
) -> Any:
...
def dq_dismiss(
    ...
    result = quality_service.dismiss(
        db, user, organization_id, issue_id, payload.reason or payload.notes or ""
    )
    db.commit()
    return result
...
def assign_alert(
    organization_id: UUID, alert_id: UUID, payload: AssignRequest, db: DbSession, user: CurrentUser
) -> Any:
...
def resolve_alert(
    organization_id: UUID, alert_id: UUID, payload: ResolveRequest, db: DbSession, user: CurrentUser
) -> Any:
...
def patch_prefs(
    organization_id: UUID, payload: dict[str, Any], db: DbSession, user: CurrentUser
) -> Any:
...
def create_sched(
    organization_id: UUID, payload: ScheduledReportCreate, db: DbSession, user: CurrentUser
) -> Any:
    result = report_service.create_scheduled(
        db, user, organization_id, payload.model_dump(by_alias=True)
    )
    db.commit()
    return result
...
def download_gen(
    organization_id: UUID, report_id: UUID, db: DbSession, user: CurrentUser
) -> Response:
...
def supplier_patch(
    ...
    result = supplier_monitoring_service.update_profile(
        db, user, organization_id, supplier_id, payload
    )
    db.commit()
    return result
...
def supplier_assess(
    organization_id: UUID, supplier_id: UUID, db: DbSession, user: CurrentUser
) -> Any:
...
def get_reg_assessment(
    organization_id: UUID, assessment_id: UUID, db: DbSession, user: CurrentUser
) -> Any:
...
def review_reg(
    organization_id: UUID,
    assessment_id: UUID,
    payload: ApplicabilityReview,
    db: DbSession,
    user: CurrentUser,
) -> Any:
    result = regulatory_service.review_assessment(
        db,
        user,
        organization_id,
        assessment_id,
