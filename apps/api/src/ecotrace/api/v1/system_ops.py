...
def system_health(db: DbSession, user: CurrentUser) -> dict[str, Any]:
    ...
    checks = {
        ...
        "storage": _storage_status(settings.attachment_storage_path),
        "reportStorage": _storage_status(settings.report_storage_path),
        "knowledgeStorage": _storage_status(settings.knowledge_storage_path),
        "scheduler": {"status": "configured", "enabled": settings.scheduler_enabled},
        "ai": {
            "status": "ok",
            "llmProvider": settings.ai_llm_provider,
            "embeddingProvider": settings.ai_embedding_provider,
        },
        "vectorSearch": {"status": "ok", "backend": settings.ai_vector_backend},
    }
    overall = (
        "ok"
        if all(
            isinstance(v, dict) and v.get("status") in {"ok", "configured"} for v in checks.values()
        )
        else "degraded"
    )
    return {
        ...
        "version": __version__,
        "checks": checks,
        "recentFailedJobs": job_service.recent_failures(db, user),
    }
...
def health_storage(user: CurrentUser) -> dict[str, Any]:
    ...
    settings = get_settings()
    return {
        "attachments": _storage_status(settings.attachment_storage_path),
        "reports": _storage_status(settings.report_storage_path),
        "knowledge": _storage_status(settings.knowledge_storage_path),
        "backups": _storage_status(settings.backup_storage_path),
    }
...
def health_scheduler(user: CurrentUser) -> dict[str, Any]:
    ...
    return {
        ...
        "pollSeconds": settings.scheduler_poll_seconds,
        "misfireGraceSeconds": settings.scheduler_misfire_grace_seconds,
    }
...
def health_ai(user: CurrentUser) -> dict[str, Any]:
    ...
    return {
        "status": "ok",
        "llmProvider": settings.ai_llm_provider,
        "llmModel": settings.ai_llm_model,
        "embeddingProvider": settings.ai_embedding_provider,
        "apiKeyConfigured": bool(settings.ai_llm_api_key),
    }
...
def health_vector(db: DbSession, user: CurrentUser) -> dict[str, Any]:
    ...
    try:
        ...
        ext_ok = False
    return {
        "status": "ok" if settings.ai_vector_backend else "unknown",
        "backend": settings.ai_vector_backend,
        "pgvectorExtension": ext_ok,
    }
...
def system_version(user: CurrentUser) -> VersionResponse:
    ...
    return VersionResponse(
        ...
        api_version="v1",
        calculation_engine_version="ecotrace-carbon-0.3.0",
        forecasting_engine_version=FORECAST_ENGINE_VERSION,
        anomaly_engine_version=ANOMALY_ENGINE_VERSION,
        rag_pipeline_version="ecotrace-rag-0.6.0",
        intelligence_engine_version=INTELLIGENCE_ENGINE_VERSION,
...
def _db_status(db: DbSession) -> dict[str, Any]:
    try:
        ...
        raise EcoTraceError(
            "Database health check failed.", code="SERVICE_UNAVAILABLE", status_code=503
        ) from exc
