...
def ai_chat_stream(
    organization_id: UUID, payload: ChatRequest, db: DbSession, user: CurrentUser
) -> StreamingResponse:
    token_iter = chat_service.chat(
        db,
        user,
        organization_id,
        message=payload.message,
...
def list_conversations(
    ...
    return chat_service.list_conversations(
        db, user, organization_id, page=page, page_size=page_size, q=q
    )
...
def delete_conversation(
    organization_id: UUID, conversation_id: UUID, db: DbSession, user: CurrentUser
) -> Any:
...
def create_folder(
    organization_id: UUID, payload: FolderCreate, db: DbSession, user: CurrentUser
) -> Any:
...
def feedback(
    organization_id: UUID, payload: FeedbackRequest, db: DbSession, user: CurrentUser
) -> Any:
...
def archive_document(
    organization_id: UUID, document_id: UUID, db: DbSession, user: CurrentUser
) -> Any:
...
def list_chunks(
    ...
    return document_service.list_chunks(
        db, user, organization_id, document_id=document_id, page=page, page_size=page_size
    )


@router.post("/search")
def search(organization_id: UUID, payload: SearchRequest, db: DbSession, user: CurrentUser) -> Any:
    return search_service.enterprise_search(
        db,
        user,
        organization_id,
        query=payload.query,
...
def search_autocomplete(
    organization_id: UUID, db: DbSession, user: CurrentUser, q: str = Query(..., min_length=1)
) -> Any:
...
def save_search(
    organization_id: UUID, payload: SavedSearchCreate, db: DbSession, user: CurrentUser
) -> Any:
...
def put_settings(
    organization_id: UUID, payload: AiSettingsUpdate, db: DbSession, user: CurrentUser
) -> Any:
    result = chat_service.update_ai_settings(
        db, user, organization_id, payload.model_dump(by_alias=True, exclude_none=True)
    )
    db.commit()
    return result
...
def upsert_prompt(
    organization_id: UUID, payload: PromptUpsert, db: DbSession, user: CurrentUser
) -> Any:
    result = chat_service.upsert_prompt_template(
        db,
        user,
        organization_id,
        code=payload.code,
...
def create_evaluation(
    organization_id: UUID, payload: EvaluationRequest, db: DbSession, user: CurrentUser
) -> Any:
    result = evaluation_service.run_evaluation(
        db, user, organization_id, name=payload.name, dataset=payload.dataset, notes=payload.notes
    )
    db.commit()
    return result
...
def retrieval_logs(
    organization_id: UUID, db: DbSession, user: CurrentUser, limit: int = Query(50, ge=1, le=200)
) -> Any:
    ...
    return [
        {
            ...
            "retrievalLatencyMs": r.retrieval_latency_ms,
            "embeddingLatencyMs": r.embedding_latency_ms,
...
def list_providers(organization_id: UUID, db: DbSession, user: CurrentUser) -> Any:
    ...
    rows = db.execute(
        select(ProviderConfig).where(
            ...
            | ProviderConfig.organization_id.is_(None)
        )
    ).scalars()
    return [
        {
...
def cost_dashboard(organization_id: UUID, db: DbSession, user: CurrentUser) -> Any:
    ...
    logs = db.execute(
        select(RetrievalLog).where(RetrievalLog.organization_id == organization_id)
    ).scalars()
    tokens = 0
    for row in logs:
