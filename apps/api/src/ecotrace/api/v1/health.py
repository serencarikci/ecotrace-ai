...
@health_router.get('/health', response_model=HealthResponse, summary='Liveness probe', description='Verifies that the API process is running.')
def health() -> HealthResponse:
    return HealthResponse(status='ok')

@health_router.get('/ready', response_model=ReadyResponse, summary='Readiness probe', description='Verifies database connectivity.')
def ready(db: DbSession) -> ReadyResponse:
    try:
        check_database_connectivity(db)
        return ReadyResponse(status='ready', database='ok')
    except Exception as exc:
        raise EcoTraceError('Database is not ready.', code='SERVICE_UNAVAILABLE', status_code=503) from exc

@meta_router.get('/meta', response_model=MetaResponse, summary='Application metadata')
def meta() -> MetaResponse:
    settings = get_settings()
    return MetaResponse(name=settings.app_name, version=settings.app_version, environment=settings.app_env, api_version='v1')
