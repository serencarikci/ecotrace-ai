...
from ecotrace.modules.emission_factors.application.source_service import (
    FactorSourceCreate,
...
def list_sources(
    ...
    return source_service.list_sources(
        db, user, page=page, page_size=page_size, active_only=active_only, search=search
    )
...
def update_source(
    ...
    return source_service.update_source(
        db, user, source_id, payload, request_id=request_id, ip_address=ip, user_agent=user_agent
    )
...
def archive_source(
    ...
    return source_service.archive_source(
        db, user, source_id, request_id=request_id, ip_address=ip, user_agent=user_agent
    )
