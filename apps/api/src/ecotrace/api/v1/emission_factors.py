...
from ecotrace.modules.emission_factors.application.factor_service import (
    FactorCreate,
    FactorResponse,
    FactorUpdate,
)
...
def list_factors(
    ...
    geography_code: str | None = Query(None, alias="geographyCode"),
    status: str | None = None,
    valid_on: date | None = Query(None, alias="validOn"),
...
def create_factor(
    ...
    return factor_service.create_draft(
        db, user, payload, request_id=request_id, ip_address=ip, user_agent=user_agent
    )
...
def download_template() -> PlainTextResponse:
    return PlainTextResponse(
        import_service.get_template(),
        media_type="text/csv",
        headers={"Content-Disposition": "attachment; filename=emission-factors-template.csv"},
    )
...
async def validate_import(
    ...
    return import_service.validate_and_import(
        db,
        user,
        filename=file.filename or "factors.csv",
        content=content,
        execute=False,
...
def update_factor(
    ...
    return factor_service.update_draft(
        db, user, factor_id, payload, request_id=request_id, ip_address=ip, user_agent=user_agent
    )
...
def activate_factor(
    ...
    return factor_service.activate_factor(
        db,
        user,
        factor_id,
        supersede_previous=payload.supersede_previous if payload else True,
        request_id=request_id,
        ip_address=ip,
        user_agent=user_agent,
    )
...
def supersede_factor(
    ...
    return factor_service.supersede_factor(
        db, user, factor_id, request_id=request_id, ip_address=ip, user_agent=user_agent
    )
...
def archive_factor(
    ...
    return factor_service.archive_factor(
        db, user, factor_id, request_id=request_id, ip_address=ip, user_agent=user_agent
    )
...
def clone_version(
    ...
    return factor_service.clone_version(
        db, user, factor_id, request_id=request_id, ip_address=ip, user_agent=user_agent
    )
