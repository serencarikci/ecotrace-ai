...
def _maybe_csv(
    report: dict[str, Any], report_type: str, fmt: str
...
def executive_report(
    ...
    report = report_service.executive_report(
...
def inventory_summary_report(
    ...
    report = report_service.inventory_summary_report(
        db, user, organization_id, inventory_id=inventory_id, allow_provisional=allow_provisional
    )
...
def target_progress_report(
    ...
    report = report_service.target_progress_report(
        db, user, organization_id, inventory_id=inventory_id, allow_provisional=allow_provisional
    )
...
def scenario_comparison_report(
    ...
    user: CurrentUser,
    format: Literal["json", "csv"] = Query("json"),
    scenario_id: UUID | None = Query(None, alias="scenarioId"),
) -> Any:
    report = report_service.scenario_comparison_report(
        db, user, organization_id, scenario_id=scenario_id
    )
