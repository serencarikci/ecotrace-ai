...
SEED_UNITS: list[dict[str, object]] = [
    {
        "code": "kWh",
        "name": "Kilowatt hour",
        "symbol": "kWh",
...
SEED_ACTIVITY_TYPES: list[dict[str, object]] = [
    {
    ...
    {
        "code": "generated_electricity",
        "name": "Generated electricity",
        "category": "electricity",
        "unit": "kWh",
        "dimension": "energy",
    },
    {
        "code": "natural_gas_consumption",
...
def upsert_user(
    db: Session, *, email: str, full_name: str, password: str, role: Role, is_verified: bool = True
) -> User:
...
def seed_demo_organization(db: Session) -> Organization:
    ...
    if org is None:
        org = Organization(
            ...
            name=DEMO_ORG_NAME,
            slug=DEMO_ORG_SLUG,
            legal_name="EcoTrace Demo Industries Ltd.",
            country_code="DE",
    ...
    for item in SEED_UNITS:
        ...
        if existing is None:
            existing = Unit(
                ...
                conversion_factor_to_base=Decimal(str(item["factor"])),
                base_unit_code=str(item["base"]),
                decimal_precision=int(str(item["precision"])),
                is_active=True,
            )
            db.add(existing)
            db.flush()
    ...
    for item in SEED_ACTIVITY_TYPES:
        ...
        if existing is None:
            existing = ActivityType(
                ...
                default_unit_code=str(item["unit"]),
                allowed_unit_dimension=str(item["dimension"]),
...
def _upsert_facility(
    ...
    *,
    ...
    existing = db.execute(
        select(Facility).where(Facility.organization_id == org.id, Facility.code == code)
    ).scalar_one_or_none()
    if existing is None:
        existing = Facility(
            ...
            code=code,
            name=name,
            description=f"Demo facility in {city}",
...
def _upsert_line(
    db: Session, org: Organization, facility: Facility, *, code: str, name: str
) -> ProductionLine:
    existing = db.execute(
        select(ProductionLine).where(
            ProductionLine.facility_id == facility.id, ProductionLine.code == code
        )
    ).scalar_one_or_none()
    if existing is None:
        existing = ProductionLine(
            ...
            code=code,
            name=name,
            production_category="general",
            capacity_value=Decimal("1000"),
            capacity_unit_code="unit",
            is_active=True,
        )
        db.add(existing)
        db.flush()
...
def _upsert_equipment(
    ...
    *,
    ...
    existing = db.execute(
        select(Equipment).where(Equipment.facility_id == facility.id, Equipment.code == code)
    ).scalar_one_or_none()
    if existing is None:
        existing = Equipment(
            organization_id=org.id,
...
def _upsert_data_source(
    ...
    *,
    ...
    existing = db.execute(
        select(DataSource).where(DataSource.organization_id == org.id, DataSource.code == code)
    ).scalar_one_or_none()
    if existing is None:
        existing = DataSource(
            organization_id=org.id,
    ...
    *,
    ...
    existing = db.execute(
        select(ReportingPeriod).where(
            ReportingPeriod.organization_id == org.id, ReportingPeriod.code == code
        )
    ).scalar_one_or_none()
    if existing is None:
...
def seed_ops(db: Session, org: Organization, analyst: User) -> None:
    ...
    manisa = _upsert_facility(
        db,
        org,
        code="MAN-WH",
        name="Manisa Logistics Warehouse",
        city="Manisa",
    ...
    meter = _upsert_equipment(
        ...
        equipment_type="electricity_meter",
    )
    boiler = _upsert_equipment(
        db, org, izmir, line_b, code="BL-01", name="Process Boiler", equipment_type="boiler"
    )
    _upsert_equipment(
        db, org, manisa, None, code="VH-01", name="Warehouse Forklift", equipment_type="vehicle"
    )
    manual_src = _upsert_data_source(
        db,
        org,
        izmir,
        meter,
    ...
    csv_src = _upsert_data_source(
        db, org, izmir, boiler, code="SRC-CSV", name="CSV utility import", source_type="csv_import"
    )
    _upsert_data_source(
        db,
        org,
    ...
    samples = [
        ...
        ("approved", Decimal("45.00"), "L", "diesel_consumption", period_q1, date(2024, 2, 5)),
        ("rejected", Decimal("12.5"), "kg", "hazardous_waste", period_jan, date(2024, 1, 28)),
    ]
    for status, quantity, unit_code, type_code, period, activity_date in samples:
        marker = f"seed:{type_code}:{status}"
        existing = db.execute(
            select(ActivityRecord).where(
                ActivityRecord.organization_id == org.id, ActivityRecord.source_reference == marker
            )
        ).scalar_one_or_none()
        ...
        if existing is None:
            ...
            existing = ActivityRecord(
                ...
                data_source_id=manual_src.id if status != "approved" else csv_src.id,
                ...
                description=f"Seed {status} sample",
                rejection_reason="Incomplete meter photo" if status == "rejected" else None,
...
def run_seed(db: Session | None = None) -> None:
    ...
    try:
        ...
        org_admin = upsert_user(
            session,
            email=settings.demo_org_admin_email,
            full_name="Demo Organization Admin",
            password=settings.demo_org_admin_password,
            role=roles[ROLE_ORGANIZATION_ADMIN],
        )
        analyst = upsert_user(
            session,
        ...
        viewer = upsert_user(
            ...
            role=roles[ROLE_VIEWER],
        )
        ensure_membership(session, organization=org, user=admin, role=roles[ROLE_SYSTEM_ADMIN])
        ensure_membership(
            session, organization=org, user=org_admin, role=roles[ROLE_ORGANIZATION_ADMIN]
        )
