        ...
        if existing is None:
            db.add(
                GwpValue(
                    ...
                    effective_to=None,
                    source_reference="EcoTrace AR5-demo reference dataset (illustrative values inspired by commonly cited AR5 100-year GWPs; not an official IPCC distribution).",
                    is_active=True,
                )
            )
            db.flush()
...
def seed_demo_inventory(
    db: Session, org: Organization, actor: User, factors: dict[str, EmissionFactor]
) -> None:
    period = db.execute(
        select(ReportingPeriod).where(
            ReportingPeriod.organization_id == org.id, ReportingPeriod.code == "2024-Q1"
        )
    ).scalar_one_or_none()
    if period is None:
    ...
    existing_run = db.execute(
        select(CarbonCalculationRun).where(
            CarbonCalculationRun.inventory_id == inventory.id, CarbonCalculationRun.run_number == 1
        )
    ).scalar_one_or_none()
    ...
    logger.info(
        "seed.inventory_run_created", calculated=calculated, failed=failed, total_kg=str(total)
    )
