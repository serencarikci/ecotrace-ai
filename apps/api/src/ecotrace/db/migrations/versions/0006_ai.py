...
def upgrade() -> None:
    ts = _ts()
    op.execute(
        "DO $$ BEGIN CREATE EXTENSION IF NOT EXISTS vector; EXCEPTION WHEN OTHERS THEN RAISE NOTICE 'vector extension unavailable; continuing with JSONB embeddings'; END $$;"
    )
    op.execute(
        "DO $$ BEGIN CREATE EXTENSION IF NOT EXISTS pg_trgm; EXCEPTION WHEN OTHERS THEN RAISE NOTICE 'pg_trgm extension unavailable'; END $$;"
    )
    op.create_table(
        "knowledge_documents",
        ...
        *ts,
        ...
        sa.UniqueConstraint("organization_id", "slug", name="uq_knowledge_documents_org_slug"),
    )
    op.create_index(
    ...
    op.create_index(
        "ix_knowledge_documents_document_type", "knowledge_documents", ["document_type"]
    )
    ...
    op.create_index(
        "ix_knowledge_document_versions_status", "knowledge_document_versions", ["status"]
    )
    op.create_table(
        "knowledge_chunks",
        sa.Column("id", postgresql.UUID(as_uuid=True), nullable=False),
    ...
    op.create_index(
        "ix_knowledge_chunks_document_version_id", "knowledge_chunks", ["document_version_id"]
    )
    ...
    op.create_table(
        ...
        *ts,
        ...
        sa.UniqueConstraint("chunk_id", "model_name", name="uq_knowledge_embeddings_chunk_model"),
    )
    op.create_index(
        "ix_knowledge_embeddings_organization_id", "knowledge_embeddings", ["organization_id"]
    )
    ...
    op.create_table(
        ...
        *ts,
        ...
        sa.ForeignKeyConstraint(["parent_folder_id"], ["chat_folders.id"], ondelete="SET NULL"),
        sa.PrimaryKeyConstraint("id", name="pk_chat_folders"),
        sa.UniqueConstraint(
            "organization_id", "owner_user_id", "name", name="uq_chat_folders_owner_name"
        ),
    )
    ...
    op.create_table(
        ...
        sa.Column("is_shared_org", sa.Boolean(), server_default="false", nullable=False),
        sa.Column("shared_at", sa.DateTime(timezone=True), nullable=True),
        sa.Column("archived_at", sa.DateTime(timezone=True), nullable=True),
        sa.Column("memory_json", postgresql.JSONB(), nullable=True),
        sa.Column("metadata_json", postgresql.JSONB(), nullable=True),
        *ts,
        sa.ForeignKeyConstraint(["organization_id"], ["organizations.id"], ondelete="CASCADE"),
        sa.ForeignKeyConstraint(["owner_user_id"], ["users.id"], ondelete="CASCADE"),
    ...
    op.create_table(
        ...
        sa.Column("ab_group", sa.String(32), nullable=True),
        sa.Column("is_active", sa.Boolean(), server_default="true", nullable=False),
        sa.Column("metadata_json", postgresql.JSONB(), nullable=True),
        *ts,
        sa.ForeignKeyConstraint(["organization_id"], ["organizations.id"], ondelete="CASCADE"),
        ...
        sa.UniqueConstraint(
            "organization_id", "code", "version_number", name="uq_prompt_templates_org_code_ver"
        ),
    )
    ...
    op.create_table(
        ...
        *ts,
        ...
        sa.UniqueConstraint(
            "organization_id", "provider_kind", "provider_name", name="uq_provider_configs_org_kind"
        ),
    )
    ...
    op.create_table(
        ...
        sa.Column("preferred_embedding_model", sa.String(128), nullable=False),
        ...
        sa.Column("citation_mode", sa.String(32), nullable=False),
        sa.Column("language_code", sa.String(8), nullable=False),
        sa.Column("monthly_budget_usd", sa.Float(), nullable=True),
        sa.Column("config_json", postgresql.JSONB(), nullable=True),
        *ts,
        sa.ForeignKeyConstraint(["organization_id"], ["organizations.id"], ondelete="CASCADE"),
        sa.PrimaryKeyConstraint("id", name="pk_organization_ai_settings"),
        sa.UniqueConstraint("organization_id", name="uq_organization_ai_settings_org"),
    )
    op.create_index(
        "ix_organization_ai_settings_organization_id",
        "organization_ai_settings",
        ["organization_id"],
    )
    op.create_table(
        ...
        sa.Column("expanded_queries_json", postgresql.JSONB(), nullable=True),
        sa.Column("filters_json", postgresql.JSONB(), nullable=True),
        sa.Column("candidate_chunk_ids", postgresql.JSONB(), nullable=True),
        sa.Column("reranked_chunk_ids", postgresql.JSONB(), nullable=True),
        ...
        sa.Column("provider", sa.String(64), nullable=True),
        sa.Column("tool_usage_json", postgresql.JSONB(), nullable=True),
        sa.Column("metadata_json", postgresql.JSONB(), nullable=True),
        *ts,
        sa.ForeignKeyConstraint(["organization_id"], ["organizations.id"], ondelete="CASCADE"),
        sa.ForeignKeyConstraint(
    ...
    op.create_table(
        ...
        *ts,
        ...
        sa.PrimaryKeyConstraint("id", name="pk_conversation_feedback"),
        sa.UniqueConstraint("message_id", "user_id", name="uq_conversation_feedback_message_user"),
    )
    op.create_index(
        "ix_conversation_feedback_organization_id", "conversation_feedback", ["organization_id"]
    )
    op.create_table(
        "evaluation_runs",
        sa.Column("id", postgresql.UUID(as_uuid=True), nullable=False),
        ...
        sa.Column("name", sa.String(255), nullable=False),
        sa.Column("status", sa.String(32), nullable=False),
        sa.Column("dataset_json", postgresql.JSONB(), nullable=True),
    ...
    op.create_table(
        ...
        *ts,
        ...
        sa.UniqueConstraint(
            "organization_id", "owner_user_id", "name", name="uq_saved_searches_owner_name"
        ),
    )
