...
def seed_ai(session: Session, org: Organization, actor: User) -> None:
    ...
    existing = session.execute(
        select(KnowledgeDocument.id).where(
            ...
            KnowledgeDocument.slug == "sustainability-policy-demo",
        )
    ).scalar_one_or_none()
    if existing:
        return
    ...
    doc = KnowledgeDocument(
        ...
        title="Demo Sustainability Policy",
        slug="sustainability-policy-demo",
        document_type="policy",
        status="published",
        language_code=language,
        tags=["policy", "demo", "carbon"],
        permissions_json={"visibility": "organization", "roles": ["organization_member"]},
    ...
    version = KnowledgeDocumentVersion(
        ...
        version_number=1,
        status="published",
        file_name="sustainability-policy-demo.md",
        content_type="text/markdown",
        storage_path=f"seed://{org.id}/sustainability-policy-demo.md",
        checksum_sha256=content_hash(body),
    ...
    for piece in chunk_text(body, strategy="markdown", chunk_size=500, overlap=80):
        chunk = KnowledgeChunk(
            ...
            language_code=language,
            tags=doc.tags,
            permissions_json=doc.permissions_json,
            metadata_json={
        ...
        session.add(
            KnowledgeEmbedding(
                ...
                vector_json=vector,
                permissions_json=doc.permissions_json,
                metadata_json={"language": language, "document_id": str(doc.id)},
            )
        )
    if not session.execute(
        select(OrganizationAiSettings.id).where(OrganizationAiSettings.organization_id == org.id)
    ).scalar_one_or_none():
        session.add(
            OrganizationAiSettings(
                organization_id=org.id,
                preferred_provider="local_grounded",
                preferred_embedding_model="local-hash-384",
                temperature=0.1,
                max_tokens=1200,
                top_p=0.95,
                system_prompt="You are EcoTrace AI Sustainability Copilot. Ground answers in evidence.",
    ...
    if not session.execute(
        select(PromptTemplate.id).where(
            PromptTemplate.organization_id == org.id, PromptTemplate.code == "copilot.system"
        )
    ).scalar_one_or_none():
        session.add(
            PromptTemplate(
                ...
                scope="organization",
                version_number=1,
                template_body="Answer in {{language}}. Use only authorized EcoTrace evidence. Cite every claim. Organization={{organization_name}}.",
                variables_json=["language", "organization_name"],
                ab_group="A",
                is_active=True,
            )
        )
    for kind, name, model in (
        ("llm", "local_grounded", "ecotrace-local-grounded"),
        ("embedding", "local_hash", "local-hash-384"),
        ("reranker", "local_lexical", "local-lexical"),
        ("ocr", "tesseract", "tesseract"),
    ):
        exists = session.execute(
            select(ProviderConfig.id).where(
                ProviderConfig.organization_id == org.id,
                ProviderConfig.provider_kind == kind,
