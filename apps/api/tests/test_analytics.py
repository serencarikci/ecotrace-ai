...
def test_compute_target_progress_on_track() -> None:
    result = compute_target_progress(
        Decimal("100"),
        Decimal("70"),
        Decimal("50"),
        direction="decrease",
...
def test_compute_target_progress_unavailable() -> None:
    result = compute_target_progress(
        Decimal("100"), None, Decimal("50"), direction="decrease", elapsed_ratio=Decimal("0.5")
    )
...
def test_analytics_dashboard_approved_inventory(client: TestClient) -> None:
    ...
    response = client.get(
        f"/api/v1/organizations/{org_id}/analytics/dashboard", headers=auth_headers(token)
    )
...
def test_analytics_org_isolation(client: TestClient) -> None:
    ...
    response = client.get(
        f"/api/v1/organizations/{fake_org}/analytics/dashboard", headers=auth_headers(token)
    )
...
def test_scenario_calculate_does_not_mutate_activities(client: TestClient) -> None:
    ...
    before_items = {
        (row["id"], row["quantity"], row.get("rowVersion")) for row in before.json()["items"]
    }
    ...
    calc = client.post(
        f"/api/v1/organizations/{org_id}/scenarios/{scenario_id}/calculate",
        headers=auth_headers(token),
    )
    ...
    after_items = {
        (row["id"], row["quantity"], row.get("rowVersion")) for row in after.json()["items"]
    }
...
def test_target_progress_endpoint(client: TestClient) -> None:
    ...
    org_id = current_org_id(client, token)
    targets = client.get(
        f"/api/v1/organizations/{org_id}/sustainability-targets", headers=auth_headers(token)
    )
    ...
    progress = client.get(
        f"/api/v1/organizations/{org_id}/sustainability-targets/{target_id}/progress",
        headers=auth_headers(token),
    )
...
def test_executive_report_csv(client: TestClient) -> None:
    ...
    org_id = current_org_id(client, token)
    response = client.get(
        f"/api/v1/organizations/{org_id}/reports/executive",
        params={"format": "csv"},
        headers=auth_headers(token),
    )
    assert response.status_code == 200, response.text
