...
def test_agents_catalog_and_execute(client: TestClient) -> None:
    ...
    run = client.post(
        f"/api/v1/organizations/{org_id}/agents/carbon_analysis/execute",
        headers=auth_headers(token),
        json={"prompt": "Summarize our carbon inventory status for this organization."},
    )
...
def test_agent_cross_org_rejected(client: TestClient) -> None:
    ...
    response = client.post(
        f"/api/v1/organizations/{foreign}/agents/carbon_analysis/execute",
        headers=auth_headers(token),
        json={"prompt": "leak"},
    )
...
def test_automation_crud_activate_run(client: TestClient) -> None:
    ...
    org_id = current_org_id(client, token)
    created = client.post(
        f"/api/v1/organizations/{org_id}/automation-rules",
        headers=auth_headers(token),
        json={
            "code": "itest-weekly-anomaly",
            "name": "IT weekly anomaly",
    ...
    activated = client.post(
        f"/api/v1/organizations/{org_id}/automation-rules/{rule_id}/activate",
        headers=auth_headers(token),
    )
    assert activated.status_code == 200, activated.text
    assert activated.json()["status"] == "active"
    run1 = client.post(
        f"/api/v1/organizations/{org_id}/automation-rules/{rule_id}/run",
        headers=auth_headers(token),
    )
    assert run1.status_code == 200, run1.text
    paused = client.post(
        f"/api/v1/organizations/{org_id}/automation-rules/{rule_id}/pause",
        headers=auth_headers(token),
    )
...
def test_anomaly_and_data_quality_scan(client: TestClient) -> None:
    ...
    ran = client.post(
        f"/api/v1/organizations/{org_id}/anomaly-rules/{rule_id}/run", headers=auth_headers(token)
    )
    assert ran.status_code == 200, ran.text
    dq = client.post(
        f"/api/v1/organizations/{org_id}/data-quality/scan", headers=auth_headers(token)
    )
...
def test_forecast_definition_list(client: TestClient) -> None:
    ...
    org_id = current_org_id(client, token)
    defs = client.get(
        f"/api/v1/organizations/{org_id}/forecast-definitions", headers=auth_headers(token)
    )
...
def test_scheduled_report_run(client: TestClient) -> None:
    ...
    org_id = current_org_id(client, token)
    created = client.post(
        f"/api/v1/organizations/{org_id}/scheduled-reports",
        headers=auth_headers(token),
        json={
            "code": "itest-exec-report",
            "name": "IT Executive Report",
    ...
    client.post(
        f"/api/v1/organizations/{org_id}/scheduled-reports/{report_id}/activate",
        headers=auth_headers(token),
    )
    ran = client.post(
        f"/api/v1/organizations/{org_id}/scheduled-reports/{report_id}/run",
        headers=auth_headers(token),
    )
    assert ran.status_code == 200, ran.text
    generated = client.get(
        f"/api/v1/organizations/{org_id}/generated-reports", headers=auth_headers(token)
    )
