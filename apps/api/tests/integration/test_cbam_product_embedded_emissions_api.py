...
def _pee(org_id: str, binding_id: str) -> str:
    return f"{_base(org_id)}/reporting-period-bindings/{binding_id}/product-embedded-emissions"
...
def test_readiness_execute_replay_list_detail_summary(client: TestClient) -> None:
    ...
    assert body["exportedElectricityNoteCode"] == ("PROCESS_LEVEL_EXPORTED_ELECTRICITY_MODELED")
    assert body["internalPrecursorNoteCode"] == ("INTERNAL_PROCESS_PRECURSOR_LEONTIEF_MODELED")
...
def test_viewer_can_read_but_not_execute(client: TestClient) -> None:
    ...
    assert (
        client.get(f"{_pee(org_id, binding_id)}/summary", headers=_auth(viewer)).status_code == 200
    )
    denied = client.post(
        f"{_pee(org_id, binding_id)}/executions",
...
def test_execution_is_blocked_when_nothing_is_ready(client: TestClient) -> None:
    ...
    assert "PRODUCT_EMBEDDED_EMISSIONS_NO_ELIGIBLE_PRODUCTS" in (ready.json()["blockingIssueCodes"])
...
def test_unknown_result_returns_404(client: TestClient) -> None:
    ...
    missing = client.get(f"{_pee(org_id, binding_id)}/results/{uuid.uuid4()}", headers=_auth(admin))
