...
def test_cross_org_facility_denied(client: TestClient) -> None:
    ...
    org_id = _org_id(client, token)
    response = client.get(
        f"/api/v1/organizations/{org_id}/facilities/{facility_id}", headers=_auth(token)
    )
...
def test_csv_template_and_import(client: TestClient) -> None:
    ...
    org_id = _org_id(client, token)
    template = client.get(
        f"/api/v1/organizations/{org_id}/imports/activity-records/template", headers=_auth(token)
    )
