...
def test_facility_cross_org_returns_404(client: TestClient, engine) -> None:
    ...
    try:
        ...
        db.close()
    response = client.get(
        f"/api/v1/organizations/{other_id}/facilities/{facility_id}", headers=headers
    )
...
def test_period_lock_and_csv_validation(client: TestClient) -> None:
    ...
    lock = client.post(
        f"/api/v1/organizations/{org_id}/reporting-periods/{period['id']}/lock", headers=headers
    )
    assert lock.status_code == 200
    assert lock.json()["status"] == "locked"
    unlock = client.post(
        f"/api/v1/organizations/{org_id}/reporting-periods/{period['id']}/unlock", headers=headers
    )
    ...
    template = client.get(
        f"/api/v1/organizations/{org_id}/imports/activity-records/template", headers=headers
    )
    ...
    executed = client.post(
        f"/api/v1/organizations/{org_id}/imports/activity-records/{job_id}/execute", headers=headers
    )
