...
def test_login_success(client: TestClient) -> None:
    response = client.post(
        "/api/v1/auth/login", json={"email": "admin@ecotrace.dev", "password": "EcoTraceAdmin!2024"}
    )
    assert response.status_code == 200
    body = response.json()
...
def test_login_failure(client: TestClient) -> None:
    response = client.post(
        "/api/v1/auth/login", json={"email": "admin@ecotrace.dev", "password": "wrong-password"}
    )
...
def test_me_endpoint(client: TestClient) -> None:
    ...
    response = client.get(
        "/api/v1/auth/me", headers={"Authorization": f"Bearer {login['accessToken']}"}
    )
    assert response.status_code == 200
