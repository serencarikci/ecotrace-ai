...
def test_key_reuse_different_effective_reference_date(client: TestClient) -> None:
    ...
    a1 = _create_ng_activity(client, admin, org_id, binding_id, installation_id, activity_date=None)
    key = str(uuid.uuid4())
    first = client.post(
        _exec_url(org_id, binding_id),
        headers=_auth(admin),
...
def test_same_uuid_different_bindings_allowed(client: TestClient) -> None:
    ...
    a2 = _create_ng_activity(client, admin, org_id, b2, inst2, activity_date="2025-01-15")
    key = str(uuid.uuid4())
    r1 = client.post(
        _exec_url(org_id, b1),
        headers=_auth(admin),
...
def test_historical_null_client_request_id_readable(seeded_db) -> None:
    ...
    assert any(
        i.result_id == executed.result.id and i.client_request_id is None for i in page.items
    )
    ...
    try:
        count = verify.execute(
            ...
            .where(
                ...
                CbamStationaryCombustionResult.reporting_period_binding_id == ctx["binding_id"],
