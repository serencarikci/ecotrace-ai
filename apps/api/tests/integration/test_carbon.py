...
def test_factor_source_crud_authorization(client: TestClient) -> None:
    ...
    create = client.post('/api/v1/emission-factor-sources', headers=_auth(admin), json={'code': 'TEST_SRC', 'name': 'Test Source', 'publisher': 'EcoTrace', 'isDemo': True})
    assert create.status_code == 201, create.text
    source_id = create.json()['id']
    analyst = _login(client, 'analyst@ecotrace.dev', 'EcoTraceAnalyst!2024')
    denied = client.post('/api/v1/emission-factor-sources', headers=_auth(analyst), json={'code': 'NOPE', 'name': 'Nope'})
    ...
    archived = client.post(f'/api/v1/emission-factor-sources/{source_id}/archive', headers=_auth(admin))
...
def test_inventory_calculate_and_isolation(client: TestClient) -> None:
    ...
    inv = client.post(f'/api/v1/organizations/{org_id}/carbon-inventories', headers=_auth(token), json={'reportingPeriodId': period['id'], 'name': 'Integration Inventory', 'description': 'test', 'gwpDatasetCode': 'AR5-demo'})
    assert inv.status_code == 201, inv.text
    inventory_id = inv.json()['id']
    validation = client.post(f'/api/v1/organizations/{org_id}/carbon-inventories/{inventory_id}/validate', headers=_auth(token))
    assert validation.status_code == 200, validation.text
    body = validation.json()
    assert 'valid' in body
    assert 'missingFactors' in body
    calc = client.post(f'/api/v1/organizations/{org_id}/carbon-inventories/{inventory_id}/calculate', headers=_auth(token), json={'partialCalculation': True})
    assert calc.status_code == 200, calc.text
    assert calc.json()['engineVersion'] == '3.0.0'
    assert Decimal(str(calc.json()['totalKgCo2e'] or '0')) >= 0
    summary = client.get(f'/api/v1/organizations/{org_id}/carbon-inventories/{inventory_id}/summary', headers=_auth(token))
    assert summary.status_code == 200
    assert 'totalTCo2e' in summary.json()
    fake_org = '00000000-0000-0000-0000-000000000099'
    blocked = client.get(f'/api/v1/organizations/{fake_org}/carbon-inventories/{inventory_id}', headers=_auth(token))
...
def test_factor_activation_and_clone(client: TestClient) -> None:
    ...
    draft = client.post('/api/v1/emission-factors', headers=_auth(admin), json={'sourceId': source_id, 'code': 'EF-TEST-CLONE', 'name': 'Clone test factor', 'activityTypeId': electricity['id'], 'scope': 'scope_2', 'category': 'purchased_electricity', 'geographyCode': 'ZZ', 'unitCode': 'kWh', 'factorValue': '0.1', 'version': 1, 'validFrom': '2024-01-01', 'validTo': '2024-12-31'})
    ...
    cloned = client.post(f'/api/v1/emission-factors/{factor_id}/clone-version', headers=_auth(admin))
    assert cloned.status_code == 201
    assert cloned.json()['version'] == 2
    assert cloned.json()['status'] == 'draft'
    edit = client.patch(f'/api/v1/emission-factors/{factor_id}', headers=_auth(admin), json={'factorValue': '0.2'})
...
def test_matching_preview(client: TestClient) -> None:
    ...
    electricity = next((t for t in types if t['code'] == 'purchased_electricity'))
    facilities = client.get(f'/api/v1/organizations/{org_id}/facilities', headers=_auth(token)).json()['items']
    facility = next((f for f in facilities if f['code'] == 'IZM-PROD'))
    preview = client.post(f'/api/v1/organizations/{org_id}/factor-matching/preview', headers=_auth(token), json={'activityTypeId': electricity['id'], 'facilityId': facility['id'], 'activityDate': '2024-02-15', 'quantity': '1000', 'unitCode': 'kWh'})
