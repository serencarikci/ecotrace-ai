...
def test_factor_source_crud_authorization(client: TestClient) -> None:
    ...
    create = client.post(
        ...
        headers=_auth(admin),
        json={"code": "TEST_SRC", "name": "Test Source", "publisher": "EcoTrace", "isDemo": True},
    )
...
def test_inventory_calculate_and_isolation(client: TestClient) -> None:
    ...
    blocked = client.get(
        f"/api/v1/organizations/{fake_org}/carbon-inventories/{inventory_id}", headers=_auth(token)
    )
...
def test_factor_activation_and_clone(client: TestClient) -> None:
    ...
    edit = client.patch(
        f"/api/v1/emission-factors/{factor_id}", headers=_auth(admin), json={"factorValue": "0.2"}
    )
