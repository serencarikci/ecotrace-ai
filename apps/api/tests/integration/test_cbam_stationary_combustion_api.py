...
def test_natural_gas_parameter_resolution(client: TestClient) -> None:
    ...
    assert response.status_code == 200, response.text
    body = response.json()
    assert body['netCalorificValue'] == '48.00000000' or Decimal(body['netCalorificValue']) == Decimal(
        '48'
    )
...
def test_unresolved_date_and_unknown_fuel(client: TestClient) -> None:
    ...
    assert any(
        d.get('code') == 'UNRESOLVED_PARAMETER_SET'
        for d in _error_details(unresolved)
    )
    unknown = client.get(
...
def test_reference_date_outside_period_when_no_activity_date(client: TestClient) -> None:
    ...
    assert any(
        d.get('code') == 'REFERENCE_DATE_OUTSIDE_PERIOD' for d in _error_details(outside)
    )
...
def test_cross_org_and_binding_isolation(client: TestClient) -> None:
    ...
    other_binding, _ = _prepare_binding(
        client, admin, org_id, start='2025-01-01', end='2025-03-31'
    )
    ...
    assert any(
        d.get('code') == 'ACTIVITY_BINDING_MISMATCH'
        for d in _error_details(wrong_activity_binding)
    )
