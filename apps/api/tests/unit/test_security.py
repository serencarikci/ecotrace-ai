...
def test_expired_token_rejection() -> None:
    settings = Settings(SECRET_KEY='test-secret-key-that-is-long-enough-32chars', INITIAL_ADMIN_PASSWORD='EcoTraceAdmin!2024', APP_ENV='test')
    now = datetime.now(UTC)
    payload = {'sub': 'user-1', 'type': TOKEN_TYPE_ACCESS, 'roles': [], 'iat': int((now - timedelta(hours=2)).timestamp()), 'exp': int((now - timedelta(hours=1)).timestamp()), 'jti': 'expired'}
...
def test_production_rejects_insecure_secret() -> None:
    ...
    with pytest.raises(ValidationError):
        Settings(SECRET_KEY='change-me-to-a-long-random-secret-at-least-32-chars', INITIAL_ADMIN_PASSWORD='EcoTraceAdmin!2024', APP_ENV='production', APP_DEBUG='false')

def test_cors_origins_parsing() -> None:
    settings = Settings(SECRET_KEY='test-secret-key-that-is-long-enough-32chars', INITIAL_ADMIN_PASSWORD='EcoTraceAdmin!2024', APP_ENV='test', CORS_ALLOWED_ORIGINS='http://a.com, http://b.com')
