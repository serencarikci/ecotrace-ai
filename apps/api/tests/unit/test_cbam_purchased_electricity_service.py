...
def test_exported_electricity_separate_not_subtracted(seeded_db: Session) -> None:
    ...
    detail = get_purchased_electricity_result(seeded_db, user, org.id, binding.id, result.result_id)
...
def test_platform_default_execution_fails_closed(seeded_db: Session) -> None:
    ...
    assert any(d.get("code") == "UNRESOLVED_PLATFORM_DEFAULT" for d in (exc.value.details or []))
...
def test_negative_electricity_rejected_on_activity(seeded_db: Session) -> None:
    ...
    assert any(d.get("code") == "NEGATIVE_ELECTRICITY_QUANTITY" for d in (exc.value.details or []))
...
def test_incompatible_activity_unit_rejected(seeded_db: Session) -> None:
    ...
    assert any(d.get("code") == "INCOMPATIBLE_ACTIVITY_TYPE" for d in (exc.value.details or []))
...
def test_immutable_snapshot_and_current_history(seeded_db: Session) -> None:
    ...
    detail1 = get_purchased_electricity_result(seeded_db, user, org.id, binding.id, r1.result_id)
    detail2 = get_purchased_electricity_result(seeded_db, user, org.id, binding.id, r2.result_id)
...
def test_api_routes_and_tenant_isolation(client: TestClient) -> None:
    ...
    facilities = client.get(f"/api/v1/organizations/{org_id}/facilities", headers=headers).json()[
        "items"
    ]
    inst = client.post(
