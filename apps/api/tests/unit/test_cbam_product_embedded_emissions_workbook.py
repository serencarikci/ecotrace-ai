...
WORKBOOK = Path(
    os.environ.get(
        ...
        str(Path(__file__).resolve().parents[4] / "local-reference" / WORKBOOK_FILENAME),
    )
)

pytestmark = pytest.mark.skipif(not WORKBOOK.is_file(), reason="CBAM SEE workbook not available")
...
def test_d_processes_attributed_emission_formulas_and_signs() -> None:
    ...
    assert ws["T62"].value == ('=IF(G11="","",SUM(L61*CONST_EFNatGas-M61*CONST_EFNatGas*0.667))')
