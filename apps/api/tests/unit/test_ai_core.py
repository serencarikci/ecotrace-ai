...
from ecotrace.modules.ai_copilot.application.evaluation_service import (
    citation_accuracy,
    faithfulness_score,
...
def test_local_grounded_llm_requires_evidence() -> None:
    ...
    result = llm.complete(
        messages=[
            ChatMessage(role="system", content="No evidence here"),
            ChatMessage(role="user", content="How much CO2?"),
        ],
        temperature=0.1,
        max_tokens=100,
        top_p=0.9,
...
def test_local_grounded_llm_with_evidence() -> None:
    ...
    result = llm.complete(
        messages=[
            ChatMessage(role="system", content="[E1] Inventory total is 12.5 tCO2e for 2024."),
            ChatMessage(role="user", content="What is the inventory total?"),
        ],
        temperature=0.1,
        max_tokens=200,
        top_p=0.9,
...
def test_reranker_orders_by_overlap() -> None:
    ...
    ranked = reranker.rerank(
        query="carbon inventory scope",
        documents=["unrelated text", "carbon inventory scope 1 totals", "passport slug"],
        top_k=2,
    )
    assert ranked[0][0] == 1
...
def test_ingestion_txt_and_slug() -> None:
    text, meta = extract_text_from_bytes(
        content=b"Sustainability policy for carbon management.",
...
def test_evaluation_metrics() -> None:
    ...
    assert (
        faithfulness_score(answer="carbon inventory total", evidence_texts=["carbon inventory"]) > 0
    )
