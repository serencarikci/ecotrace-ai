...
def test_incorrect_period_wide_formula_not_used_by_monthly_share() -> None:
    ...
    period = period_wide_shortcut_share(
        total_d=Decimal('927'), total_e=Decimal('69.03')
    )
    assert s1 != period
    assert s2 != period
...
def test_d0_e_positive_fails() -> None:
    with pytest.raises(ValueError, match='TOTAL_PRODUCTION_MUST_BE_POSITIVE'):
        monthly_cbam_share(
            total_production_tonnes=Decimal('0'), cbam_quantity_tonnes=Decimal('1')
        )
...
def test_largest_remainder_exact_balance_and_tie_break() -> None:
    ...
    ordered = sorted(rows, key=lambda r: (-(r.raw_allocated - r.final_allocated + r.rounding_adjustment), str(r.group_id)))
