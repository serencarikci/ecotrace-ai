...
def test_superseded_rejected_for_new_link_but_historical_readable(seeded_db) -> None:
    ...
    assert v2.version == 2
    still = production_record_service.get_production_record(seeded_db, admin, org.id, record.id)
...
def test_legacy_null_profile_readable_and_linkable(seeded_db) -> None:
    ...
    seeded_db.flush()
    viewed = production_record_service.get_production_record(seeded_db, admin, org.id, legacy.id)
