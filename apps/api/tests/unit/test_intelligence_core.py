...
def test_target_trajectory_labels() -> None:
    assert (
        target_trajectory_label(current=100, target=80, forecast_at_target=75, periods_remaining=6)
        == "likely_on_track"
    )
    assert (
        target_trajectory_label(
            current=100, target=80, forecast_at_target=None, periods_remaining=6
        )
        == "insufficient_data"
    )
