...
def test_execute_golden_path_exact_decimals_and_idempotency(seeded_db) -> None:
    ...
    assert abs(GOLDEN_CBAM_FOSSIL_CO2_RAW - GOLDEN_CBAM_FOSSIL_CO2_RAW_STORED) < Decimal(
        '1e-17'
    )
...
def test_precondition_codes_independent(seeded_db) -> None:
    ...
    act = create_ng_activity(
        seeded_db, user, organization, binding2, installation2, qty=Decimal('10'), day=date(2024, 7, 15)
    )
    ...
    seeded_db.flush()
    r2 = get_direct_emissions_allocation_readiness(
        seeded_db, user, organization.id, binding2.id
    )
...
def test_precondition_monthly_basis_missing(seeded_db) -> None:
    ...
    act = create_ng_activity(
        seeded_db, user, organization, binding, installation, qty=Decimal('188'), day=date(2024, 7, 15)
    )
...
def test_list_and_summary(seeded_db) -> None:
    ...
    summary = get_direct_emissions_allocation_summary(
        seeded_db, user, organization.id, binding.id
    )
...
def test_same_profile_version_grouped_different_versions_separate(seeded_db) -> None:
    ...
    for mday, qty, d, e in WORKBOOK_MONTHS[1:]:
        ...
        p = ensure_org_product(seeded_db, organization.id, code=f'PX-{mday.month}-{uuid.uuid4().hex[:3]}')
    ...
    same = next(p for p in detail.product_allocations if p['productProfileVersionId'] == str(profile.id))
...
def test_stale_source_activity_quantity(seeded_db) -> None:
    ...
    seeded_db.flush()
    codes = compute_allocation_stale_reasons(
        seeded_db, user, organization.id, binding.id, result
    )
...
def test_stale_production_quantity(seeded_db) -> None:
    ...
    production_record_service.update_production_record(
        ...
        organization.id,
        row.id,
        ProductionRecordUpdate(row_version=row.row_version, quantity=row.quantity + Decimal('0.01')),
    )
    codes = compute_allocation_stale_reasons(
        seeded_db, user, organization.id, binding.id, result
    )
...
def test_stale_methodology_or_workbook_on_result(seeded_db) -> None:
    ...
    seeded_db.flush()
    codes2 = compute_allocation_stale_reasons(
        seeded_db, user, organization.id, binding.id, result
    )
