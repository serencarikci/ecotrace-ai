...
from ecotrace.modules.reference_data.application.unit_conversion import (
    convert_between,
    convert_to_base,
    normalize_quantity,
)
...
def test_normalize_quantity_checks_dimension(seeded_db) -> None:
    ...
    with pytest.raises(ValidationAppError):
        normalize_quantity(
            seeded_db, quantity=Decimal("10"), unit_code="kg", activity_type=activity
        )
...
def test_normalize_quantity_mwh_to_kwh(seeded_db) -> None:
    ...
    seed_units(seeded_db)
    types = seed_activity_types(seeded_db)
    qty, unit = normalize_quantity(
        seeded_db,
        quantity=Decimal("1.5"),
        unit_code="MWh",
        activity_type=types["purchased_electricity"],
    )
    assert unit == "kWh"
