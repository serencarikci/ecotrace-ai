...
def test_submit_approve_workflow(seeded) -> None:
    ...
    submitted = submit_record(
        ...
        org.id,
        draft.id,
        ReasonRequest(row_version=draft.row_version),
    )
    ...
    approved = approve_record(
        ...
        org.id,
        draft.id,
        ReasonRequest(row_version=submitted.row_version),
    )
...
def test_reject_requires_reason(seeded) -> None:
    ...
    submitted = db_session.execute(
        select(ActivityRecord).where(
            ActivityRecord.organization_id == org.id,
            ActivityRecord.status == "submitted",
        )
    ).scalar_one()
