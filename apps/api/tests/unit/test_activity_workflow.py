...
def seeded(seeded_db):
    ...
    analyst = seeded_db.execute(
        select(User).where(User.normalized_email == normalize_email("analyst@ecotrace.dev"))
    ).scalar_one()
    org_admin = seeded_db.execute(
        select(User).where(User.normalized_email == normalize_email("orgadmin@ecotrace.dev"))
    ).scalar_one()
...
def test_submit_approve_workflow(seeded) -> None:
    ...
    draft = db_session.execute(
        select(ActivityRecord).where(
            ActivityRecord.organization_id == org.id, ActivityRecord.status == "draft"
        )
    ).scalar_one()
    submitted = submit_record(
        db_session, analyst, org.id, draft.id, ReasonRequest(row_version=draft.row_version)
    )
    assert submitted.status == "submitted"
    approved = approve_record(
        db_session, org_admin, org.id, draft.id, ReasonRequest(row_version=submitted.row_version)
    )
...
def test_optimistic_concurrency(seeded) -> None:
    ...
    with pytest.raises(ConflictError):
        update_record(
            ...
            org.id,
            draft.id,
            ActivityUpdate(row_version=draft.row_version + 5, notes="stale"),
        )
...
def test_locked_period_blocks_mutations(seeded) -> None:
    ...
    period = db_session.execute(
        select(ReportingPeriod).where(
            ReportingPeriod.organization_id == org.id, ReportingPeriod.code == "2024-01"
        )
    ).scalar_one()
...
def test_reject_requires_reason(seeded) -> None:
    ...
    submitted = db_session.execute(
        select(ActivityRecord).where(
            ActivityRecord.organization_id == org.id, ActivityRecord.status == "submitted"
        )
    ).scalar_one()
    with pytest.raises(ValidationAppError):
        reject_record(
            ...
            org.id,
            submitted.id,
            ReasonRequest(reason="   ", row_version=submitted.row_version),
        )
