...
def test_facility_coordinate_validation(seeded: Session) -> None:
    ...
    with pytest.raises(ValidationAppError):
        facility_service.create_facility(seeded, admin, org.id, FacilityCreate(code='BAD-LAT', name='Bad', facility_type='office', country_code='TR', city='Izmir', timezone='Europe/Istanbul', latitude=Decimal('95'), longitude=Decimal('27')))
...
def test_facility_date_validation(seeded: Session) -> None:
    ...
    assert org is not None
    with pytest.raises(ValidationAppError):
        facility_service.create_facility(seeded, admin, org.id, FacilityCreate(code='BAD-DATE', name='Bad dates', facility_type='office', country_code='TR', city='Izmir', timezone='Europe/Istanbul', operational_start_date=date(2024, 6, 1), operational_end_date=date(2024, 1, 1)))
...
def test_locked_period_rejects_writes(seeded: Session) -> None:
    period = ReportingPeriod(organization_id=seeded.execute(select(Organization.id)).scalar_one(), code='LOCK-TEST', name='Locked', period_type='monthly', start_date=date(2025, 1, 1), end_date=date(2025, 1, 31), status='locked')
    seeded.add(period)
    seeded.flush()
