...
@Component({
  selector: 'app-cbam-direct-emissions-allocation',
  standalone: true,
  imports: [
    DatePipe,
    MatButtonModule,
  ...
  templateUrl: './direct-emissions-allocation.component.html',
  styleUrl: './direct-emissions-allocation.component.scss',
})
export class CbamDirectEmissionsAllocationComponent {
  ...
  readonly readiness = signal<CbamDirectEmissionsAllocationReadiness | null>(null);
  readonly summary = signal<CbamDirectEmissionsAllocationPeriodSummary | null>(null);
  readonly results = signal<CbamDirectEmissionsAllocationResultSummary[]>([]);
  readonly resultsTotal = signal(0);
  readonly resultsPage = signal(1);
  readonly resultsPageSize = signal(10);
  readonly selectedDetail = signal<CbamDirectEmissionsAllocationResultDetail | null>(null);
  ...
  readonly monthTotal = monthCbamTotal;
    ...
    this.load$
      .pipe(
        switchMap((bindingId) => {
          ...
          return forkJoin({
            readiness: this.api.getDirectEmissionsAllocationReadiness(bindingId),
            summary: this.api.getDirectEmissionsAllocationSummary(bindingId).pipe(
              catchError(() => of(null)),
            ),
            results: this.api.listDirectEmissionsAllocationResults(bindingId, {
              page,
              pageSize,
            }),
          }).pipe(
  ...
  executeAllocation(): void {
    ...
    this.api
      .executeDirectEmissionsAllocation(this.bindingId(), { clientRequestId })
      .subscribe({
        next: (response) => {
  ...
  private loadDetail(resultId: string): void {
    ...
    this.api.getDirectEmissionsAllocationResult(this.bindingId(), resultId).subscribe({
      next: (detail) => {
